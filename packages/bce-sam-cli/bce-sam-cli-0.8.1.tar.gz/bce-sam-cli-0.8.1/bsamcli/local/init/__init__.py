...
LAYER_RUNTIME_TEMPLATE_MAPPING = {
    "python2": os.path.join(_templates, "cookiecutter-bsam-layer-python"),
    "python3": os.path.join(_templates, "cookiecutter-bsam-layer-python"),
    "nodejs10": os.path.join(_templates, "cookiecutter-bsam-layer-nodejs"),
    "nodejs12": os.path.join(_templates, "cookiecutter-bsam-layer-nodejs"),
    "php7": os.path.join(_templates, "cookiecutter-bsam-layer-common"),
    "lua5.3": os.path.join(_templates, "cookiecutter-bsam-layer-common"),
    "golang": os.path.join(_templates, "cookiecutter-bsam-layer-common"),
    "java8": os.path.join(_templates, "cookiecutter-bsam-layer-common"),
    "dotnetcore2.2": os.path.join(_templates, "cookiecutter-bsam-layer-common"),
    "powershell6.2": os.path.join(_templates, "cookiecutter-bsam-layer-common"),
}
...
def generate_layer_project(name='bsam-sample-layer', compatible_runtimes=("nodejs10", "nodejs12"),
        output_dir="."):
    """Generates layer project using cookiecutter and options given
    ...
    Parameters
    ----------
    compatible_runtimes: tuple, optional
        Layer compatiable Runtimes (the default is "nodejs10,nodejs12")
    output_dir: str, optional
        Output directory where project should be generated
        (the default is ".", which implies current folder)
    name: str, optional
        Name of the layer
        (the default is "bsam-sample-layer", which implies a project named bsam-sample-layer will be created)
    ...
    runtime = get_layer_demo_runtime(compatible_runtimes)
    params = {
        "template": LAYER_RUNTIME_TEMPLATE_MAPPING[runtime],
        "output_dir": output_dir,
    }
    ...
    params['extra_context'] = {'layer_name': name, 'compatible_runtimes': ','.join(compatible_runtimes)}
    params['no_input'] = True

    try:
        LOG.debug("Baking a new layer template with cookiecutter with all parameters")
        cookiecutter(**params)
    except CookiecutterException as e:
        raise GenerateProjectFailedError(project=name, provider_error=e)

def get_layer_demo_runtime(compatible_runtimes):    
    for r in compatible_runtimes:
        if r.find("nodejs") or r.find("python"):
            return r
    return "java8" # that means use common layer demo