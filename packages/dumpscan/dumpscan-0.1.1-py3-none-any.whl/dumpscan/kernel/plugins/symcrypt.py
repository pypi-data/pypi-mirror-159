        ...
        for proc in pslist.PsList.list_processes(
            ...
            for offset, rule_name, _, _ in layer.scan(
                ...
                if matching_cert:
                    ...
                    match_string = f"[green]{thumbprint}[/green] -> {subject}"
                else:
                    # If there's no matching cert, then print out first 20 bytes of modulus
                ...
                if key.hasPrivateKey and output:
                    ...
                    if matching_cert:
                        ...
                        with self.open(f"{filename}.pfx") as f:
                            f.write(pfx)
                    else:
                        with self.open(f"{mod_str[:40].upper()}_modulus.key") as f:
                            f.write(
