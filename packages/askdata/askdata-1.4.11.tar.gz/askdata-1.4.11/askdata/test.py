    ...
    if check_component_types:
        ...
        for position, component in enumerate(check_component_types):
            ...
            if check_component(response, position) == component:
                ...
                #failed['Component', position] = check_component(response, position)
    ...
    if check_table_columns:
        ...
        for position, columns in enumerate(check_table_columns):
            ...
            if columns != []:  
                ...
                if check_col_names(response, position) == set(columns):
                    ...
                    #failed['Table', position] = check_col_names(response, position)
                    
                    #failed_message += "<br><b>Expected columns</b>: {}, but given {}  ".format(columns, list(check_col_names(response, position)))
