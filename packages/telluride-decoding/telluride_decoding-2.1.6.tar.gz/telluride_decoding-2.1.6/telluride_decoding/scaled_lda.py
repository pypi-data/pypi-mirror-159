...
class LinearDiscriminantAnalysis(object):
  ...
  def fit(self, x: np.ndarray, y: np.ndarray):
    ...
    scatter_matrix = np.linalg.inv(scatter_within).dot(scatter_between)
    scatter_matrix += self._lambda * np.eye(scatter_matrix.shape[0])

    try:
      eigen_vals, eigen_vecs = np.linalg.eig(scatter_matrix)
    except np.linalg.LinAlgError:
      logging.warn('Incomplete scatter matrix:', scatter_matrix)
      self._w = np.eye(scatter_matrix.shape[0])
      return
