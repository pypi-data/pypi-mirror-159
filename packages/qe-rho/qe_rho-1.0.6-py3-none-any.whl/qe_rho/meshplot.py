...
def pwscfplot(pwscf, compare=True):
    data = pwscf.rho_g2r()
    a = pwscf.cell.at
    mesh_plot(data, output_name='SAD.pdf', filtering='average', title='SAD', a=a)
    if exists(join('pwscf.save', 'charge-density.dat')):
        data_rhor = pwscf.read_charge(join('pwscf.save', 'charge-density.dat'))
        mesh_plot(data_rhor, output_name='rhor.pdf', filtering='average', title='converged', a=a)
        if compare:
            mesh_plot(np.abs(data_rhor-data), output_name='diff.pdf', filtering='average', title='diff', a=a)
