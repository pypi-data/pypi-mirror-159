    ...
    def sync_callback(self, response={}):
        if super().get_bank_id() in [super().EQUITY]:
            ...
            if super().get_bank_id() == super().EQUITY:
                ...
                response["amount"] = request.get("destinations", [])[
                    0].get("amount", -1)

                if response.get("code") == 0:
                    response["completed"] = True

                callback_url = request.get("transfer", {}).get("callback_url", "")

                t1 = threading.Thread(target=self._send_to_callback,
                                    args=[response, callback_url])
                t1.start()

    def get_callback_data(self, callback_data={}, forward=True):
        ...
        if 'MessageReference' in callback_data.keys() or 'messageReference' in callback_data.keys() :
            ...
            if len(callback):
                ...
                response["code"] = int(callback_data.get("MessageCode",callback_data.get("messageCode", -1)))
                response["account_reference"] = callback_data.get("MessageReference", callback_data.get("messageReference", ""))
                response["date"] = callback_data.get("MessageDateTime", callback_data.get("messageDateTime", ""))
                ...
                if 'Destinations' in callback_data.keys():
                    response["amount"] = callback_data.get(
                        "Destinations", {}).get("Amount", "")
                elif 'destination' in callback_data.keys():
                    response["amount"] = callback_data.get(
