...
'''
store data collected in files for potential later use
INPUT: start_t- string that represents the start date of data collection. end_t- string that represents the end date of data collection
        json_max- int number of tweets collected per interval. interval_len- int that represents number of minutes in interval
'''
...
'''
INPUT: prefix- string representing the file prefix to search for
        json_max- max number of results stored in file for each time interval
        interval_len- int that represents length of collection intervals in minutes
        has_sample- boolean that represents whether the file is a comparison sample or the data specified in the query parameters
OUTPUT: tuple that contains the string file name that was searched for, the string start and end date of data storage and the int length of data collection in minutes
'''
def find_file(onlyfiles, prefix, json_max, interval_len, has_sample):
    past_file = ""

    #search in specified directory for sentiment file that matches specified data storage file values (which are contained within the file name)
