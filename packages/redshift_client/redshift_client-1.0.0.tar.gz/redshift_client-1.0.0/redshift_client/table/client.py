...
class TableClient:
    ...
    def move(self, source: TableId, target: TableId) -> Cmd[None]:
        return self.append(source, target) + self.delete_cascade(source)

    def overwrite(self, source: TableId, target: TableId) -> Cmd[None]:
        nothing = Cmd.from_cmd(lambda: None)
        recreate = self.exist(target).bind(
            lambda b: self.delete_cascade(target)
            + self.create_like(source, target)
            if b
            else nothing
        )
        return recreate + self.move(source, target)
