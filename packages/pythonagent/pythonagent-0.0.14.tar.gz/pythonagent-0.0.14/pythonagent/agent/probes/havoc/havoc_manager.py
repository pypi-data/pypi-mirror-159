...
class NDNetHavocMonitor(object):
    ...
    def parse_nethavoc_config(self, json_conf, header_dict):
        ...
        try:
            ...
            havoc_type = havoc_conf.havocType
            print("havoc_conf in parser", havoc_conf)
            self.logger.info("Havoc type: {}".format(havoc_type))
            print("havoc type", havoc_type)
            ...
            print("havoc_type_full_str", havoc_type_full_str)
            print("nethavocmap", self.netHavocConfigMap)
            config_list = self.netHavocConfigMap[havoc_type_full_str]
            print("initial config list", config_list)
            self.logger.info("config_list: {}".format(config_list))

            if config_list:
                for config in config_list:
                    if nhmid == config.nhmid:
                        print("config in parser", config)
    ...
    def apply_method_call_delay(self, req_method_fqm):
        ...
        print("config_list inside havoc mamanger method call delay", config_list)
    ...
    def increase_memory_usage(self):
        print("Trying to apply memory leak")
        ...
        if config_list:
            for config in config_list:
                ...
                print("HERE")
                print("leaksinmb", leaksINMB)
                totalDurationInSec = config.totalDurationInSec
                print("totalDurationInSec", totalDurationInSec)
                memoryLeaksInSec = config.memoryLeaksInSec
                print("memoryLeaksInSec", memoryLeaksInSec)
                apply_memory_leak(leaksINMB, totalDurationInSec, memoryLeaksInSec)

        else:
            self.logger.error("config not set for apply_method_call_failure")
