...
class CALCSModel(DARMModel):
    ...
    def drivealign_out_to_displacement(self, frequencies, arm, stage):
        """
        Output of CALCS DRIVEALIGN to approximate DARM displacement
        ...
        if hasattr(self.calcs, f'{arm.lower()}arm_{stage.lower()}_coiloutf'):
            val = getattr(self.calcs, f'{arm.lower()}arm_{stage.lower()}_coiloutf')
            if val != '':
        ...
        if hasattr(self.calcs, f'{arm.lower()}arm_{stage.lower()}_analog'):
            val = getattr(self.calcs, f'{arm.lower()}arm_{stage.lower()}_analog')
            if val != '':
    ...
    def compute_actuation_single_stage(self, frequencies, arm, stage):
        ...
        CALCS approximation of a specific arm, stage from DARM_CTRL to DARM
        ...
        calcs_stage_actuation : `complex128`, array-like
            CALCS approximation of DARM_CTRL to displacement transfer function
        """
        ...
        if (output_matrix[arm_idx, stage_idx] != 0.0):
            ...
            drivealign_out_to_disp = self.drivealign_out_to_displacement(
                frequencies, arm=arm, stage=stage)
            calcs_stage_actuation = (output_matrix[arm_idx, stage_idx] *
                                     dig_filt[stage_idx - 1] *
    ...
    def gds_actuation_correction(self, frequencies, stage, daqdownsample=True):
        ...
        # Compute ideal model sum of actuators for A_{xn} + A_{yn}
        A_x = 0
        A_y = 0
        if (self.actuation.xarm is not None and
                hasattr(self.actuation.xarm, 'suspension_file') and
                self.actuation.xarm.suspension_file != ''):
            A_x = self.actuation.xarm.compute_actuation_single_stage(
                frequencies, stage=stage)
        if (self.actuation.yarm is not None and
                hasattr(self.actuation.yarm, 'suspension_file') and
                self.actuation.yarm.suspension_file != ''):
            A_y = self.actuation.yarm.compute_actuation_single_stage(
                frequencies, stage=stage)
        A_n = A_x + A_y

        # Compute sum of CALCS approximated A_{xn,calcs} + A_{yn,calcs}
        A_x_calcs = 0
        A_y_calcs = 0
        if (self.actuation.xarm is not None and
                hasattr(self.calcs, f'xarm_{stage.lower()}_analog') and
                getattr(self.calcs, f'xarm_{stage.lower()}_analog') != ''):
            A_x_calcs = self.compute_actuation_single_stage(
                frequencies, arm='x', stage=stage)
        if (self.actuation.yarm is not None and
                hasattr(self.calcs, f'yarm_{stage.lower()}_analog') and
                getattr(self.calcs, f'yarm_{stage.lower()}_analog') != ''):
            A_y_calcs = self.compute_actuation_single_stage(
                frequencies, arm='y', stage=stage)
        A_n_calcs = A_x_calcs + A_y_calcs
    ...
    def calcs_darm_actuation(self, frequencies):
        ...
        # Assume the same as in-loop DARM output matrix
        # we won't actually multiply these values it's just to help
        # loop through stages that are used
        output_matrix = self.actuation.darm_output_matrix_values()

        # Start with zeros
        calcs_actuation = np.zeros(len(frequencies), dtype='complex128')

        # Loop over arms and stages to compute the digital filters and scale/sum
        # everything together
        # output matrix * digital filters * calcs digital out to displacement
        for arm_idx, arm in enumerate(['x', 'y']):
            for stage_idx, stage in enumerate(['top', 'uim', 'pum', 'tst']):
                if output_matrix[arm_idx, stage_idx] != 0.0:
                    ...
                    calcs_actuation += (
                        self.compute_actuation_single_stage(
                            frequencies, arm=arm, stage=stage))
