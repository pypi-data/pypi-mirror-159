...
class PrizeBaseModel():
    ...
    def _delete_act_prize_dependency_key(self, act_id, model_id, prize_id, delay_delete_time=0.01):
        ...
        if act_id:
            if model_id:
                dependency_key_list.append(f"act_prize_list:actid_{act_id}_moduleid_{model_id}")
            else:
    ...
    def get_act_prize_list(self,app_id,act_id,module_id,prize_name,ascription_type,is_del,page_size,page_index,order_by="sort_index desc,id asc",condition="",params=[],field="*",is_cache=True):
        ...
        if is_cache:
            ...
            page_list, total = act_prize_model.get_cache_dict_page_list(field=field, page_index=page_index, page_size=page_size, where=where, group_by="", order_by=order_by, params=params_list,dependency_key=dependency_key)
        else:
