...
from .lib.utils import has_value
...
class IsRequired(PatternValidator):

    def __init__(self, blank: bool | list = False):
        pattern = [None, ''] if blank else []
        super().__init__(*pattern, deny=True)

    def validate(self, value, key, data):
        if has_value(data, key):
            return super().validate(value, key, data)
...
def _create_test(origin):
    return lambda value: type(value) == origin
