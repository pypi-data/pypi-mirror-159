...
def data_pipeline(df: pd.DataFrame, check_object_on_float: bool = False) -> Tuple:
    ...
    categ_columns = set([col for col in df.columns if df[col].dropna().nunique() <= 50])
    ...
    int_columns = (int_columns | float_to_int_cols) - categ_columns
    float_columns = float_columns - categ_columns - int_columns
    str_columns = set(tmp_df.columns) - float_columns - categ_columns - int_columns
    ...
    return str_columns, float_columns, categ_columns, date_columns, int_columns
