...
class Dataset:
    def __init__(self, df: pd.DataFrame, metadata: dict, keys_mode: str, kde_path: str):
        self.df = df
    ...
    def _train_kde(self):
        pk_column = self.df[self.primary_key_name]
        if self.primary_key_type is float:
            bw_width = len(pk_column) / len(pk_column) ** 1.3
            pk_column = gaussian_kde(pk_column, bw_method=bw_width)

        with open(self.kde_path + "pk_column.pkl", "wb") as file:
            dill.dump(pk_column, file)
        logger.info(
            f"The kde parameters of {self.primary_key_name} is saved at {self.kde_path}"
        )
    ...
    def pipeline(self) -> pd.DataFrame:
        ...
        self.primary_key_type = (
            str
            if self.primary_key_name in (str_columns | categ_columns | date_columns)
            else float
        )
        ...
        if self.foreign_key_name and self.keys_mode == "multable":
