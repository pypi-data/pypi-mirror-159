...
class TokenGrabber:
    ...
    async def get_token(self) -> Dict[str, str]:
        result: Dict[str, str] = await self._get_token()
    ...
    async def _get_token_data(self) -> dict:
        ...
        if 'token' in response_text:
            ...
            print(f'Требуется подтверждение по почте для {self.user.email}')
            time.sleep(2)
            link: str = MailReader(
            ...
            self.browser.close()
            return await self._get_token_data()
            # todo заходить на почту и разрешать доступ
    ...
    @staticmethod
    def get_browser(disable_gpu: bool = True):
