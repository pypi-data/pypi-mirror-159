...
class Login:
	def __init__(
		...
		self.__api = API()
		self.__spo = Spo()
		self.__download_job = Download_JOB(self.__api, self.__gw_api)
	...
	def download_trackdee(
		...
		track = DW_TRACK(preferences, self.__download_job).dw()
	...
	def download_albumdee(
		...
		album = DW_ALBUM(preferences, self.__download_job).dw()
	...
	def download_playlistdee(
		...
		playlist = DW_PLAYLIST(preferences, self.__download_job).dw()
