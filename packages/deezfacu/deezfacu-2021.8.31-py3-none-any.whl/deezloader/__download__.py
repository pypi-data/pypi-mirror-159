		...
		try:
			...
			for a in range(
				...
				if "errors" in medias[a]:
					c_media_json = self.__get_url(infos_dw[a], quality_download)
...
class EASY_DW:
	...
	def easy_dw(self) -> Track:
		...
		try:
			...
			try:
				self.__fallback_ids = self.__api.not_found(song, self.__song_metadata['music'])
				self.__infos_dw = self.__gw_api.get_song_data(self.__fallback_ids)
				print(self.__fallback_ids, self.__ids)
