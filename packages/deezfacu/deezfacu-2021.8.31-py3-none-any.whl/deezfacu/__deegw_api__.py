...
class API_GW:
	def __init__(
		...
		self.__client_id = 551722
		self.__client_secret = "f20f8b7902425158e5954e0fa8a72699"
		self.__try_link = "https://api.deezer.com/platform/generic/track/3135556"
	...
	def __set_arl(self):
		...
		self.__req.get(self.__try_link, headers = c_headers).json()
	...
	def __get_access_token(self):
		...
		to_hash = (
			f"{self.__client_id}{self.__email}{password}{self.__client_secret}"
		)

		request_hash = md5hex(to_hash)

		params = {
			"app_id": self.__client_id,
			"login": self.__email,
			"password": password,
			"hash": request_hash
		}
