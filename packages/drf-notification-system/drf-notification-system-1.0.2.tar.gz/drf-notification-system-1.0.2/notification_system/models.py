...
class NotificationEvent(BaseModel):
    ...
    recipients = models.ManyToManyField(to=User, verbose_name=_('members'), related_name='recipients', blank=True)
    ...
    def get_recipients(self):
        ...
        if self.notification_group:
            recipients = recipients | self.notification_group.members.all()

        return recipients.filter(is_active=True, email__isnull=False).distinct()
