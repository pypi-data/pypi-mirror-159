...
def derive_parameters_from_tce_xml(response):
    tree = ET.parse(response)
    root = tree.getroot()
    planet_results_list = root.findall("dv:planetResults", NS)

    params_for_mask_list = []
    for planet_results in planet_results_list:
        parameter = planet_results.find("dv:allTransitsFit/dv:modelParameters", NS)
        period = float(parameter.find("dv:modelParameter/[@name='orbitalPeriodDays']", NS).get("value"))
        duration = float(parameter.find("dv:modelParameter/[@name='transitDurationHours']", NS).get("value")) / 24
        t0 = float(parameter.find("dv:modelParameter/[@name='transitEpochBtjd']", NS).get("value"))

        if 0.5 < period < 5 and duration < 0.4:
            params_for_mask_list.append((period, max(0.4, 2 * duration), t0))
