...
class FlareLightCurve(LightCurve):
    ...
    def _remove_model(self):
        if 0 < self.period <= 2:
            period_array = np.array([1, 2]) * self.period
            ...
            if 0 < self.period <= 2:
                ...
                return sigma_clip(model_flux, sigma_upper=10000, sigma_lower=3.5, masked=True).filled(np.nan)
        return 0
    ...
    def mark_tce(self):
        """Mark if the candidates are caused by TCEs."""

        self._generate_mask_from_tce()
        if self.candidates is not None:
            tce = np.zeros(len(self.candidates), dtype=bool)
            for row in self.candidates.itertuples():
                tce[row.Index] = self._is_tce(row.i_peak)

            self.candidates.tce = tce
    ...
    def plot_candidates(self, figure_folder, threshold=0.5):
        ...
        if self.candidates is not None:
            self.candidates.loc[self.candidates.tce, "flare_prob"] = -99
            ...
            for row in self.candidates.itertuples():
                ...
                ax_detrended_lc.scatter(
                    self.time.value[self.tce_mask], self.flux[self.tce_mask], s=1, lw=0, c="tab:orange"
                )
