    ...
    def _remove_model(self):
        if 0 < self.period <= 2:
            ...
            if 0 < self.period <= 2:
                ...
                model_flux = sigma_clip(model_flux, sigma_upper=100000, masked=True).filled(np.nan)
                return pd.Series(model_flux).interpolate(method="linear", limit_direction="both", limit=3).to_numpy()
    ...
    def _generate_mask_from_tce(self):
        ...
        if tce_xml_url is not None:
            ...
            if len(params_list):
                ...
                tce_mask = np.any(mask_2d_array, axis=0)
                if np.sum(tce_mask) < self.time.size * 0.5:
                    self.meta["tce_mask"] = tce_mask
    ...
    def plot_candidates(self, figure_folder, threshold=0.5):
        ...
        if self.candidates is not None:
            ...
            for row in self.candidates.itertuples():
                ...
                i_plot = i_plot[~np.isnan(self.standardized_flux[i_plot])]
