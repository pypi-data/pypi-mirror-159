...
def preprocessing(X, method = None):
    temp = type(X)
    if method == 'mass':
...
def build_tani_graph(clu_file, data):
    """
...
def find_communities(X, pp_method = None, nbr_num = 10, n_threads = None, verbose = False, keep_temp_files = False, temp_name = '', partition_type = la.RBConfigurationVertexPartition, resolution = 0.8, seed = None): 
    """_summary_
    ...
    """
    X = preprocessing(X, method = pp_method)
    ...
    tic_prune = time.time()
    G = build_tani_graph(knng_result, X)
    if verbose:
...
def bc(X, nbr_num = 10, seed = None, metric = "cosine"):
    k = nbr_num
...
def clustering(X, nbr_num = 10, seed = None):
    if sys.platform.startswith("linux"):
        return(find_communities(X, nbr_num=nbr_num, seed = seed))
    else:
        X = preprocessing(X)
        return(bc(X, nbr_num= nbr_num, seed = seed))