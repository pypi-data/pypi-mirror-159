...
class CVATDumper:
    ...
    def call_cvat_dump_via_cli(
        ...
        logger.info(
            f"Dump annotation data: Task-ID={task_info.task_ID}, "
        ...
        if return_code != 0:
            raise IOError("CVAT download did not succeed, return-code=%s" % return_code)
    ...
    def __create_target_zip_path(dump_task_config: CVATTaskDumpConfig) -> str:
        ...
        if dump_task_config.target_zip_path == "":
            ...
            logger.debug(
                ...
                "will be used to store the downloaded task:" % target_zip_path
            )
        else:
    ...
    def __create_zip_extract_dir_path(
        ...
        if not os.path.isdir(zip_extract_dir):
            ...
            logger.debug(
                "dump_task_config.target_zip_path is not set, therefore the default '%s' "
                "will be used to store the downloaded task:" % zip_extract_dir
            )
    ...
    def __post_clean_up(target_zip_path: str, zip_extract_dir: str) -> None:
        if os.path.isfile(target_zip_path):
            logger.debug(f"(DUMP) Clean up zip file: '%s'" % target_zip_path)
            os.remove(target_zip_path)

        if os.path.isdir(zip_extract_dir):
            logger.debug(
                f"(DUMP) Clean up zip extraction directory: '%s'" % zip_extract_dir
            )
    ...
    def __extract_and_copy_annotations_to_data_dir(
        ...
        if not os.path.isdir(dataset_dir):
            logger.warning(
                ...
                "Skip the extraction and move of annotation data." % dataset_dir
            )
            return
        ...
        for annotation_path in annotation_file_paths:
            ...
            logger.debug(
                f"Copy annotation '{annotation_path}' to zip '{dataset_annotation_path}'"
            )
