    ...
    def write(
        ...
        for annotation_id, annotation in enumerate(annotations):
            ...
            for bounding_box in annotation.bounding_boxes:
                w = (
                    bounding_box.box.xmax - bounding_box.box.xmin
                ) / annotation.get_width()
                h = (
                    bounding_box.box.ymax - bounding_box.box.ymin
            ...
            if not os.path.exists(img_target):
                ...
                # TODO: ensure correct behavior of symlinks and only use is when not under windows
                if self.darknet_train_set_config.write_as_symlink:
                    ...
                    logger.debug(
                        "Write image %s as symlink %s", created_relpath, symlink_path
                    )
                else:
                    copyfile(annotation.image_path, img_target)

                    logger.debug(
                        "Copy image %s as symlink %s", annotation.image_path, img_target
                    )
