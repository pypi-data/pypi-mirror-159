...
class ClassMappingConfig(BaseConfigClass):
    ...
    number_model_classes: Optional[int] = related.ChildField(
        cls=int, required=False, default=None
    )
    number_classes: Optional[int] = related.ChildField(
        cls=int, required=False, default=None
    )
    ...
    def check_values(self) -> bool:
        ...
        if self.number_model_classes is None:
            logger.error(
                "'number_model_classes' of config class ClassMappingConfig is not set!"
            )
            return False
