...
class CSVAnnotationParser(AnnotationParser):
    ...
    def parse(self) -> List[BaseAnnotation]:
        ...
        # TODO: do we need to provide backward compatibility to old annotation files?

        if os.path.isfile(self.csv_file_path):
            with open(
                file=self.csv_file_path, mode="r", encoding="'utf-8"
            ) as annotations_file:
                ...
                for index in tqdm(range(len(csv_lines))):
                    ...
                    # TODO: what is the index when both pascal-voc and coco data are used?
                    ...
                    if pascal_voc_input_data_index is not None:
                        ...
                        image_dir = self.coco_input_data[
                            coco_input_data_index
                        ].input_root_dir

                        # TODO: is it correct to use input-path here?
                        #       for coco we use a .json file as annotation basis
                        annotation_dir = self.coco_input_data[
                            coco_input_data_index
                        ].input_path
                        ...
                        image_dir = self.cvat_input_data[
                            cvat_input_data_index
                        ].input_root_dir

                        # TODO: is it correct to use input-path here?
                        #       for coco we use a .json file as annotation basis
                        annotation_dir = self.cvat_input_data[
                            cvat_input_data_index
                        ].input_path
                    ...
                    if (
                        ...
                        csv_builder = CSVAnnotationBuilder(
                            csv_line=csv_line, mapper=self.mapper
                        )
                        ...
                        try:
                            ...
                            logger.warning(
                                "%s, annotation will be skipped"
                                % str(forbidden_class_error),
                            )
                            continue
                    else:
                        logger.warning(
    ...
    def __get_input_data_index(
        ...
        for i in range(0, dataset_len):
            ...
            replacement_string = (
                AnnotationParser.csv_directory_replacement_string.format(i)
            )
