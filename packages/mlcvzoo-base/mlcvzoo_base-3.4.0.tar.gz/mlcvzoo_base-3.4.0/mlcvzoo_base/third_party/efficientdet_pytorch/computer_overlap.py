...
def compute_overlap(boxes: np.ndarray, query_boxes: np.ndarray) -> Any:  # type: ignore[type-arg]
    ...
    iw = np.minimum(
        np.expand_dims(boxes[:, 2], axis=1), query_boxes[:, 2]
    ) - np.maximum(np.expand_dims(boxes[:, 0], 1), query_boxes[:, 0])
    ih = np.minimum(
        np.expand_dims(boxes[:, 3], axis=1), query_boxes[:, 3]
    ) - np.maximum(np.expand_dims(boxes[:, 1], 1), query_boxes[:, 1])
