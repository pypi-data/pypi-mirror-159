...
from mlcvzoo_base.data_preparation.cvat_annotation_handler.utils import (
...
class PascalVOCUploader:
    ...
    def __post_clean_up(source_zip_path: str, target_zip_path: str) -> None:
        if os.path.isfile(source_zip_path):
            logger.info(f"(UPLOAD) Clean up dump zip file: '%s'" % source_zip_path)
            os.remove(source_zip_path)

        if os.path.isfile(target_zip_path):
            logger.info(f"(UPLOAD) Clean up upload zip file: '%s'" % target_zip_path)
    ...
    def call_cvat_upload_via_cli(
        ...
        logger.info(
            f"Upload annotation data: Task-ID={task_info.task_ID}, "
            f"zip-path={target_zip_path}"
        )
        ...
        if return_code != 0:
            raise ValueError("Upload did not succeed, return-code=%s" % return_code)
    ...
    def write_upload_zip_file(upload_task_config: CVATTaskUploadConfig) -> str:
        ...
        if os.path.isfile(target_zip_path):
            logger.info(f"(UPLOAD) Clean up zip file: '%s'" % target_zip_path)
            os.remove(target_zip_path)
