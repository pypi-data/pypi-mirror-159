...
from mlcvzoo_base.data_preparation.custom_exceptions import (
    ClassMappingNotFoundError,
    ForbiddenClassError,
)
...
class CVATAnnotationParser(AnnotationParser):
        ...
        for dataset_count, input_data in enumerate(self.cvat_input_data):
            ...
            if root is not None:
                ...
                for image in images:
                    ...
                    try:
                        classifications = self.__parse_cvat_classifications(
                            cvat_tags=cvat_tags
                        )
    ...
    def __parse_cvat_bounding_boxes(
        ...
        for cvat_bbox in cvat_bboxes:
            ...
            # TODO: store difficult bounding boxes in a list, and add extra parameter for logging
            # TODO: handle occluded as well?
            # TODO: is that handling of use_difficult correct?
            #       skipping when use_difficult=True and appending when it is False?!
            if difficult:
                if use_difficult:
    ...
    def __parse_cvat_polygons(
        ...
        for cvat_polygon in cvat_polygons:
            ...
            if difficult:
                if use_difficult:
                    logger.warning("Found difficult segmentation, which will be used!")
                else:
                    logger.warning(
