...
def parse_replacement_config(
    ...
    # TODO: When it not necessary to handle the occurrence of "replacement_config" in the yaml,
    #       remove this function and directly use the ConfigBuilder instead
    ...
    if "replacement_config" in yaml_dict:
        replacement_config_dict = yaml_dict.pop("replacement_config")
    else:
        replacement_config_dict = yaml_dict

    replacement_config = to_model(ReplacementConfig, replacement_config_dict)
...
def str2bool(v: Any) -> bool:
    ...
    """
    # TODO: Could input type also be turned into "Union[bool, str]" ?
    #  therefore the raising of an error could be excluded by design
...
def create_configuration(
    ...
    if "base_config" in configuration_vars and isinstance(
        ...
        set_model_info(
            yaml_config_path=from_yaml, base_config=configuration_vars["base_config"]
        )
...
def write_config_to_yaml(config: BaseConfigClass, output_yaml_config_path: str) -> None:
    ...
    with open(file=output_yaml_config_path, mode="w") as output_yaml_file:
        logger.info(
            "Write current model-configuration to file: %s", output_yaml_config_path
        )
