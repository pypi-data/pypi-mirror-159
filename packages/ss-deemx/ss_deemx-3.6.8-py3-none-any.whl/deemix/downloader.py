...
from deemix.utils.pathtemplates import (
    generatePath,
    generateAlbumName,
    generateArtistName,
...
def downloadImage(url, path, overwrite=OverwriteOption.DONT_OVERWRITE):
    if path.is_file() and overwrite not in [
    ...
    try:
        ...
        if "cdns-images.dzcdn.net" in url:
            ...
            if pictureSize > 1200:
                return downloadImage(
                    urlBase
                    + pictureUrl.replace(f"{pictureSize}x{pictureSize}", "1200x1200"),
                    path,
                    overwrite,
                )
    except (
        ...
        if e.errno == errno.ENOSPC:
            raise DownloadFailed("noSpaceLeft") from e
        logger.exception(
            "Error while downloading an image, you should report this to the developers: %s",
            e,
        )
    return None


def getPreferredBitrate(
    dz, track, preferredBitrate, shouldFallback, feelingLucky, uuid=None, listener=None
):
    ...
    def testURL(track, url, formatName):
        ...
        request = requests.head(
            url, headers={"User-Agent": USER_AGENT_HEADER}, timeout=30
        )
        try:
            request.raise_for_status()
            track.filesizes[f"{formatName.lower()}"] = int(
    ...
    def getCorrectURL(track, formatName, formatNumber, feelingLucky):
        ...
        wrongLicense = (
            (formatName == "FLAC" or formatName.startswith("MP4_RA"))
            and not dz.current_user.get("can_stream_lossless")
            or formatName == "MP3_320"
            and not dz.current_user.get("can_stream_hq")
        )
        if (
            track.filesizes.get(formatName.lower())
            and track.filesizes[formatName.lower()] != "0"
        ):
            try:
        ...
        if not url and feelingLucky:
            url = generateCryptedStreamURL(
                track.id, track.MD5, track.mediaVersion, formatNumber
            )
            if testURL(track, url, formatName):
                return url
            url = None
        return url
    ...
    for formatNumber, formatName in formats.items():
        ...
        while True:
            ...
            if not url:
                getCorrectURL(currentTrack, formatName, formatNumber, feelingLucky)
            if url or not hasAlternative:
                break
            url_attempts += 1
        ...
        if not shouldFallback:
            ...
            if isGeolocked:
                raise WrongGeolocation(dz.current_user["country"])
            raise PreferredBitrateNotFound
        if not falledBack:
            falledBack = True
            logger.info(
                "%s Fallback to lower bitrate",
                f"[{track.mainArtist.name} - {track.title}]",
            )
...
class Downloader:
    ...
    def start(self):
        if not self.downloadObject.isCanceled:
            if isinstance(self.downloadObject, Single):
                ...
                if track:
                    self.afterDownloadSingle(track)
                ...
                with ThreadPoolExecutor(self.settings["queueConcurrency"]) as executor:
                    for pos, track in enumerate(
                        self.downloadObject.collection["tracks"], start=0
                    ):
                        tracks[pos] = executor.submit(
                            self.downloadWrapper,
                            {
                                "trackAPI": track,
                                "albumAPI": self.downloadObject.collection.get(
                                    "albumAPI"
                                ),
                                "playlistAPI": self.downloadObject.collection.get(
                                    "playlistAPI"
                                ),
                            },
                        )
    ...
    def log(self, data, state):
        if self.listener:
            self.listener.send(
                "downloadInfo",
                {"uuid": self.downloadObject.uuid, "data": data, "state": state},
            )
    ...
    def download(self, extraData, track=None):
        ...
        if track.MD5 == "":
            raise DownloadFailed("notEncoded", track)
        ...
        try:
            selectedFormat = getPreferredBitrate(
                ...
                self.settings["feelingLucky"],
                self.downloadObject.uuid,
                self.listener,
            )
        ...
        (filename, filepath, artistPath, coverPath, extrasPath) = generatePath(
            track, self.downloadObject, self.settings
        )
        ...
        # Save extrasPath
        if extrasPath and not self.downloadObject.extrasPath:
            self.downloadObject.extrasPath = extrasPath

        # Generate covers URLs
        embeddedImageFormat = f'jpg-{self.settings["jpegImageQuality"]}'
        if self.settings["embeddedArtworkPNG"]:
            embeddedImageFormat = "png"

        track.album.embeddedCoverURL = track.album.pic.getURL(
            self.settings["embeddedArtworkSize"], embeddedImageFormat
        )
        ext = track.album.embeddedCoverURL[-4:]
        if ext[0] != ".":
            ext = ".jpg"  # Check for Spotify images
        track.album.embeddedCoverPath = TEMPDIR / (
            (
                f"pl{track.playlist.id}"
                if track.album.isPlaylist
                else f"alb{track.album.id}"
            )
            + f"_{self.settings['embeddedArtworkSize']}{ext}"
        )

        # Download and cache coverart
        self.log(itemData, "getAlbumArt")
        track.album.embeddedCoverPath = downloadImage(
            track.album.embeddedCoverURL, track.album.embeddedCoverPath
        )
        ...
        if coverPath:
            ...
            for pic_format in self.settings["localArtworkFormat"].split(","):
                if pic_format in ["png", "jpg"]:
                    ...
                    if extendedFormat == "jpg":
                        extendedFormat += f"-{self.settings['jpegImageQuality']}"
                    url = track.album.pic.getURL(
                        self.settings["localArtworkSize"], extendedFormat
                    )
                    # Skip non deezer pictures at the wrong format
                    if (
                        isinstance(track.album.pic, StaticPicture)
                        and pic_format != "jpg"
                    ):
                        continue
            ...
            returnData["albumFilename"] = generateAlbumName(
                self.settings["coverImageTemplate"],
                track.album,
                self.settings,
                track.playlist,
            )
        ...
        if artistPath:
            ...
            for pic_format in self.settings["localArtworkFormat"].split(","):
                ...
                if pic_format == "jpg":
                    ...
                    url = track.album.mainArtist.pic.getURL(
                        self.settings["localArtworkSize"], extendedFormat
                    )
                    if track.album.mainArtist.pic.md5 == "":
                        continue
            ...
            returnData["artistFilename"] = generateArtistName(
                self.settings["artistImageTemplate"],
        ...
        if track.playlist:
            if len(self.playlistURLs) == 0:
                for pic_format in self.settings["localArtworkFormat"].split(","):
                    if pic_format in ["png", "jpg"]:
                        ...
                        if extendedFormat == "jpg":
                            extendedFormat += f"-{self.settings['jpegImageQuality']}"
                        url = track.playlist.pic.getURL(
                            self.settings["localArtworkSize"], extendedFormat
                        )
                        if (
                            isinstance(track.playlist.pic, StaticPicture)
                            and pic_format != "jpg"
                        ):
                            continue
            ...
            if not self.playlistCoverName:
                ...
                track.playlist.dateString = track.playlist.date.format(
                    self.settings["dateFormat"]
                )
                self.playlistCoverName = generateAlbumName(
                    self.settings["coverImageTemplate"],
                    track.playlist,
                    self.settings,
                    track.playlist,
        ...
        if self.settings["syncedLyrics"] and track.lyrics.sync:
            if not (filepath / f"{filename}.lrc").is_file() or self.settings[
                "overwriteFile"
            ] in [OverwriteOption.OVERWRITE, OverwriteOption.ONLY_TAGS]:
        ...
        if (
            not trackAlreadyDownloaded
            and self.settings["overwriteFile"] == OverwriteOption.DONT_CHECK_EXT
        ):
        ...
        if (
            ...
            and self.settings["overwriteFile"] == OverwriteOption.KEEP_BOTH
        ):
            baseFilename = str(filepath / filename)
            c = 1
        ...
        if (
            not trackAlreadyDownloaded
            or self.settings["overwriteFile"] == OverwriteOption.OVERWRITE
        ):
            track.downloadURL = track.urls[formatsName[track.bitrate]]
            if not track.downloadURL:
                raise DownloadFailed("notAvailable", track)
            try:
        ...
        if (
            not trackAlreadyDownloaded
            or self.settings["overwriteFile"]
            in [OverwriteOption.ONLY_TAGS, OverwriteOption.OVERWRITE]
        ) and not track.local:
            ...
            if extension == ".mp3":
                ...
                try:
                    ...
                    writepath.unlink()
                    logger.warning(
                        "%s Track not available in FLAC, falling back if necessary",
                        f"{itemData['artist']} - {itemData['title']}",
                    )
        ...
        try:
            ...
            if error.track:
                ...
                if not track.searched and self.settings["fallbackSearch"]:
                    ...
                    searchedId = self.dz.api.get_track_id_from_metadata(
    ...
    def afterDownloadSingle(self, track):
        if not self.downloadObject.extrasPath:
            self.downloadObject.extrasPath = Path(self.settings["downloadLocation"])
        ...
        try:
            if self.settings["saveArtwork"] and "albumPath" in track:
                for image in track["albumURLs"]:
                    downloadImage(
                        image["url"],
                        track["albumPath"] / f"{track['albumFilename']}.{image['ext']}",
                        self.settings["overwriteFile"],
                    )
        except Exception as e:
            ...
            if self.settings["saveArtworkArtist"] and "artistPath" in track:
                for image in track["artistURLs"]:
                    downloadImage(
                        image["url"],
                        track["artistPath"]
                        / f"{track['artistFilename']}.{image['ext']}",
                        self.settings["overwriteFile"],
                    )
        except Exception as e:
        ...
        try:
            if self.settings["logSearched"] and "searched" in track:
                ...
                with open(
                    self.downloadObject.extrasPath / "searched.txt",
                    "w+",
                    encoding="utf-8",
                ) as f:
                    searchedFile = f.read()
        ...
        try:
            if self.settings["executeCommand"] != "":
                execute(
                    ...
                    .replace("%folder%", quote(str(self.downloadObject.extrasPath)))
                    .replace("%filename%", quote(track["filename"]))
                )
        except Exception as e:
    ...
    def afterDownloadCollection(self, tracks):
        ...
        for i, track in enumerate(tracks):
            track = track.result()
            if not track:
                return  # Check if item is cancelled

            # Log errors to file
            if track.get("error"):
                if not track["error"].get("data"):
                    track["error"]["data"] = {
                        "id": "0",
                        "title": "Unknown",
                        "artist": "Unknown",
                ...
                if self.settings["saveArtwork"] and "albumPath" in track:
                    for image in track["albumURLs"]:
                        downloadImage(
                            image["url"],
                            track["albumPath"]
                            / f"{track['albumFilename']}.{image['ext']}",
                            self.settings["overwriteFile"],
                        )
            except Exception as e:
        ...
        try:
            if self.settings["logErrors"] and errors != "":
                with open(
                    self.downloadObject.extrasPath / "errors.txt", "w", encoding="utf-8"
                ) as f:
                    f.write(errors)
        except Exception as e:
        ...
        try:
            if (
                self.settings["saveArtwork"]
                and self.playlistCoverName
                and not self.settings["tags"]["savePlaylistAsCompilation"]
            ):
                for image in self.playlistURLs:
                    downloadImage(
                        image["url"],
                        self.downloadObject.extrasPath
                        / f"{self.playlistCoverName}.{image['ext']}",
                        self.settings["overwriteFile"],
                    )
        except Exception as e:
        ...
        try:
            if self.settings["createM3U8File"]:
                filename = (
                    generateDownloadObjectName(
                        self.settings["playlistFilenameTemplate"],
                        self.downloadObject,
                        self.settings,
                    )
                    or "playlist"
                ...
                with open(
                    self.downloadObject.extrasPath / f"{filename}.m3u8",
                    "w",
                    encoding="utf-8",
                ) as f:
        ...
        try:
            if self.settings["executeCommand"] != "":
                execute(
                    self.settings["executeCommand"].replace(
                        "%folder%", quote(str(self.downloadObject.extrasPath))
                    )
                )
        except Exception as e:
            self.afterDownloadErrorReport("ExecuteCommand", e)
