...
class Spotify(Plugin):
    def __init__(self, configFolder=None):
        ...
        self.settings = {"fallbackSearch": False}
        self.enabled = False
        self.sp = None
    ...
    def parseLink(cls, link):
        ...
        if "&" in link:
            link = link[: link.find("&")]
        if link.endswith("/"):
            link = link[:-1]  #  Remove last slash if present

        link_type = None
        link_id = None

        if not "spotify" in link:
            return (link, link_type, link_id)  # return if not a spotify link
    ...
    def generateTrackItem(self, dz, link_id, bitrate):
        ...
        if "isrc" in cachedTrack:
            try:
                return generateTrackItem(dz, f"isrc:{cachedTrack['isrc']}", bitrate)
    ...
    def generateAlbumItem(self, dz, link_id, bitrate):
        ...
        try:
            return generateAlbumItem(dz, f"upc:{cachedAlbum['upc']}", bitrate)
        except GenerationError as e:
            raise AlbumNotOnDeezer(f"https://open.spotify.com/album/{link_id}") from e

    def generatePlaylistItem(self, dz, link_id, bitrate):
        if not self.enabled:
            raise Exception("Spotify plugin not enabled")
        ...
        playlistAPI["various_artist"] = dz.api.get_artist(
            5080
        )  # Useful for save as compilation
        ...
        return Convertable(
            {
                ...
                "size": len(tracklist),
                "collection": {"tracks": [], "playlistAPI": playlistAPI},
                "plugin": "spotify",
    ...
    def getTrack(self, track_id, spotifyTrack=None):
        if not self.enabled:
            raise Exception("Spotify plugin not enabled")
        cachedTrack = {"isrc": None, "data": None}
    ...
    def getAlbum(self, album_id, spotifyAlbum=None):
        if not self.enabled:
            raise Exception("Spotify plugin not enabled")
        cachedAlbum = {"upc": None, "data": None}
    ...
    def convertTrack(self, dz, downloadObject, track, pos, conversion, cache, listener):
        ...
        if "isrc" in cachedTrack:
            try:
                ...
                if "id" not in trackAPI or "title" not in trackAPI:
                    trackAPI = None
        ...
        if not trackAPI:
            trackAPI = {
                ...
                "media_version": 0,
                "filesizes": {},
                "album": {"title": track["album"]["name"], "md5_image": ""},
                "artist": {"id": 0, "name": track["artists"][0]["name"]},
            }
        trackAPI["position"] = pos + 1

        conversion["next"] += (1 / downloadObject.size) * 100
        if (
            round(conversion["next"]) != conversion["now"]
            and round(conversion["next"]) % 2 == 0
        ):
            conversion["now"] = round(conversion["next"])
            if listener:
                listener.send(
                    "updateQueue",
                    {"uuid": downloadObject.uuid, "conversion": conversion["now"]},
                )
    ...
    def convert(self, dz, downloadObject, settings, listener=None):
        ...
        if listener:
            listener.send("startConversion", downloadObject.uuid)
        with ThreadPoolExecutor(settings["queueConcurrency"]) as executor:
            for pos, track in enumerate(downloadObject.conversion_data, start=0):
                collection[pos] = executor.submit(
                    self.convertTrack,
                    dz,
                    downloadObject,
                    track,
    ...
    def _convertPlaylistStructure(cls, spotifyPlaylist):
        ...
        deezerPlaylist = {
            ...
            "duration": 0,
            "fans": spotifyPlaylist["followers"]["total"]
            if "followers" in spotifyPlaylist
            else 0,
            ...
            "picture": cover,
            "picture_small": cover
            or "https://e-cdns-images.dzcdn.net/images/cover/d41d8cd98f00b204e9800998ecf8427e/56x56-000000-80-0-0.jpg",
            "picture_medium": cover
            or "https://e-cdns-images.dzcdn.net/images/cover/d41d8cd98f00b204e9800998ecf8427e/250x250-000000-80-0-0.jpg",
            "picture_big": cover
            or "https://e-cdns-images.dzcdn.net/images/cover/d41d8cd98f00b204e9800998ecf8427e/500x500-000000-80-0-0.jpg",
            "picture_xl": cover
            or "https://e-cdns-images.dzcdn.net/images/cover/d41d8cd98f00b204e9800998ecf8427e/1000x1000-000000-80-0-0.jpg",
            "picture_thumbnail": cover
            or "https://e-cdns-images.dzcdn.net/images/cover/d41d8cd98f00b204e9800998ecf8427e/75x75-000000-80-0-0.jpg",
    ...
    def loadSettings(self):
        ...
        with open(
            self.configFolder / "config.json", "r", encoding="utf-8"
        ) as settingsFile:
            try:
                settings = json.load(settingsFile)
            except json.decoder.JSONDecodeError:
                with open(
                    self.configFolder / "config.json", "w", encoding="utf-8"
                ) as f:
                    json.dump({**self.credentials, **self.settings}, f, indent=2)
    ...
    def saveCache(self, newCache):
        with open(
            self.configFolder / "cache.json", "w", encoding="utf-8"
        ) as spotifyCache:
