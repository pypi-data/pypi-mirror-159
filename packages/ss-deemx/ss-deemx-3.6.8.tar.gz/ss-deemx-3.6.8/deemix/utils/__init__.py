...
def formatListener(key, data=None):
    ...
    if key == "downloadInfo":
        ...
        elif data["state"] == "downloaded":
            message = "Track downloaded."
        elif data["state"] == "alreadyDownloaded":
            message = "Track already downloaded."
        elif data["state"] == "tagging":
            message = "Tagging track."
        elif data["state"] == "tagged":
            message = "Track tagged."
        return f"[{data['uuid']}] {data['data']['artist']} - {data['data']['title']} :: {message}"
    if key == "downloadWarn":
        errorMessage = ErrorMessages[data["state"]]
        solutionMessage = ""
        if data["solution"] == "fallback":
            solutionMessage = "Using fallback id."
        if data["solution"] == "search":
            solutionMessage = "Searching for alternative."
