...
from res_reg_lmnt_awikner import *
import subprocess
import numpy as np
import time
...
all_data = []
all_run_opts = []
for noisetype, traintype, noise_values_array, reg_values, reg_train_times_in in zip(noisetypes, traintypes, noise_values_array_all, reg_values_all, reg_train_times_all): 
...
for data, run_opts in zip(all_data, all_run_opts):
    data_slice = data.data_slice(reg = run_opts.reg_values[0], reg_train = run_opts.reg_train_vals[0],\
