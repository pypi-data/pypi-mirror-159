...
from ..tools.misc import get_truncated_uid, deduplicate
...
class SceneEngineClient(object):
    ...
    def calculate_performance_metrics_per_dimension(
            self,
            comparison_task_id: str,
            dimension: str,
            set_id: str,
        ...
        set_id:
            id of the set that images belong to
        iou_threshold:
            iou threshold to use in confusion matrix calculation
        search:
        ...
        -------
        Dict
             A dictionary that has 3 keys; job_id, performance_metrics_task_id, and performances.
             The ID of the task that calculated the performances, and the performances if the job is finished.
             Example:
             {
                "job_id": "some_job_id",
                "performance_metrics_task_id": "some_performance_metrics_task_id",
                "performances": {
                    "image_id_1": {
                        "cat": {'precision': 0.75, 'recall': 0.47368421052631576, 'f1': 0.5806451612903226},
                        "dog": {'precision': 0, 'recall': 0, 'f1': N/A}
        ...
        job_id = resp.json().get("job_id")
        performance_metrics_task_id = resp.json().get("performance_metrics_task_id")
        performances = resp.json().get("performances")

        if performances:
            return {
                "performance_metrics_task_id": performance_metrics_task_id,
        ...
        if performance_metrics_task_id:
            resp = self.requests.get(
                "annotations_comparisons/performance_metrics_per_dimension/{}".format(performance_metrics_task_id),
                trailing_slash=False)
