...
from pregex.exceptions import NonPositiveArgumentException, NegativeArgumentException, \
    NeitherStringNorPregexException, MinGreaterThanMaxException, InvalidCapturingGroupNameException, \
    NonStringArgumentException, NonIntegerArgumentException
...
class Pregex():
    ...
    '''
    __flags: re.RegexFlag = re.MULTILINE | re.DOTALL
    ...
    def __init__(self, pattern: str, group_on_concat=False, group_on_quantify=False) -> 'Pregex':
        ...
        self.__compiled: re.Pattern = None
    ...
    def compile(self) -> None:
        ...
        '''
        self.__compiled = re.compile(self.__pattern, flags=self.__flags)
    ...
    def has_match(self, text: str) -> bool:
        ...
        '''
        return bool(re.search(self.__pattern, text) if self.__compiled is None \
    ...
    def is_exact_match(self, text: str) -> bool:
        ...
        :param str text: The piece of text that is to be matched.
        '''
        return bool(re.fullmatch(self.__pattern, text, flags=self.__flags) \
            if self.__compiled is None else self.__compiled.fullmatch(text))

    def iterate_matches(self, text: str) -> Iterator[str]:
        '''
    ...
    def iterate_matches_and_pos(self, text: str) -> Iterator[tuple[str, int, int]]:
        '''
        Generates any possible matches with 'text' one by one, \
    ...
    def iterate_groups(self, text: str, include_empty: bool = True) -> Iterator[tuple[str]]:
        '''
    ...
    def iterate_groups_and_pos(self, text: str, include_empty: bool = True) -> Iterator[list[tuple[str, int, int]]]:
        '''
        Generates any tuples of captured groups that may have occured \
    ...
    def replace(self, text: str, repl: str, count: int = 0) -> str:
        ...
        '''
        if count < 0:
            raise NegativeArgumentException(count)
        return re.sub(str(self), repl, text, count, flags=self.__flags)
    ...
    def _to_pregex(pre: str or 'Pregex') -> 'Pregex':
        if isinstance(pre, str):
            ...
            raise NeitherStringNorPregexException()
    ...
    def __repr__(self) -> str:
        ...
        '''
        return re.sub(r"\\\\", r"\\", repr(self.__pattern)[1:-1])
    ...
    def __iterate_match_objects(self, text: str) -> Iterator[re.Match]:
        '''
        Iterates over all matches as objects of type 're.Match'
        '''
        return re.finditer(self.__pattern, text, flags=self.__flags) \
    ...
    def _exactly(self, n: int) -> 'Pregex':
        if not isinstance(n, int) or isinstance(n, bool):
            raise NonIntegerArgumentException(n)
        if n < 1:
            raise NonPositiveArgumentException(n)
        elif n == 1:
            return self
        else:
    ...
    def _at_least(self, n: int, is_greedy: bool = True)-> 'Pregex':
        if not isinstance(n, int) or isinstance(n, bool):
            raise NonIntegerArgumentException(n)
        if n < 0:
            raise NegativeArgumentException(n)
        elif n == 0:
    ...
    def _at_least_at_most(self, min: int, max: int, is_greedy: bool = True) -> 'Pregex':
        if not isinstance(min, int) or isinstance(min, bool):
            raise NonIntegerArgumentException(min)
        if not isinstance(max, int) or isinstance(max, bool):
            raise NonIntegerArgumentException(max)
        if min < 0:
            raise NegativeArgumentException(min)
        elif max < 1:
            raise NonPositiveArgumentException(max)
        elif max < min:
            raise MinGreaterThanMaxException(min, max)
        elif min == max:
    ...
    def _capturing_group(self, name: str) -> 'Pregex':
        ...
        if name != '' and re.fullmatch("[A-Za-z_][A-Za-z_0-9]*", name) is None:
        ...
        if self.__is_group():
            ...
            if name != '':
                if pattern.startswith('(?P'):
                    pattern = re.sub('\(\?P<[^>]*>', f'(?P<{name}>', pattern)
                else:
