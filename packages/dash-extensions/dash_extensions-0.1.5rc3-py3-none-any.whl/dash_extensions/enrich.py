...
class PrefixIdTransform(DashTransform):
    ...
    def apply(self, callbacks, clientside_callbacks):
        prefixed_ids = []
        callbacks = self._apply(callbacks, prefixed_ids)
        clientside_callbacks = self._apply(clientside_callbacks, prefixed_ids)
        return callbacks, clientside_callbacks

    def _apply(self, callbacks, prefixed_ids):
        for callback in callbacks:
            for i in callback.inputs:
                self._prefix_process(i, prefixed_ids)
            for o in callback.outputs:
                self._prefix_process(o, prefixed_ids)
        return callbacks

    def _prefix_process(self, obj, prefixed_ids):
        cid = obj.component_id
        if cid in prefixed_ids:
            return
        prefixed_cid = apply_prefix(self.prefix, cid, self.escape)
        prefixed_ids.append(prefixed_cid)
        obj.component_id = prefixed_cid
