...
def config_file(config_path: str, datetime_format: Dict = {
        'date_time_format': ''}, config_default: Dict[str, Any] = {}) -> Dict:
    ...
    if config.get('timezone_offset') is not None:
        ...
        config['date_time'] = datetime.datetime.now(datetime.timezone.utc).replace(tzinfo=None)
...
def set_schema(stream: str, config: Dict, stream_data: Dict) -> None:
...
async def save_json(message_type: str, stream: str, stream_data: Dict, config: Dict[str, Any], record: Dict = None, save: Callable = write) -> None:
    ...
    if (sys.getsizeof(stream_data[stream]['file_data']) >= config.get('memory_buffer', 0) or message_type is None) \
