...
class Loader():
    ...
    async def writelines(self, lines: TextIO = sys.stdin,
                         set_schemas: Callable = set_schema,
                         writeline: Callable = save_json) -> Tuple[Optional[Any], Dict[Any, Any]]:
        ...
        for raw_line in lines:
            ...
            if message_type == 'SCHEMA':
                ...
                self.set_schemas(stream, self.config, stream_data)
                ...
                await self.writeline(message_type, stream, stream_data, self.config, record_to_load)
        ...
        for stream in stream_data:
            await self.writeline(None, stream, stream_data, self.config)
