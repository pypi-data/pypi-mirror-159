...
def classify_audio_clip(clip):
    ...
    classifier.load_state_dict(torch.load(get_model_path('classifier.pth'), map_location=torch.device('cuda')))
    clip = clip.cuda(self.device).unsqueeze(0)
...
class TextToSpeech:
    ...
    def __init__(self, autoregressive_batch_size=None, models_dir=MODELS_DIR, enable_redaction=True, device=None):
        ...
        if os.path.exists(f'{models_dir}/autoregressive.ptt'):
            ...
            self.diffusion = self.diffusion.cuda(self.device)
        else:
            self.autoregressive = UnifiedVoice(max_mel_tokens=604, max_text_tokens=402, max_conditioning_inputs=2, layers=30,
                                          model_dim=1024,
                                          heads=16, number_text_tokens=255, start_text_token=255, checkpointing=False,
                                          train_solo_embeddings=False).cuda(self.device).eval()
            ...
            self.diffusion = DiffusionTts(model_channels=1024, num_layers=10, in_channels=100, out_channels=200,
                                          ...
                                          layer_drop=0, unconditioned_percentage=0).cuda(self.device).eval()
    ...
    def load_cvvp(self):
        ...
        self.cvvp = CVVP(model_dim=512, transformer_heads=8, dropout=0, mel_codes=8192, conditioning_enc_depth=8, cond_mask_percentage=0,
                         speech_enc_depth=8, speech_mask_percentage=0, latent_multiplier=1).cuda(self.device).eval()
    ...
    def get_random_conditioning_latents(self):
        ...
        if self.rlg_auto is None:
            ...
            self.rlg_auto.load_state_dict(torch.load(get_model_path('rlg_auto.pth', self.models_dir), map_location=torch.device('cuda')))
            self.rlg_diffusion = RandomLatentConverter(2048).eval()
            self.rlg_diffusion.load_state_dict(torch.load(get_model_path('rlg_diffuser.pth', self.models_dir), map_location=torch.device('cuda')))
        with torch.no_grad():
    ...
    def tts(self, text, voice_samples=None, conditioning_latents=None, k=1, verbose=True, use_deterministic_seed=None,
        ...
        with torch.no_grad():
            ...
            self.clvp = self.clvp.cuda(self.device)
            if self.cvvp is not None:
                self.cvvp = self.cvvp.cuda(self.device)
            del samples
            ...
            for b in range(best_results.shape[0]):
                ...
                wav_candidates.append(wav.cuda(self.device))
            self.diffusion = self.diffusion.cuda(self.device)
            self.vocoder = self.vocoder.cuda(self.device)
