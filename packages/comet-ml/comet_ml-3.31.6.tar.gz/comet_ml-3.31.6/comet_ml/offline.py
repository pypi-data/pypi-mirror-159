...
class OfflineSender(object):
            ...
            for i, line in enumerate(messages_files):
                try:
                    ...
                    if (
                        ...
                        self._process_standard_output_message(message)
    ...
    def _send_parameter_messages_batch(self, message_items):
        ...
        if self.use_http_messages:
            self._process_rest_api_send(
                ...
                items=message_items,
                compress=self.message_batch_metric_compress,
            )
        else:
    ...
    def _process_standard_output_message(self, message):
        # type: (Dict[str, Any]) -> None
        message = message["payload"]
        ...
        self._process_rest_api_send(
            self.rest_api_client.log_experiment_output,
            rest_fail_prompt=STANDARD_OUTPUT_SENDING_ERROR,
            general_fail_prompt="Failed to send standard output message",
            experiment_key=self.experiment_id,
            output=message["output"],
            context=message.get("context"),
            stderr=message["stderr"],
            timestamp=message.get("local_timestamp"),
        )
