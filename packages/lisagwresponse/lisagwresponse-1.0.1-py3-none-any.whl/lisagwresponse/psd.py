...
def white_generator(psd):
    ...
    Args:
        psd: strain power spectral density [/Hz]

    Returns:
        A function of (fs, size) the sampling frequency `fs` [Hz] and number of samples `size`.
    """
...
def ifft_generator(spectrum):
    ...
    Args:
        spectrum: one-sided power spectral density function [A^2/Hz], where A is the unit of
            the desired output time series

    Returns:
        A function of (fs, size) the sampling frequency `fs` [Hz] and number of samples `size`,
        which produces a stochastic time series whose spectrum matches `spectrum`.
    """
    def generator(fs, size):
        """Generator function."""
        fs = float(fs)
