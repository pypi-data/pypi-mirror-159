"""
    ...
    Tpose:      Returns the tranpose of given Matrix
    
    orthCheck:  Returns bool value True if provided matrix is orthogonal False if else.
...
def det(A: list, n:int) -> int:
    ...
    for index in mainDLow:
        detL *= index

    return detU * detL
     

def Tpose(A: list, n: int) -> list:
    """Return a nxn matrix B = A^T

    Keyword arguments:
    A -- A nxn 2-dimensional list
    n -- size of the square matrix
    """
    B = [ [0] * n for index in range(n)]
...
def orthCheck(A: list, n: int) -> bool:
    """Return a bool True --> A is ortho
              else A !ortho
    ...
    """
    ##
    #  We will check orthogonality with the use of this theoreom
    #  
    #  A^T * A = I
    #  
    # we will populate a nxn I
    # then tranpose A
    # multiply A by A^T 
    # then check eq. if it holds return TRUE else FALSE
    # #

    # Ident mat for test
    IdentMat = [[0] * n for index in range(n)]
    for index in range(n):
        IdentMat[index][index] = 1
    
    # Tranpose A
    aT = Tpose(A,n)

    # multiply A A^T
    testMat = MatMult(A,aT,n)
