...
def find_h5_files(user_str):
    ...
    print("Found {} files starting with {}".format(len(all_files), user_str))
    return all_files
...
def save_spec(file_path, data, monav, entry="1.1"):
    ...
    print("Saved scan {} to {}".format(scannr, file_path))
    return scannr
...
def execute_sum(
    ...
    cmdfilename = os.path.relpath(filename, outdir)
...
def run(
    ...
    if outdir is None:
        outdir = os.getcwd()
    resfile = os.path.join(outdir, "temp.res")
    do_sum = sum_single or sum_single
    if do_sum and not os.path.isfile(resfile):
        raise RuntimeError("Cannot find {}".format(resfile))

    all_h5_files = find_h5_files(user_str)
    if not all_h5_files:
        print("Did not find any files starting/matching with {}...".format(user_str))

    ret = list()
    for h5file in all_h5_files:
        print("\nProcessing file " + h5file)
        scans_h5 = get_all_scans_h5(h5file)
        if not scans_h5:
            print("No scans to process")
            continue

        # Convert HDF5 to ASCII
        specfile = os.path.splitext(h5file)[0] + ".dat"
        ...
        missing_scans = sorted(
            set(range(first_scan, last_scan + 1)) - set(scan_numbers)
        )
