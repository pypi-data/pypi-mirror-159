...
class ElmaApiWebClient:
    ...
    __SessionToken: str
    __AppToken: str
    authorization: 'ElmaApiWebClient.Authorization'
    workflow: 'ElmaApiWebClient.Workflow'
    workflow_instance_service: 'ElmaApiWebClient.WorkflowInstanceService'
    ...
    def __init__(self, host_name: str, port: int, debug_mode=False) -> None:
        ...
        self.authorization = ElmaApiWebClient.Authorization(self)
        self.workflow = ElmaApiWebClient.Workflow(self)
        self.workflow_instance_service = ElmaApiWebClient.WorkflowInstanceService(self)
    ...
    def init_with_auth(host_name: str, port: int, debug_mode=False,
        ...
        auth_result = ElmaApiWebClient.Authorization(instance).login_with_user_name(user_login=user_login,
    ...
    class Workflow(ApiModule):
        ...
        def start_process_form(self, start_process_from_rq: StartProcessFromRq = None,
                               process_header_id=None,
                               auth_token: str = None, session_token: str = None) -> StartProcessFromRs:
            if start_process_from_rq is None:
                start_process_from_rq = StartProcessFromRq(process_header_id)
            return self.__workflow_post_request(path=start_process_form_path, rq=start_process_from_rq,
                                                rsp_cls=StartProcessFromRs,
                                                auth_token=auth_token, session_token=session_token)
        ...
        def execute_user_task_async(self, execute_user_task_async_rq: ExecuteUserTaskRq,
                                    _id, context, selected_connector_uid,
                                    auth_token: str = None, session_token: str = None) -> ExecuteUserTaskRs:

            if execute_user_task_async_rq is None:
                execute_user_task_async_rq = ExecuteUserTaskRq(_id, context, selected_connector_uid)
