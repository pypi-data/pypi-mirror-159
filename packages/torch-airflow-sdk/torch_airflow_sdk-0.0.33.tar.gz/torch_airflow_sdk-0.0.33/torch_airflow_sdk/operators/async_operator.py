...
from torch_sdk.common import AsyncExecutor


class AsyncOperator(BaseOperator):
    """
    Description:
        AsyncOperator is used to execute a policy by passing rule_type and rule_id.
        It will return immediately after starting the rule execution.
    ...
    def __init__(self, *, rule_type, rule_id, incremental=False, **kwargs):
        ...
        After getting the execution_id you need to create object of AsyncExecutor by passing rule_type and
        ...
        def asyncoperator_result(**context):
            ...
            async_executor = AsyncExecutor('DATA_QUALITY', torch_client)
            execution_result = async_executor.get_status(execution_id)

        If you want to wait till the execution finishes before getting the result call get_result function
        execution_result = async_executor.get_result(execution_id)
    ...
    def execute(self, context):
        torch_client = TorchClient(**torch_credentials)
        execution_return = torch_client.execute_rule(self.rule_type, self.rule_id, sync=False, incremental=self.incremental)
