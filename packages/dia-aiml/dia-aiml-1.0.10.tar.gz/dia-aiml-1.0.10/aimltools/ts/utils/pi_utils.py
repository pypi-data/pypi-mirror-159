...
class OSIPIReader:
    ...
    def read_tags(self, tagname_list, starttime, endtime, boundary_type, serialize="list"):
        pi_tag_values = []
        pi_tag_df = pd.DataFrame(columns=self.__df_column_names)
        for tagname in tagname_list:
            tags_coll = self.__read_tag(tagname, starttime, endtime, boundary_type, serialize=serialize)
            if serialize == "list":
                pi_tag_values.append(tags_coll)
            else:
                pi_tag_df = pd.concat([pi_tag_df, tags_coll])

        #in case serialize="df", finally sort dataframe by time...
        if serialize == "list":
