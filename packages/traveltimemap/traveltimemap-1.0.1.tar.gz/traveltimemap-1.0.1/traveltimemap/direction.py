...
def map_direction(df):
    ...
    folium.RegularPolygonMarker(location=(data.Lat.mean(), data.Long.mean()), popup = popup, fill_color='red', number_of_sides=3,
                                radius=15, rotation=(math.degrees(
            (data.Lat.mean() - lat[0] / data.Long.mean() - long[0]))) * 270).add_to(map)

    folium.Marker(location=[33.89212744151436, -117.50632763273958], popup='<img src="./traveltimemap/i15.png" width="50" height="50">', icon=folium.Icon(color='red')).add_to(map)
    folium.Marker(location=[33.92182711328787, -117.55013228351774], popup='<img src="./traveltimemap/sr91.jpeg" width="50" height="50">', icon=folium.Icon(color='red')).add_to(map)
