...
class SvmSchema(ResourceSchema, metaclass=ResourceSchemaMeta):
    ...
    aggregates = fields.List(fields.Nested("netapp_ontap.models.s3_bucket_svm_aggregates.S3BucketSvmAggregatesSchema", unknown=EXCLUDE), data_key="aggregates")
...
Example: svm1 """
    ...
    retention_period = Size(
        data_key="retention_period",
    )
    r""" Volume delete retention period specifies the default duration, in hours that deleted volumes are retained in the recovery queue for possible future recovery. """
...
Example: 02c9e252-41be-11e9-81d5-00a0986138f7 """
    ...
    """links,aggregates.links,aggregates.name,aggregates.uuid,aggregates_delegated,anti_ransomware_default_volume_state,certificate.links,certificate.name,certificate.uuid,cifs.links,cifs.ad_domain,cifs.allowed,cifs.enabled,cifs.name,comment,dns,fc_interfaces,fcp,ip_interfaces,ipspace.links,ipspace.name,ipspace.uuid,is_space_enforcement_logical,is_space_reporting_logical,iscsi,language,ldap.ad_domain,ldap.base_dn,ldap.bind_dn,ldap.enabled,ldap.servers,max_volumes,name,ndmp,nfs,nis,nsswitch,nvme,qos_policy.links,qos_policy.max_throughput_iops,qos_policy.max_throughput_mbps,qos_policy.min_throughput_iops,qos_policy.min_throughput_mbps,qos_policy.name,qos_policy.uuid,retention_period,s3.links,s3.certificate,s3.enabled,s3.is_http_enabled,s3.is_https_enabled,s3.name,s3.port,s3.secure_port,snapmirror,snapshot_policy.links,snapshot_policy.name,snapshot_policy.uuid,state,subtype,uuid,"""
    ...
    """aggregates.links,aggregates.name,aggregates.uuid,anti_ransomware_default_volume_state,certificate.name,certificate.uuid,cifs.allowed,comment,fcp,ip_interfaces,is_space_enforcement_logical,is_space_reporting_logical,iscsi,language,max_volumes,name,ndmp,nfs,nsswitch,nvme,qos_policy.max_throughput_iops,qos_policy.max_throughput_mbps,qos_policy.min_throughput_iops,qos_policy.min_throughput_mbps,qos_policy.name,qos_policy.uuid,retention_period,snapmirror,snapshot_policy.name,snapshot_policy.uuid,state,"""
    ...
    """aggregates.links,aggregates.name,aggregates.uuid,anti_ransomware_default_volume_state,cifs.ad_domain,cifs.allowed,cifs.enabled,cifs.name,comment,dns,fc_interfaces,fcp,ip_interfaces,ipspace.name,ipspace.uuid,is_space_enforcement_logical,is_space_reporting_logical,iscsi,language,ldap.ad_domain,ldap.base_dn,ldap.bind_dn,ldap.enabled,ldap.servers,max_volumes,name,ndmp,nfs,nis,nsswitch,nvme,routes,s3.certificate,s3.enabled,s3.is_http_enabled,s3.is_https_enabled,s3.name,s3.port,s3.secure_port,snapmirror,snapshot_policy.name,snapshot_policy.uuid,subtype,"""
...
"""
    ...
    if RECLINE_INSTALLED:
        ...
        def svm_show(
            fields: List[Choices.define(["aggregates_delegated", "anti_ransomware_default_volume_state", "comment", "is_space_enforcement_logical", "is_space_reporting_logical", "language", "max_volumes", "name", "retention_period", "state", "subtype", "uuid", "*"])]=None,
        ) -> ResourceTable:
            ...
            Args:
                ...
                retention_period: Volume delete retention period specifies the default duration, in hours that deleted volumes are retained in the recovery queue for possible future recovery.
                state: SVM State
    ...
    if RECLINE_INSTALLED:
        ...
        async def svm_modify(
            ...
            Args:
                ...
                retention_period: Volume delete retention period specifies the default duration, in hours that deleted volumes are retained in the recovery queue for possible future recovery.
                query_retention_period: Volume delete retention period specifies the default duration, in hours that deleted volumes are retained in the recovery queue for possible future recovery.
                state: SVM State
