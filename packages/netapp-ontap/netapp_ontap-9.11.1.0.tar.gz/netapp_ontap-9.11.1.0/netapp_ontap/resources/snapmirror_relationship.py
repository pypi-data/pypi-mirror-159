...
class SnapmirrorRelationshipSchema(ResourceSchema, metaclass=ResourceSchemaMeta):
    ...
    r""" The fabriclink field of the snapmirror_relationship. """
...
### Examples
The following examples show how to create FlexVol, FlexGroup, object store, SVM and Consistency Group SnapMirror relationships. Note that the source SVM name should be the local name of the peer SVM.</br>
...
"""
    ...
    if RECLINE_INSTALLED:
        ...
        async def snapmirror_relationship_create(
            ...
            if exported_snapshot is not None:
                kwargs["exported_snapshot"] = exported_snapshot
            if fabriclink is not None:
                kwargs["fabriclink"] = fabriclink
            if group_type is not None:
                kwargs["group_type"] = group_type
            if healthy is not None:
                kwargs["healthy"] = healthy
...
"""
    ...
    def restore_action(
        ...
        **kwargs
        ...
        r"""Cross cluster POST to create SnapMirror restore relationship.
### Examples
<br/>
   Performing a source side SnapMirror restore of a whole volume from an object store with archive_retrieval_priority high.
   <br/>
   ```
   POST "/api/snapmirror/relationships/5aadf886-2039-11ea-b47a-005056a778b7/restore" '{"source": {"path": "S3store1:/objstore/object1"}, "destination": { "path": "test_vserv_dst:dst_vol_dp"}, "archive_retrieval_priority": "high"}'
   ```
   <br/>
"""
