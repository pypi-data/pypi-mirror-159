...
from uds_vlab_ipam_api.lib import const, Database
from uds_vlab_ipam_api.ddns_updater import get_ip
from uds_vlab_ipam_api.lib.exceptions import DatabaseError, CliError

logger = get_logger(__name__, loglevel=const.UDS_VLAB_IPAM_LOG_LEVEL)
...
class PortMapView(BaseView):
    ...
    @requires(version=2, username=const.UDS_VLAB_IPAM_OWNER, verify=const.VLAB_VERIFY_TOKEN)
...
def remove_port_map(nat_id, filter_id, target_port, target_addr, conn_port, db):
    ...
    :type db: uds_vlab_ipam_api.lib.database.Database
    """
