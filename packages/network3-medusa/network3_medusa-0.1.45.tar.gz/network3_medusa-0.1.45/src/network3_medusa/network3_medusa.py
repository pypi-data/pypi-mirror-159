...
class Network3Medusa():
    ...
    def network3_medusa(self):
        self.py2output()
        parsed_json=self.output.stdout
    ...
    def add_file_to_hedera(self, parsed_json):
        ...
        resp = tran.setKeys(OPERATOR_KEY.getPublicKey()).setContents(json.dumps(parsed_json)).setMaxTransactionFee(Hbar(2)).execute(client)
