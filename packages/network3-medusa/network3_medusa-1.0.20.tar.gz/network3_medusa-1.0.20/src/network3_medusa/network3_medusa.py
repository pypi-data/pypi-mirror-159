        ...
        if 'eth_admin_fec_state' in raw_json:
            ...
            for interface in json_dict['TABLE_interface']['ROW_interface']:
                if first:
                    ...
                    raw_json=interface
                    print("first")
                    self.add_file_to_hedera(f"{ str(raw_json) },")
                elif json_dict['TABLE_interface']['ROW_interface'][-1]:
                    raw_json=interface
                    print("last")
                    self.append_file_to_hedera(str(raw_json))
                else:
                    raw_json=interface
                    print("middle")
                    self.append_file_to_hedera(f"{ str(raw_json) },")
