...
import click
import subprocess
from hedera import Client, AccountId, PrivateKey, PublicKey, Hbar, FileCreateTransaction, FileContentsQuery
from jnius import autoclass

OPERATOR_ID = AccountId.fromString(os.getenv("OPERATOR_ID"))
OPERATE_PUBLIC_KEY = PublicKey.fromString(os.getenv("OPERATOR_PUBLIC_KEY"))
...
class Network3Medusa():
    ...
    def add_file_to_hedera(self, raw_json):
        ...
        resp = tran.setKeys(OPERATE_PUBLIC_KEY).setContents(raw_json).setMaxTransactionFee(Hbar(2)).execute(client)
