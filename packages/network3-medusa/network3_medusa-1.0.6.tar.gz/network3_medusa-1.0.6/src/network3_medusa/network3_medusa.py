...
class Network3Medusa():
    ...
    def network3_medusa(self):
        ...
        if 'eth_admin_fec_state' in raw_json:
            for interface in enumerate(json.loads(raw_json).get()):
                if interface == 0:
                    self.add_file_to_hedera(interface)
                else:
                    self.append_file_to_hedera(interface)
            print(self.receipt.toString())
