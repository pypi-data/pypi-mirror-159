...
class HelpCommand:
    ...
    def __generateBaseCommandsList(self):
        ...
        for command in self.base_commands:
            commandDetail = f"{str(self.prefix)}{str(self.client.get_command(command).help)}"
    ...
    def __generatePages(self):
        ...
        if cogsExist:
            ...
            for command in self.base_commands_list:
                string += command + "\n"
            embed = discord.Embed(title=f"{self.client.user.name} Commands", description=f"""\t*BASE COMMANDS*
            """ + string)
        pages.append(embed)
        return pages
