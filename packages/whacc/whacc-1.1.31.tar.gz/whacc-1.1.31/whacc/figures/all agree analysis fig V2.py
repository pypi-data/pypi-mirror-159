...
for ind in range(3):
    ...
    df_list = []
    df_list.append(
        analysis.thresholded_error_types(x_and, human_one, edge_threshold=4, frame_num_array=frame, thresholds=[0.5]))
    df_list.append(
        analysis.thresholded_error_types(x_or, human_one, edge_threshold=4, frame_num_array=frame, thresholds=[0.5]))
    df_list.append(
        analysis.thresholded_error_types(x_and, whacc, edge_threshold=4, frame_num_array=frame, thresholds=[0.5]))
    df_list.append(
        analysis.thresholded_error_types(x_or, whacc, edge_threshold=4, frame_num_array=frame, thresholds=[0.5]))
...
""" touch count error version of the plot but with concensus rules and disagree touches are removed"""
from whacc import analysis
