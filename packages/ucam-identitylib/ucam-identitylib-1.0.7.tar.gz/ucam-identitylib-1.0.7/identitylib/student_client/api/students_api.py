...
class StudentsApi(object):
    ...
    def list_v1alpha2_recent_graduates_recent_graduates_get(
        ...
        **kwargs
        ...
        Keyword Args:
            affiliation (str): An affiliation to filter by, in the format `<value>@<scheme>`. When included only graduates with a matching affiliation are returned.. [optional]
    ...
    def list_v1alpha2_students_students_get(
        ...
        **kwargs
        ...
        Keyword Args:
            affiliation (str): An affiliation to filter by, in the format `<value>@<scheme>`. When included only the students with a matching affiliation are returned.. [optional]
            cursor (str): The cursor indicating a unique page of results - this should be auto generated on the `next` and `previous` fields and does not need to be manually added / updated.. [optional]
            _return_http_data_only (bool): response data without head status
                code and headers. Default is True.
            _preload_content (bool): if False, the urllib3.HTTPResponse object
