...
from mixsim.config.parsers.registry_utils import RegistryFunc, withregistry
from mixsim.config.utils import (
    get_type_arguments,
    is_dict,
    is_list,
    is_set,
...
def decode_dataclass(
    cls: Type[BaseModel], d: Dict[str, Any]) -> Dataclass:
    ...
    for name, field in cls.__fields__.items():
        if name not in obj_dict:
            if field.field_info.default is MISSING and field.field_info.default_factory is MISSING:
                logger.warning(
        ...
        try:
            ...
            raise ParsingError(
                f"Failed when parsing value='{raw_value}' into field \"{cls}.{name}\" of type {field.type_}.\n\tUnderlying error is \"{format_error(e)}\"")
...
def decode_field(field: pydantic.Field(), raw_value: Any) -> Any:
...
def get_decoding_fn(cls: Type[T]) -> Callable[[Any], T]:
    ...
    raise Exception(f"No decoding function for type {cls}, consider using decode.register")
