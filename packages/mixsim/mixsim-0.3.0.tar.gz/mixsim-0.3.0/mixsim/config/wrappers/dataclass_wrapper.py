...
from typing import Dict, List, Optional, Type, Union, cast, Callable

from pydantic import BaseModel, Field
from mixsim.config import utils
...
class DataclassWrapper(Wrapper[BaseModel]):
    def __init__(
        self,
        dataclass: Type[BaseModel],
        ...
        for name, field in dataclass.__fields__.items():
            # if not field.init:
            #     continue
            if name[-2:] == "fn":
                continue
            elif utils.is_tuple_or_list_of_dataclasses(field.type_):
                raise NotImplementedError(
                    f"Field {field.name} is of type {field.type_}, which isn't "
                ...
                dataclass, name = field.type_, field.name
                child_wrapper = DataclassWrapper(
                    dataclass, name, parent=self, _field=field
                )
                self._children.append(child_wrapper)

            elif utils.contains_dataclass_type_arg(field.type_):
                dataclass = utils.get_dataclass_type_arg(field.type_)
                name = field.name
                child_wrapper = DataclassWrapper(
    ...
    def add_arguments(self, parser: argparse.ArgumentParser):
        from mixsim.config.argparsing import ArgumentParser
    ...
    @property
    def defaults(self) -> List[BaseModel]:
        if self._defaults:
    ...
    @defaults.setter
    def defaults(self, value: List[BaseModel]):
