...
class Graph(object):
    def __init__(self, config, user=None, cred=None, strict=False):
        ...
        self.client = pyorientdb.OrientDB(config.host, config.port, config.serialization_type)
        ...
        self.scripts = config.scripts or pyorientdb.Scripts()
    ...
    def create_class(self, cls):
        ...
        except pyorientdb.PyOrientSchemaException:
        ...
        for prop_name, prop_value in props:
            ...
            except pyorientdb.PyOrientCommandException:
            ...
            if prop_value.indexed:
                ...
                except pyorientdb.PyOrientIndexException:
    ...
    def edge_from_record(self, record, edge_cls=None):
        ...
        if type(in_prop) is pyorientdb.OrientRecordLink:
            in_hash = in_prop.get_hash()

        out_hash = None
        out_prop = props['out']
        if type(out_prop) is pyorientdb.OrientRecordLink:
    ...
    def element_from_record(self, record):
        if not isinstance(record, pyorientdb.OrientRecord):
            return record

        record_data = record.oRecordData
        try:
            if isinstance(record_data['in'], pyorientdb.OrientRecordLink) and \
