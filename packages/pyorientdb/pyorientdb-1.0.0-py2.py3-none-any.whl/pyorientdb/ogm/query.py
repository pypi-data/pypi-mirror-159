...
class Query(object):
    ...
    def parse_record_prop(self, prop):
        if isinstance(prop, list):
            g = self._graph
            return g.elements_from_links(prop) if len(prop) > 0 and isinstance(prop[0], pyorientdb.OrientRecordLink) else prop
        elif isinstance(prop, pyorientdb.OrientRecordLink):
