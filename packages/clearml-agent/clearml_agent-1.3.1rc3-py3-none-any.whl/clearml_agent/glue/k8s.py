...
class K8sIntegration(Worker):
    ...
    def _get_kubectl_command(self, command="get pods", extra_labels=None):
        labels = [self._get_agent_label()] + (list(extra_labels) if extra_labels else [])
        return 'kubectl {command} -l {labels} -n {namespace} -o=JSON'.format(
            command=command, namespace=self.namespace, labels=",".join(labels)
        )
    ...
    def _monitor_hanging_pods_daemon(self):
        ...
        while True:
            output = get_bash_output(self._get_kubectl_command())
    ...
    def _get_used_pods(self):
        ...
        try:
            kubectl_cmd_new = self._get_kubectl_command()
            ...
            if self.max_pods_limit:
                if current_pods >= self.max_pods_limit:
                    ...
                    for namespace in namespaces:
                        get_bash_output(
                            self.KUBECTL_DELETE_CMD.format(namespace=namespace, selector=self._get_agent_label())
                        )
                    # go to sleep
                    sleep(self._polling_interval)
                    continue
