...
def process(request="request",
    ...
    logging.info(msg="context:{}".format(str(context)))
    data_input = {}
    testing = {}
    # </editor-fold>
    for rule in rules_list:
        if rule["active"] == 'True':
            logging.info(msg="start processing rule config object:{}".format(str(rule)))
            ...
            logging.info(msg="entries to evaluate:{}".format(data_output["log"]["records"]))
            ...
            logging.info(msg="entries went through notification:{}".format(data_output["notifications"]["records"]))
            if data_output["notifications"]["records"] > 0:
                if "slack" in rule["notifications"]:
                    logging.info(msg="prepare sending slack message")
