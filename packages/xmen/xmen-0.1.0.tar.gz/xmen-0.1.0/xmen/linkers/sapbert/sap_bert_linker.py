...
class SapBERTLinker(EntityLinker):
    ...
    def write_dict_embeddings(
        ...
        candidate_dense_embeds = wrapper.embed_dense(term_dict.term.tolist(), agg_mode='cls', show_progress=True, batch_size=batch_size)
        if subtract_mean:
    ...
    def predict_batch(self, dataset, batch_size):
        def get_result(sample):            
            mentions_index, mention_strings = zip(*[(j, ' '.join(mention['text'])) for j, doc_entities in enumerate(sample['entities']) for mention in doc_entities])
    ...
    def _get_concepts(self, mention_strings):
        ...
        for i, _ in enumerate(mention_strings):
            ...
            n_cuis = 0
            cuis, scores = [], []
            yield [{'db_id' : cui, 'score' : score, 'db_name' : self.kb_name} for cui, score in sorted_predicted]