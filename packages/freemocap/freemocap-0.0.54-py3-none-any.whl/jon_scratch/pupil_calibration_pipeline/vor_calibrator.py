...
from jon_scratch.pupil_calibration_pipeline.spherical_coordiate_converters.cart2sph_and_sph2cart import cart2sph, \
    sph2cart
...
class VorCalibrator:
    ...
    def calibrate(self,
        ...
        self.fixation_distance = self.get_distance_between_two_points(fixation_point_fr_xyz,
        ...
        self.uncalibrated_gaze_vector_endpoint_fr_xyz = self.rotate_gaze_lasers([0, 0, 0],
                                                                                eye_socket_origin_fr_xyz,
        ...
        self.calibrated_rotational_offset = self.calculate_optimal_rotational_offset(eye_socket_origin_fr_xyz,
                                                                                     eye_socket_rotation_matricies,
                                                                                     head_rotation_matricies,
        ...
        gaze_vector_endpoint_fr_xyz = self.rotate_gaze_lasers(self.calibrated_rotational_offset,
                                                              eye_socket_origin_fr_xyz,
                                                              eye_socket_rotation_matricies,
                                                              head_rotation_matricies,
                                                              gaze_x,
    ...
    def calculate_optimal_rotational_offset(self,
        ...
        optimization_results = optimize.least_squares(self.get_error_for_a_given_rotational_offset_guess,
    ...
    def get_error_for_a_given_rotational_offset_guess(self,
        ...
        gaze_laser_endpoint_during_vor_fr_xyz = self.rotate_gaze_lasers(rotational_offset_guess,
                                                                        eye_socket_origin_fr_xyz,
                                                                        eye_socket_rotation_matricies,
                                                                        head_rotation_matricies,
                                                                        gaze_x,
        ...
        if self.debug:
            self.plot_optimization_error(error, gaze_laser_endpoint_during_vor_fr_xyz, eye_socket_origin_fr_xyz,
                                         fixation_point_fr_xyz)
        return error

    def plot_optimization_error(self, error, gaze_laser_endpoint_during_vor_fr_xyz, eye_socket_origin_fr_xyz,
                                fixation_point_fr_xyz):
        figure_number = 13451
        ...
        ax.plot(mean_eye_origin_x, mean_eye_origin_y, mean_eye_origin_z, 'mh', label='mean eye origin')

        ax.plot(self.uncalibrated_gaze_vector_endpoint_fr_xyz[self.vor_start_frame:self.vor_end_frame, 0],
                self.uncalibrated_gaze_vector_endpoint_fr_xyz[self.vor_start_frame:self.vor_end_frame, 1],
                self.uncalibrated_gaze_vector_endpoint_fr_xyz[self.vor_start_frame:self.vor_end_frame, 2], 'k-o',
                label='original gaze xyz')

        ax.plot(gaze_laser_endpoint_during_vor_fr_xyz[:, 0],
                gaze_laser_endpoint_during_vor_fr_xyz[:, 1],
                gaze_laser_endpoint_during_vor_fr_xyz[:, 2], 'r-o',
                label='gaze_rotated_by_guess_then_head_rotation_xyz')

        ax.plot(fixation_point_fr_xyz[:, 0],
                fixation_point_fr_xyz[:, 1],
                fixation_point_fr_xyz[:, 2], 'b-o', label='mean_fixation_point_xyz')
    ...
    def rotate_gaze_lasers(self,
        ...
        offset_rotation_matrix_guess = Rotation.from_euler('xyz',
                                                           (offset_x_euler, offset_y_euler, offset_z_euler)).as_matrix()
        ...
        gaze_rotated_by_guess_fr_xyz = [
            offset_rotation_matrix_guess @ original_gaze_fr_xyz[:, this_frame_number] for
            this_frame_number in
            range(original_gaze_fr_xyz.shape[1])]
