...
sessionID = datetime.datetime.now().strftime("sesh_%Y-%m-%d_%H_%M_%S_mch")
...
def stuff_incoming_frames_into_a_queue(camera: OpenCVCamera, thread_queue: queue.Queue):
    ...
    while thread_exit_event.is_set() is False:
        ...
        logger.info('{} grabbed a frame at timestamp {}'.format(camera.name, timestamp_ns/1e9))
        camera.opencv_video_writer_object.write(image)
        if not success:
...
def start_camera_thread(camera: OpenCVCamera, incoming_frames_queue: queue.Queue):
    ...
    cam_vid_filename = camera.name +'_synchronized.mp4'
    camera_save_path = session_path / cam_vid_filename
    camera.initialize_video_writer(str(camera_save_path))
...
if __name__ == '__main__':
    ...
    camera_list = []
    this_port_number = -1
    while camera_found_bool and len(camera_list) < 5:
        this_port_number+=1
