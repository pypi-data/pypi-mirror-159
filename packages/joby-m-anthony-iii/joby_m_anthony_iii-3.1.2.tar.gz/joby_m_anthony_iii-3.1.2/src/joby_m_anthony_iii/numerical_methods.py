...
def spectral_radius(A: tuple) -> float:
	...
	Notes
	-----
	:math:`\rho(\mathbf{A}) = \max|\lambda|`, where :math:`\lambda` is the set of eigenvalues for `A` :cite:`burdenNumericalAnalysis2019`.
	"""
	matrix_name, A = _retrieve_name(A), np.array(A)
	if not(np.sum(A.shape) - A.shape[0] == A.shape[0]):
		raise IndexError(f"Matrix, '{matrix_name}' must be square!")
...
class Norm:
	...
	Notes
	-----
	Definition :cite:`burdenNumericalAnalysis2019`:
	...
	Theorem :cite:`burdenNumericalAnalysis2019`:
...
class EigenValues:
	...
	def inverse_power_method(self, x: tuple, q: float) -> pd.DataFrame:
		...
		Notes
		-----
		Supposed to converge faster than `power_method` :cite:`burdenNumericalAnalysis2019`.
		"""
		self.__vector_name = _retrieve_name(x)
		if np.sum(np.array(x).shape) - np.array(x).shape[0] > 1: raise IndexError(f"Systems vector, {self.__vector_name} must be one-dimensional array!")
		if len(x) != len(self.A): raise IndexError(EigenValues.__BadVectorDataError(self.__matrix_name, self.A.shape, self.__vector_name, np.array(x).shape))
	...
	def qr_algorithm(self) -> pd.DataFrame:
		r"""Approximate dominant eigenvalue and associated eigenvector of matrix, `A` by decomposition :cite:`burdenNumericalAnalysis2019`.
		...
		Notes
		-----
		This method is preferred over `power_method` and `inverse_power_method` by keeping round-off error to a minimum :cite:`burdenNumericalAnalysis2019`.

		Examples
		--------
		Refer to this `example <https://www.youtube.com/watch?v=FAnNBw7d0vg>`_ for an explanation and demonstration :cite:`thebrightsideofmathematicsQRDecompositionSquare2020`.
		"""
...
class SingleVariableIteration:
	...
	def bisection(self) -> pd.DataFrame:
		r"""Root-finding method: :math:`f(x) = 0` :cite:`burdenNumericalAnalysis2019`.
		...
		Notes
		-----
		Relying on the Intermediate Value Theorem (IVT), this is a bracketed, root-finding method. Generates a sequence :math:`{p_{n}}_{n=1}^{\infty}` such :math:`f(x=p_{n}) = 0` and converges by :math:`\mathcal{O}(1 / (2^{N}))` :cite:`burdenNumericalAnalysis2019`.
...
class LeastSquares:
	...
	def power(self) -> Tuple[float, float, FunctionType]:
		r"""Given a domain and range, yield the coefficients for an equation and the equation of the form :math:`y = ax^{b}` :cite:`weissteinLeastSquaresFitting`.
