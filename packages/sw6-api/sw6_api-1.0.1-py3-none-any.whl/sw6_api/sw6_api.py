...
class Shop:
    ...
    def __init__(self, target, username=None, password=None, upload_product_images: bool = True):
        ...
        self.target = target
        # looking for config
        if not os.path.exists(f'.data/{target}_shop_donfig.yaml'):
            ...
            with Progress() as progress:
                ...
                progress.update(progress_task1, advance=1)
            with open(f'.data/{target}_shop_donfig.yaml', 'w') as configfile:
                yaml.safe_dump(config, configfile)
                progress.update(progress_task1, advance=1)
        else:
            config = yaml.safe_load(open(f'.data/{target}_shop_donfig.yaml'))
    ...
    def generate_admin_request(self, method, url, payload=None):
        if payload is None:
            payload = {}
    ...
    def update_all_products(self):
        ...
        datas = [
            {
                ...
                'visibilities': [
                    {
                        'id': md5((uuid + self.sales_channel_id).encode()).hexdigest(),
        ...
        chunks = [
            datas[x:x + 100] for x in range(0, len(datas), 100)
        ]
        for batch in chunks:
            payload = {
                "update_visibilities": {
    ...
    def create_product(self, data):
                ...
                'price': [
                    {
                        ...
                        'gross': float(child_data['purchase_price'].replace(',', '.')),
                        'net': float(child_data['purchase_price'].replace(',', '.')) / 1.19,
                        'linked': True,
                        'listPrice':
                            {
                                'currencyId': self.currency_id,
                                'gross': float(child_data['strike_price'].replace(',', '.')),
                                'net': float(child_data['strike_price'].replace(',', '.')) / 1.19,
                                'linked': True
                            }
                    }
                ],
        ...
        if self.upload_product_images:
            all_images = list(set(image_urls + [image for child_data in data['children'] for image in child_data['images']]))
            tasks = []
        ...
        manufacturor_media_push = self.generate_admin_request(
            'POST', f'https://{self.target}/api/_action/media/{manufacturer_media_id}/upload?extension=webp&fileName={manufacturer_name}_Herstellerbild',
            manufacturor_media_data)
        manufacturor_resp_data = json.loads(manufacturor_media_push.text) if manufacturor_media_push.text != '' else 'no response => success'
        manufacturor_resp_data = ''
    ...
    def edit_cms_pages(self, **sites_contents):
    ...
    def edit_snippets(self, **snippets_contents):
        ...
        with Progress(
            ...
            for snippet_set in snippet_sets:
                if set['attributes']['name'] == 'BASE de-DE':
                    snippet_set_id = snippet_set['id']

            for snippet in snippets:
                for name, value in snippets_contents.items():
                    if name == snippet['attributes']['translationKey']:
                        value['id'] = snippet['id']
