...
class DAppTonClient(TonClient):
    ...
    def deploy_wallet(self, wallet: WalletContract, wait_for_result=False):
        timeout = 20 if wait_for_result else 0
        query = wallet.create_init_external_message()
        base64_boc = bytes_to_b64str(query["message"].to_boc(False))

        return self._run(self.provider.broadcast([BroadcastQuery(boc=base64_boc, timeout=timeout)]))
    ...
    def transfer(self, from_wallet: WalletContract, to_addr: str, amount: TonCurrencyEnum.ton, payload=None,
        ...
        timeout = 10 if wait_for_result else 0
