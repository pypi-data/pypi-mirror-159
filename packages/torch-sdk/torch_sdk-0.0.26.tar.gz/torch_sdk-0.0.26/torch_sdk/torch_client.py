...
from torch_sdk.common import AsyncExecutor, SyncExecutor, Executor
...
class TorchClient:
    ...
    def get_pipeline_run(self, pipeline_run_id: str) -> PipelineRun:
        ...
        Description:
            To get an existing pipeline from torch catalog
        :param uid: run id of the pipeline run
    ...
    def execute_rule(self, rule_type, rule_id, sync=True, incremental=False):
        """
        Description:
        ...
        """
        if sync:
            rule_executor = SyncExecutor(rule_type, self)
            rule_executor.execute(rule_id, incremental)
        else:
            rule_executor = AsyncExecutor(rule_type, self)
            rule_executor.execute(rule_id, incremental)
        return rule_executor

    def get_rule_status(self, rule_type, execution_id):
        """
        Description:
        ...
        return rule_executor.get_status(execution_id)

    def get_rule_result(self, rule_type, execution_id):
        """
        Description:
        ...
        """
        rule_executor = Executor(rule_type, self)
        return rule_executor.get_result(execution_id)
