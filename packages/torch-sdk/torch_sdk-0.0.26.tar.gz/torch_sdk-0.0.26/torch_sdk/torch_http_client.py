...
from torch_sdk.models.dqrule import DataQualityRuleExecutionResult
from torch_sdk.models.reconcillationrule import ReconcillationRuleExecutionResult
...
class TorchHttpClient(TorchClientInterFace, ABC):
    ...
    # get pipeline run with particular id
    ...
    def get_reconciliation_rule_result(self, execution_id) -> ReconcillationRuleExecutionResult:
        response = self._get(
            url=f'{self._catalog_api_base}/rules/reconciliation/executions/{execution_id}/result'
        )
        return ReconcillationRuleExecutionResult(**response)

    def get_dq_rule_result(self, execution_id) -> DataQualityRuleExecutionResult:
        response = self._get(
            url=f'{self._catalog_api_base}/rules/data-quality/executions/{execution_id}/result'
        )
        return DataQualityRuleExecutionResult(**response)
