...
import numpy as np
from runml_infuse.utils.sampling import reservoir_sampling
...
def encompass_batching(
    ...
    if backend == 'tensorflow':
        from runml_infuse.cd.tensorflow.preprocess import preprocess_drift
    elif backend == 'pytorch':
        from runml_infuse.cd.pytorch.preprocess import preprocess_drift  # type: ignore[no-redef]
...
def get_input_shape(shape: Optional[Tuple], x_ref: Union[np.ndarray, list]) -> Optional[Tuple]:
    ...
    if isinstance(shape, tuple):
        ...
        logger.warning('Input shape could not be inferred. '
                       'If runml_infuse.models.tensorflow.embedding.TransformerEmbedding '
