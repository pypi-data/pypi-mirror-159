...
from .color import color_val, COLORS_81
...
def draw_bboxes(img,
                ...
                wait_time=0,
                out_file=None):
    """Draw bboxes and class labels (with scores) on an image with colors.
    ...
    for bbox, label in zip(bboxes[:,:5], bboxes[:,-1].astype('int32')):
        ...
        left_top = (bbox_int[0], bbox_int[1])
        right_bottom = (bbox_int[2], bbox_int[3])
        color = COLORS_81[label%80 + 1].tolist()
        ...
        text_w, text_h = cv2.getTextSize(label_text, font, font_scale, thickness)[0]
        cv2.rectangle(img, left_top, (bbox_int[0] + text_w, bbox_int[1] + text_h + 5), color, -1)
        cv2.putText(img, label_text, (bbox_int[0], bbox_int[1] + 15),
