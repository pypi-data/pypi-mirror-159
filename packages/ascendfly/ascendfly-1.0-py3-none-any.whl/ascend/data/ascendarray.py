...
class AscendArray():
    ...
    def to_numpy(self, nbytes=None):
        ...
        if self._flag != 'HOST':
            array = np.zeros(self._shape, self._dtype)
            try:
                array_ptr = acl.util.numpy_to_ptr(array)
            except:
                array_ptr = acl.util.bytes_to_ptr(array.tobytes())

            memcpy_d2h(array_ptr, self._data, _nbytes)
        else:
            try:
                array = acl.util.ptr_to_numpy(self._data, self._shape, np_type)
            except:
                bytes_data = acl.util.ptr_to_bytes(self._data, _nbytes)
