...
class Cursor(common.DBAPICursor):
    ...
    def cancel(self):
        ...
        self.connection.query_cancel(self._query_id)
    ...
    def fetchone(self):
        ...
        if len(self._batch) == 0:
            ...
            if len(self._batch) == 0:
                return None
        _logger.info("length of batch: ", self._batch)
        self.rows_count += len(self._batch)
        _logger.info("Count: ", self.rows_count)
        if self.rows_count <= 1000:
            _logger.info("e6x - Breaking the loop after 1000 - fetch many records")
            return self._batch
