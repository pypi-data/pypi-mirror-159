...
def _process_target_audio(mixdb: MixtureDatabase,
    ...
    for idx in range(len(target_file_index)):
        ...
        augmented_target_audio.append(apply_augmentation(audio_in=raw_target_audio[target_file_index[idx]],
    ...
    mixture.samples = max([len(item) for item in augmented_target_audio])

    for idx in range(len(augmented_target_audio)):
        pad = mixture.samples - len(augmented_target_audio[idx])
        augmented_target_audio[idx] = np.pad(augmented_target_audio[idx],
                                             (0, pad),
                                             mode='constant',
                                             constant_values=0)
    ...
    mixture.class_count = get_class_count(mixdb=mixdb,
                                          mixture=mixture,
                                          target_audio=augmented_target_audio)

    return sum(augmented_target_audio)
