...
class Stat:
    ...
    def calculating(self):
        ...
        if self.statsbool['R2']:
            R2_mat = self.get_R2_mat()
            c_nan = np.isnan(R2_mat).sum()
            R2_mat = pd.DataFrame(R2_mat).fillna(0)
            total = R2_mat.shape[0] * (R2_mat.shape[0]-1)/2 - c_nan

            self.result['R2'] = (sum(R2_mat.sum()) - R2_mat.shape[0])/total
    ...
    def get_R2_mat(self):
        ...
        cpgAnn = self.tabixCPG(self.cpg_path)
        [MC, M0, M1, count, strand_] = self.buildBinaryMatrix()
        ...
        for i in range(0, len(sites_pos)):
            ...
            if not self.start < ref_posArray[i] <= self.end:
                continue
            for j in [j for j in range(0, len(sites_pos)) if sites_pos[j] >= sites_pos[i]]:
                if not self.start < ref_posArray[j] <= self.end:
                    continue
                pj = sites_pos[j]


                (N00, N01, N10, N11), (Ni0, Ni1, Nj0, Nj1) = self.count2pos(MC, M0, M1, count, pi, pj)
                r2, pval = self.D_r2(N11, N10, N01, N00)
                R_matrix[i][j] = r2
