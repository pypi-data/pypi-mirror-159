...
class multicallerbsc(object):
    ...
    def loadMultiCall(self):
        ...
        multiCall = self.web3.eth.contract(
            self.web3.toChecksumAddress(multicallAddress), abi=abi)
        return(multiCall)
    ...
    def getContract(self, address, abiString):
        ...
        contract = self.web3.eth.contract(
            self.web3.toChecksumAddress(address), abi=abi)
        return(contract)
    ...
    def execute(self):
        ...
        while retries < self.maxRetries:
            ...
            try:
                    ...
                    while not success and internalRetries < maxInternalRetries:
                        try:
                            ...
                            for rawOutput, decoder in zip(outputDataRaw, sublistDecoder):
                                outputData.append(self.decodeData(
                                    self.listToString(decoder), rawOutput))
                            success = True
                        except OverflowError:
                            internalRetries += 1
                            print("Internal retry", internalRetries,
                                  "of", maxInternalRetries)
                ...
                self.batches += 1
                print("Too many requests in one batch. Reattempting with",
                      self.batches, "batches...")
            except Exception as e:
                print("Attempt", retries, "of",
                      self.maxRetries, "failed. Retrying...")
                # self.reset();
                # raise e;
        self.reset()
        return(outputData)
