...
def lof_outlier_removal(dataset, threshold=10):
...
def plot_lof_outlier_removal(dataset, threshold=10):
    LOF = LocalOutlierFactor(n_neighbors=threshold)
    Y_hat = LOF.fit_predict(dataset)
    mask = Y_hat != -1
    out_mask = Y_hat == -1
    new_dataset = dataset[mask]
    outliers = dataset[out_mask].reshape(-1,1)
    plt.rcParams.update({'font.family':'times new roman', 'font.size':11.5,
                        'text.color':'darkblue'})
    plt.figure(figsize=(13,6))
    plt.suptitle('Local Outlier Method', fontsize=16, color='black')
    plt.subplot(1,2,1)
    plt.title('Un-preprocessed dataset')
    plt.boxplot(dataset)
    plt.subplot(1,2,2)
