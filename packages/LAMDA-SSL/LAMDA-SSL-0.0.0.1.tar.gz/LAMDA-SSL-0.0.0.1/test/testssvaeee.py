...
parser.add_argument('--alpha', type=float, default=0.1, help='Classification loss multiplier controlling generative vs. discriminative learning.')
    ...
    with tqdm(total=n_batches, desc='epoch {} of {}'.format(epoch+1, args.n_epochs)) as pbar:
        for i in range(n_batches):
            ...
            if is_supervised:
                ...
                y = None
            x = x.to(args.device).view(x.shape[0], -1).bernoulli()
            print(x.max())
