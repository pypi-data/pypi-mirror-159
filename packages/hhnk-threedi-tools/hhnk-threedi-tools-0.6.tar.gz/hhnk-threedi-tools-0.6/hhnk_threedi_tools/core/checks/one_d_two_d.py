...
def run_levels_depths_at_timesteps(timestep_df, dem_path, grid_result, folder, revision, ):
    """
    ...
    try:
        ...
        timestrings = [int(round(grid_result.nodes.timestamps[t] / 60 / 60, 0)) for t in timesteps_arr]

        dem_list, dem_nodata, dem_meta = hrt.load_gdal_raster(dem_path)

        for timestep, timestr in zip(timesteps_arr, timestrings):
            # output files
            wlvl_output_path = getattr(folder.output.one_d_two_d[revision], f'waterstand_T{timestr}').path
            depth_output_path = getattr(folder.output.one_d_two_d[revision], f'waterdiepte_T{timestr}').path
            ...
            _ = create_depth_raster(wlvl_list, dem_list, dem_nodata, dem_meta, depth_output_path)
        return timestrings
    except Exception as e:
        raise e from None


def run_node_stats(grid_result, timestep_df):
    """
...
def run_flowline_stats(threedi_result, timestep_df):
    """
    ...
    flowlines_gdf = read_flowline_results(threedi_result=threedi_result, timesteps_df=timestep_df)
    pumplines_gdf = read_pumpline_results(threedi_result=threedi_result, timesteps_df=timestep_df)
