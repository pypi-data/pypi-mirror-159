...
class ImplicitEndpoint:
    ...
    def flask_view(self):
        ...
        if view_type == 'API' and request.method == 'POST':
            ...
            #if self.request.headers['Origin'] == 'http://localhost:3000/':
                # set and get as
            response = []
            for call in data:
        ...
        if view_type == 'API' and request.method == 'POST':
            ...
            if self.request.headers['Origin'] == 'http://localhost:3000/':
                ...
                if 'Authorization' in self.request.headers:
                    self.request.session.session_key = self.request.headers['Authorization']
            response = []
            for call in data:
                if "name" in call and call['name'] in self._functions:
                    try:
            ...
            if is_localhost and session_id:
                ret.headers['Authorization'] = session_id
        elif view_type == 'JS':
