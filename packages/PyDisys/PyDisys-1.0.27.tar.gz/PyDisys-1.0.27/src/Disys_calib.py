...
class Disys_Calib:
    ...
    def __init__(self,path_to_data : str):
        ...
        try :         
            ...
            log.exception("Error : files not found. Input path may be not correct.")
        ...
        log.info("Disys_Calib initialized : Amount of data {}.".format(n))
        log.info("Data features {}, labels features {}".format(X_feature_names.values,Y_feature_names.values))
    ...
    def __init__(self,df_input : pd.DataFrame, mainpath, serialNum):
        ...
        try :    
            ...
            log.exception("Error : data not found. Check the names!")
    ...
    def CreateNN(self,width:int,height:int):
        ...
        print("\n********** Start training **********")
        log.info("Amount of data for training: {}".format(n_train))
        log.info("Amount of data for test: {}".format(n_test))
        log.info("\n********** Start training **********")
        ...
        for i in range(step):
            ...
            log.info("ERRORE RETE step {}/{}: Y_test ->  min {:.4f} , mean {:.4f} , max {:.4f} ".format((i+1),step,np.min(err),np.mean(err),np.max(err)) )
    ...
    def SetLoggerHandlers(self):
        # self.consoleH = logging.StreamHandler()
        self.fileH = logging.FileHandler('pyoutput.log')
        # self.consoleH.setLevel(logging.WARNING)
        self.fileH.setLevel(logging.INFO)
        # log.addHandler(self.consoleH)
