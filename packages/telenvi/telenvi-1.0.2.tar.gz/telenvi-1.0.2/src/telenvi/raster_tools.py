...
class GeoIm:
    ...
    def __init__(self, ds, array = None, ds_encoding = gdalconst.GDT_UInt16, ar_encoding = np.uint16):
        ...
        if type(array) == np.ndarray:
            ...
            self.array = self.ds.ReadAsArray().astype(ar_encoding)

        self.ds_encoding = ds_encoding
        self.ar_encoding = self.array.dtype
    ...
    def _updateArray(self):
        ...
        self.array = self.ds.ReadAsArray().astype(self.ar_encoding)
...
""")
    ...
    def copy(self):
        ...
        """
        return GeoIm(self.ds, array = self.array, ar_encoding = self.ar_encoding, ds_encoding = self.ds_encoding)
    ...
    def shiftOriginPoint(self, offsetX, offsetY, inplace=False):
        ...
        # dans la fonction d'initialisation du nouveau GeoIm.
            ...
            return GeoIm(shiftedDs, array = self.array, ar_encoding=self.ar_encoding, ds_encoding=self.ds_encoding)
    ...
    def cropFromVector(self, vector, polygon=0, inplace=False):
        ...
        if inplace:
            ...
            return GeoIm(crop_ds, ar_encoding=self.ar_encoding, ds_encoding=self.ds_encoding)
    ...
    def resize(self, xSize, ySize, method="near", inplace=False):
        ...
        if inplace:
            ...
            return GeoIm(res_ds, ar_encoding = self.ar_encoding, ds_encoding = self.ds_encoding)
    ...
    def merge(self, ls_geoim, inplace=False):
        ...
        if inplace:
            ...
            return GeoIm(merged_ds, ar_encoding=self.ar_encoding, ds_encoding=self.ds_encoding)
    ...
    def save(self, outpath, driverName="GTiff"):
        ...
        createRasterFile(self.ds, outpath, driverName, array = self.array, ds_encoding=self.ds_encoding)
...
def openGeoRaster(
    ...
    ds_mode = False,
    ar_encoding = None,
    ds_encoding = gdalconst.GDT_UInt16,
    verbose = False,
    ):
    ...
    :returns:
        ds : an osgeo.gdal.Dataset instance if read_array == False
    ...
    if numBand != None:
        if verbose: print(f"extract band\n---\n{numBand}\n---\n")
        inDs = chooseBandFromDs(inDs, numBand)
    ...
    if ds_mode :
        ...
        # Find array encoding type
        if ar_encoding == None:
            ar_encoding = inDs.ReadAsArray(0,0,1,1).dtype
            print(ar_encoding)

        geoim = GeoIm(inDs, ds_encoding = ds_encoding, ar_encoding = ar_encoding)
...
def cropDsFromIndex(ds, index, ds_encoding = gdalconst.GDT_UInt16, ar_encoding = np.uint16):
    """
        :descr:
            cut the image according to an matrixian area
...
def stackDs(ls_ds, ar_encoding = np.uint16):
    """
        :descr:
...
def createRasterFile(ds, outpath, driverName="GTiff", array = None, ds_encoding=gdalconst.GDT_UInt16):
