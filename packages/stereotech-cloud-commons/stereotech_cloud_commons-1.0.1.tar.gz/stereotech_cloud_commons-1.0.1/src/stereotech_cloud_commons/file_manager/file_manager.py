...
if TYPE_CHECKING:
    ...
    DBComp = database.StereotechCloudDatabase
    SCMDComp = shell_command.ShellCommandFactory
    _T = TypeVar("_T")

VALID_GCODE_EXTS = ['.gcode', '.g', '.gco', '.ufp', '.nc']
FULL_ACCESS_ROOTS = ["gcodes", "config", "images"]
...
class FileManager:
    def __init__(self, config: ConfigHelper) -> None:
        ...
        gc_path: str = config.get("gcode_path", "")
        images_path: str = config.get("images_path", "")
        ...
        if images_path:
            self.register_directory('images', images_path)
    ...
    def register_directory(self, root: str, path: Optional[str]) -> bool:
        ...
        if not os.access(path, permissions):
            logging.info(
                f"\nSteapp does not have permission to access path "
                f"({path}) for ({root}).")
            return False
        if path != self.file_paths.get(root, ""):
            ...
            if root == "gcodes":
                ...
                stereotech_cloud_db = db.wrap_namespace("stereotech_cloud")
                stereotech_cloud_db["file_manager.gcode_path"] = path
    ...
    def get_image_directory(self) -> str:
        return self.file_paths.get('images', '')
    ...
    def gen_temp_upload_path(self) -> str:
        ...
        return os.path.join(
            tempfile.gettempdir(),
            f"stereotech_cloud.upload-{loop_time}.mru")
...
class MetadataStorage:
    def __init__(self,
        ...
        version = db.get_item(
            "stereotech_cloud", "file_manager.metadata_version", 0)
        if version != METADATA_VERSION:
            # Clear existing metadata when version is bumped
            for fname in self.mddb.keys():
                self.remove_file_metadata(fname)
            db.insert_item(
                "stereotech_cloud", "file_manager.metadata_version",
                METADATA_VERSION)
        self.pending_requests: Dict[
            str, Tuple[Dict[str, Any], asyncio.Event]] = {}
        self.busy: bool = False
        if self.gc_path:
            # Check for removed gcode files while stereotech_cloud was shutdown
