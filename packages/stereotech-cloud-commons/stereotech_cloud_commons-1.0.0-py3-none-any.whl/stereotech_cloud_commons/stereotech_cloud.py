#!/usr/bin/env python3
# Steapp - HTTP/Websocket API Server for Klipper
#
...
class Server:
    ...
    def __init__(self,
                 args: Dict[str, Any],
                 file_logger: Optional[utils.SteappLoggingHandler],
    ...
    def start(self) -> None:
        ...
        logging.info(
            f"Starting Steapp on ({self.host}, {hostport}), "
            f"Hostname: {hostname}")
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Steapp - Klipper API Server")
    parser.add_argument(
        "-c", "--configfile", default="~/stereotech_cloud.conf",
        metavar='<configfile>',
        help="Location of stereotech_cloud configuration file")
    parser.add_argument(
        "-l", "--logfile", default="/tmp/stereotech_cloud.log", metavar='<logfile>',
    ...
    if sys.version_info < (3, 7):
        msg = f"Steapp requires Python 3.7 or above.  " \
    ...
    while True:
        try:
            ...
            logging.exception("Steapp Error")
            estatus = 1
            break
        try:
            server.start()
