...
class SteSlicer(PrusaSlicer):
    def check_identity(self, data: str) -> Optional[Dict[str, str]]:
        match = re.search(r";STE Slicer 5D Gcode\s(.*)", data)
        if match:
            return {
                'slicer': "Stereotech STE Slicer",
                'slicer_version': '~0.3'
            }
        return None
    ...
    def parse_filament_total(self) -> Optional[float]:
        filament = self._parse_sum_float(
            r";Filament\sused:\s(\d+\.?\d*)m", self.header_data)
        if filament is not None:
            filament *= 1000
        return filament
    ...
    def parse_estimated_time(self) -> Optional[float]:
        return self._parse_sum_float(r";TIME:.*", self.header_data)
...
SUPPORTED_SLICERS: List[Type[BaseSlicer]] = [
    ...
    Cura, SteSlicer, Simplify3D, KISSlicer, IdeaMaker, IceSL]
SUPPORTED_DATA = [
