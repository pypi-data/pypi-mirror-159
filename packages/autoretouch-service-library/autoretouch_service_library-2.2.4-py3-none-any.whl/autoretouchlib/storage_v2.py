...
class Storage:
    ...
    def store_from_filename(
        ...
        with open(filename, "rb") as file:
            blob = file.read()
        return self.store(blob, FileType.from_filename(filename), organization_id)

    def uri_for(self, organization_id: OrganizationId, content_hash: Union[FileContentHash, str]):
        if isinstance(content_hash, str):
            content_hash = FileContentHash(content_hash)
        headers = {} if not self.id_token else {"Authorization": f"Bearer {self.id_token}"}
        params = {
        ...
        if response.status_code not in (200, 201):
            logging.warning(response.status_code, response.content.decode('utf-8'))
            raise HTTPException(status_code=response.status_code,
                                detail=f"could not get uri from storage: {response.content.decode('utf-8')}")
