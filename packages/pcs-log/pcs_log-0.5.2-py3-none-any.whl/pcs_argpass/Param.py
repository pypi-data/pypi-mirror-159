...
This is only raised within the "Process"-function.
    ...
    def _InitTranslation(self) -> dict:
        return {
            ...
            'NoBool':
                "Value '{OptValue}' for parameter {ParKey} is not valid boolean (YyTtJj1NnFf)",
            'ParamNotDefined':
...
#           Text += "\n"
    ...
    def __Make_OptName(self,OptionNameIn):
        ...
        if '.' in OptionName:
            ...
            if len(wList) != 2:
                raise self.ParamError(f"Error in prefixed parameter {OptionName}") from None # PrefixError
        ...
        for DefArgName in self.__Definition.keys():
            ...
            if Req:
                if not DefArgName in self.keys():
                    ...
                    **{'DefArgName':DefArgName, 'ParList':ParList}))
