...
class Note(object):
    ...
    def segments(self):
        ...
        if self.start and self.start > 0:
            ...
            start_breaks = list(frange(0.0, self.start, SAY_SEGMENT_SILENCE_DURATION, 10))
        ...
        for index, total_time in enumerate(segment_breaks[1:]):
            segment = self._get_segment(index, note, velocity, phoneme, type=self.segment_options.get('type', 'note'))
            self._segments.append(segment)
    ...
    def play(self):
        ...
        for note in self.notes:
            cmd_kwargs = prepare_options_for_say(
                text=note.to_say_text(), **self.get_kwargs(note=note.note, type="note")
            )
    ...
    def notes(self):
        if not len(self._notes):
            for note in self.sequence:
                self._notes.append(Note(**self.get_kwargs(**note)))
        return self._notes
