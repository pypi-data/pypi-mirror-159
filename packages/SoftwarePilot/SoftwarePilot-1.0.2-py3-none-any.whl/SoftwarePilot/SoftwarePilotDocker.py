...
class SoftwarePilotDocker:
	...
	def list_containers(self):
		...
		table = PrettyTable(["id", "image", "name", "status", "labels"])
		for c in self.client.containers.list():
			image_name = c.image.tags[0].split(':')[0]
			table.add_row([c.short_id, image_name, c.name, c.status, c.labels])
		print (table)
	'''
	...
	def list_images(self):
		...
		table = PrettyTable(["id", "tags", "labels"])
		for i in self.client.images.list():
			#image_name = c.image.tags[0].split(':')[0]
			table.add_row([i.short_id, i.tags, i.labels])
		print (table)
	'''
