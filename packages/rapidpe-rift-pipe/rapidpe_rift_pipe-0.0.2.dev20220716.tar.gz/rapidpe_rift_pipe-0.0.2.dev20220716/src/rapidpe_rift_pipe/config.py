...
#    exe_compute_intrinsic_grid: str
    ...
    def load(fname) -> "Config":
        ...
        # Executables
        attrs["exe_create_event_dag"] = cfg.get("General", "exe_create_event_dag")
        attrs["exe_generate_initial_grid"] = cfg.get(
            "General", "exe_generate_initial_grid",
            fallback="",
        )
        attrs["exe_grid_refine"] = cfg.get("General", "exe_grid_refine")
        attrs["exe_integration_extrinsic_likelihood"] = cfg.get(
...
_initial_grid_setup_spec = {
    ...
    # TODO: split this into 2 groups, one for gstLAL templates and one for
    #       fixed region.
    "default" : {
    }
}
