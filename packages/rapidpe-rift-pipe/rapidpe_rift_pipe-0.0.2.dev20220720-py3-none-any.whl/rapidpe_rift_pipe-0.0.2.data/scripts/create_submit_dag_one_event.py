...
#    if ("initial_grid_xml" not in event_info) and config.exe_generate_initial_grid == "":
    if not (cfg.has_option("General","exe_generate_initial_grid") and cfg.get("General","exe_generate_initial_grid") != "") and not "initial_grid_xml" in event_info:
        exe_generate_initial_grid = cfg.get("General","exe_generate_initial_grid")
        sys.exit("ERROR: if the initial_grid_xml is not provided, you need to provide the executable to generate the inital grid")
elif len(sys.argv) == 3:
    event_info = convert_dict_string_to_dict(sys.argv[2])

else:
    sys.exit("ERROR: need to pass cfg_name and string of dict of event args if event not in config. args given:"+sys.argv)
...
#        os.system("mkdir "+init_directory+"/"+config.output_parent_directory)
    ...
    integrate_likelihood_cmds_dict = convert_section_args_to_dict(cfg,"LikelihoodIntegration")
...
#    integrate_likelihood_cmds_dict["output-file"] = integration_output_file_base+"0.xml.gz"
    ...
    if cfg.has_section("CreateEventDag"):
        it_0_dag_cmd += convert_cfg_section_to_cmd_line(cfg,"CreateEventDag")
    ...
    grid_job = write_generic_dag_job(config.exe_grid_refine, "GridRefine", convert_section_args_to_dict(cfg,"GridRefine"),convert_section_args_to_dict(cfg,"GridRefine_condor_commands"))
    ...
    iteration_job = write_generic_dag_job(config.exe_create_event_dag, "CreateEventDag", convert_section_args_to_dict(cfg,"CreateEventDag"),convert_section_args_to_dict(cfg,"CreateEventDag_condor_commands"))
...
#        grid_job_node.add_macro("macroresultfile", result_file)
        ...
        if indx == (config.n_iterations_per_job-1):
            ...
            iter_pt_str += "convert_result_to_txt.py "+output_dir+" 0.95\n"
            iter_pt_str +="plot_skymap.py "+output_dir+" 0.95\n"
            iter_pt_str +="plot_intrinsic_posterior.py "+output_dir+" "+config.distance_coordinates +"\n"
            iter_pt_str += "create_summarypage.py "+ output_dir +"\n"
...
def write_generic_dag_job(exe, job_name,dict_of_kwargs,dict_of_condor_commands={},log_dir="logs/",log_files_uniq_str = "$(cluster)-$(process)"):
    ###
    ...
    dag_job.add_condor_cmd('accounting_group',config.accounting_group)
