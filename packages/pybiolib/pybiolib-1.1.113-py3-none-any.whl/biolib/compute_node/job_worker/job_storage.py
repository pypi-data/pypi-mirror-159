...
class JobStorage:
    ...
    @staticmethod
    def upload_module_output(job_uuid: str, module_output: bytes) -> None:
    ...
    @staticmethod
    def get_input(job: Job) -> RemoteIndexableBuffer:
        presigned_download_url = BiolibJobApi.get_job_storage_download_url(job=job, storage_type='input')
        return RemoteIndexableBuffer(url=presigned_download_url)
