...
# Plotting functions
def node(k, ax, p, name, scale=1, color=None, fontsize=None, nodesize=1):
    if color is None:
        color = 'gray'
    if fontsize is None:
        fontsize = 8*scale
...
def link(k1, k2, ax, p, weight, bend=0, scale=1, nodesize=1, alpha=None):
    # Node coordinates
...
def link_auto(k, ax, p, weight, v=None, scale=1, nodesize=1, alpha=None):
    # Node coordinates
...
def plot_network(inter, layout, width=1, height=1, scale=1, names=None,
    ...
    ax.axis('off')
    pos = ax.get_position()
    plt.xlim([-w*pos.width/2, w*pos.width/2])
    plt.ylim([-h*pos.height/2, h*pos.height/2])
    scale = scale * np.min([pos.width,pos.height])
    # Draw nodes
    for k in range(G):
        node(k, ax, p, names[k], scale, fontsize=fontsize, color=vcolor[k],
            nodesize=nodesize)
    # Draw links
    for k1, k2 in e:
        weight = inter[k1,k2]
        if k1 != k2:
            if (k2, k1) in e:
                link(k1, k2, ax, p, weight, bend, scale, nodesize=nodesize,
                    alpha=alpha)
            else: link(k1, k2, ax, p, weight, 0, scale, nodesize=nodesize,
                alpha=alpha)
        else:
            v = vdict.get(k1)
            if v is None:
            ...
            link_auto(k1, ax, p, weight, v, scale, nodesize=nodesize,
                alpha=alpha)
