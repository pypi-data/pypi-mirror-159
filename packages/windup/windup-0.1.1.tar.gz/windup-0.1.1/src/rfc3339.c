...
static int _get_local_utc_offset(void) {
    if (!local_utc_offset) {
        ...
        local_utc_offset = (int)info.tm_gmtoff;
#else
        local_utc_offset = 0;
#endif
    }
...
//        (int)floor(fraction + 0.5) : (int)ceil(fraction - 0.5);
    ...
    // use math->modf >>>
    double integer_temp;
    double fraction = modf(timestamp, &integer_temp);
    int usec = floor(fraction * 1000000 + 0.5);
    // use math->modf <<<
...
static void _format_date_time(date_time_struct *dt, char* datetime_string) {
    ...
    sprintf(
        ...
        sign,
        offset / HOUR_IN_SECS,
        offset / MINUTE_IN_SECS % HOUR_IN_MINS
    );
}
...
static void check_date_time_struct(date_time_struct *dt) {
    if ((*dt).ok != 1) {
        if ((*dt).date.ok != 1) {
            PyErr_SetString(
                PyExc_ValueError,
                "Unsupported date-time string. Date invalid."
            );
        } else if ((*dt).time.ok != 1) {
            PyErr_SetString(
                PyExc_ValueError,
                "Unsupported date-time string. Time invalid."
            );
        } else {
...
static PyObject *to_string(PyObject *self, PyObject *args) {
    PyObject *obj = NULL;
...
// convert
static PyObject *str_usec_to_msec(PyObject *self, PyObject *args) {
    char *usecs;

    if (!PyArg_ParseTuple(args, "s", &usecs))
        return NULL;

    char msecs[4];
    int msec = (int)((float)atoi(usecs) / 1000.0 + 0.5);
    sprintf(msecs, "%03d", msec);

    return PyUnicode_FromString(msecs);
}
...
static PyMethodDef rfc3339_methods[] = {
    ...
    {
        "str_usec_to_msec",
        (PyCFunction) str_usec_to_msec,
        METH_VARARGS,
        PyDoc_STR("Convert usec to msec.")
    },
    {NULL}
};
...
PyMODINIT_FUNC PyInit_rfc3339(void) {
