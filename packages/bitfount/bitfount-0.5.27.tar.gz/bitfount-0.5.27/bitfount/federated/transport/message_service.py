...
from grpc import (  # type: ignore[attr-defined]  # Reason: https://github.com/grpc/grpc/issues/20940  # noqa: B950
    RpcError,
    StatusCode,
    aio,
)
...
class _AutoRetryMessageServiceStub:
    ...
    def __init__(self, orig: MessageServiceStub) -> None:
        ...
        self.PodConnect = cast(
            aio.UnaryUnaryMultiCallable, _auto_retry_grpc(orig.PodConnect)
        )
        self.SetupTaskMailboxes = cast(
            aio.UnaryUnaryMultiCallable, _auto_retry_grpc(orig.SetupTaskMailboxes)
        )
        self.SendBitfountMessage = cast(
            aio.UnaryUnaryMultiCallable, _auto_retry_grpc(orig.SendBitfountMessage)
        )
        self.GetLargeObjectStorage = cast(
            aio.UnaryUnaryMultiCallable, _auto_retry_grpc(orig.GetLargeObjectStorage)
        )
        ...
        self.GetBitfountMessage = cast(
            aio.UnaryUnaryMultiCallable,
            orig.GetBitfountMessage,
        )
        self.AcknowledgeMessage = cast(
            aio.UnaryUnaryMultiCallable, orig.AcknowledgeMessage
        )

        # This is deprecated but we attach it anyway
        self.StartTraining = cast(
            aio.UnaryUnaryMultiCallable, _auto_retry_grpc(orig.StartTraining)
        )
...
class _MessageService:
    ...
    def __init__(self, session: BitfountSession, config: MessageServiceConfig):
        ...
        self._stub: Optional[_AutoRetryMessageServiceStub] = None
    ...
    async def stub(self) -> _AutoRetryMessageServiceStub:
        ...
        if self._stub is None:
            self._stub = _AutoRetryMessageServiceStub(await self.config.stub)
        return self._stub
