...
def get_pmonitor_server(cfg_dir):
    ...
    module      = importlib.import_module('.'.join(classname.split(".")[:-1]))
    klass       = getattr(module, classname.split('.')[-1])
    server_endpoint = pmonitor_server_cfg['server_endpoint']
    config      = pmonitor_server_cfg['config']
    pmonitor_server = klass(server_endpoint, config)
...
def main(daemon_args: Dict, quit_requested: Callable[[], bool]) -> None:
    ...
    try:
        ...
        logger.warn(f"main: unable to register node, node_id={node_id}, error: {e['resp']['status']}")
    ...
    try:
        ...
        logger.warn(f"main: unable to deregister node, node_id={node_id}, error: {e['resp']['status']}")
