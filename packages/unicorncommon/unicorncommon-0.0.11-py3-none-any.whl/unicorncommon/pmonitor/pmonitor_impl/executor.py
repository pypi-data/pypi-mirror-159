...
class Executor:
    ...
    def handle_request_run_application(self, request):
        ...
        try:
            try:
                ...
                logger.exception(f"{log_prefix}: failed to create run, error: {e['resp']['status']}")
            ...
            if not launched:
                ...
                if run is not None:
                    try:
                        ...
                        logger.exception(f"{log_prefix}: failed to delete run({run['id']}), error: {e['resp']['status']}")
    ...
    def handle_request_run_task(self, request):
        ...
        try:
            ...
            logger.exception(f"{log_prefix}: failed to get task({task_id}), error: {e['resp']['status']}")
            return {"status": e['resp']['status']}
        ...
        try:
            ...
            if task['is_singleton']:
                try:
                    ...
                    logger.exception(f"{log_prefix}: failed to acquire task lock, error: {e['resp']['status']}")
                    return {"status": e['resp']['status']}
            ...
            try:
                ...
                logger.info(f"{log_prefix}: launch wrapper via command: \"{cmd}\"")
                r = subprocess.Popen(
                    f"pwrapper -r {run['id']} -t {task['id']} --data-dir {self.base_data_dir} --cfg-dir {self.cfg_dir}",
                    shell = True,
                )
                launched = True
                logger.info(f"{log_prefix}: wrapper launched, pid={r.pid}")
            ...
            if not launched:
                ...
                if active_run_set:
                    try:
                        ...
                        logger.exception(f"{log_prefix}: failed to relase task lock, error: {e['resp']['status']}")
                if run is not None:
                    try:
                        self.dbagent_client.delete_run(run['id'])
                        logger.warn(f"{log_prefix}: run({run['id']}) is deleted!")
    ...
    def handle_request_stop_run(self, request):
        ...
        try:
            ...
            logger.exception(f"{log_prefix}: failed to get run, run_id={id}, error: {e['resp']['status']}")
            return {"status": e.['resp']['status']}
        log_prefix = f"{log_prefix}(Run({run['id']})"
