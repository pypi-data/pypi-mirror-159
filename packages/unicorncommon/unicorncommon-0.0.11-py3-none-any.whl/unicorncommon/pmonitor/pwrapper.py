...
# 4: create child process failed, process not launched.
# 5: unable to set run to started after child process is launched, we tried to kill the process.
# 6: unable to remove the task singleton lock after process is terminated
# 7: unable to set run to finished after process is terminated.
# 20: both 6 and 7
#############################################################
...
def cleanup(run_id, task_id, is_singleton, dbagent_client, *, logger):
    ...
    if task_id is None or not is_singleton:
        ...
        try:
            ...
            logger.exception(f"{log_prefix}: failed to release task lock, error is {e['resp']['status']}")
    try:
        dbagent_client.delete_run(run_id)
        logger.info(f"{log_prefix}: Run({run_id}) is deleted")
    except DBAgentException as e:
        logger.exception(f"{log_prefix}: failed to delete run, Run({run_id}), error is {e['resp']['status']}")
...
def main():
    ...
    try:
        ...
        logger.exception(f"{log_prefix}: failed to get run info, erro: {e['resp']['status']}")
    ...
    kill_cmd = run.get("task", {}).get("kill_cmd")
    APP_CTX['kill_cmd'] = APP_CTX
    ...
    try:
        ...
        logger.warn(f"{log_prefix}: set run({run_id}) to started!")
    except DBAgentException as e:
        logger.exception(f"{log_prefix}: failed to set run({run_id}) to started, error: {e['resp']['status']}")
    ...
    if task is None or not task['is_singleton']:
        ...
        try:
            ...
            logger.exception(f"{log_prefix}: failed to release task lock, error: {e['resp']['status']}")
            exit_code = 6
    ...
    try:
        ...
        logger.exception(f"{log_prefix}: failed to set run to finished, error: {e['resp']['status']}")
        if exit_code == 0:
            exit_code = 7
        else:
            exit_code = 20
