...
class Executor:
    def __init__(self, *, dbagent_client, base_data_dir, node_id, cfg_dir):
        self.pmonitor_dir   = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        self.base_data_dir  = base_data_dir
        self.node_id        = node_id
        self.dbagent_client = dbagent_client
        self.cfg_dir        = cfg_dir
        self.pwrapper_cmd   = os.path.join(self.pmonitor_dir, "pwrapper.py")
    ...
    def handle_request_run_application(self, request):
        ...
        try:
            ...
            try:
                cmd = f"{self.pwrapper_cmd} -r {run['id']} --data-dir {self.base_data_dir} --cfg-dir {self.cfg_dir}"
    ...
    def handle_request_run_task(self, request):
        ...
        try:
            ...
            try:
                if task['is_singleton']:
                    cmd = f"{self.pwrapper_cmd} -r {run['id']} -t {task['id']} --is-singleton --data-dir {self.base_data_dir} --cfg-dir {self.cfg_dir}"
                else:
                    cmd = f"{self.pwrapper_cmd} -r {run['id']} -t {task['id']} --data-dir {self.base_data_dir} --cfg-dir {self.cfg_dir}"
                logger.info(f"{log_prefix}: launch wrapper via command: \"{cmd}\"")
                r = subprocess.Popen(
