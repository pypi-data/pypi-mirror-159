...
def format_numero_factura(pto_venta:int,cbte_num:int,tipo_cbte:int):
    if tipo_cbte == 6:
...
def limit_string(string:str,limit:int):
    if len(string) > limit:
        return string[:limit]
    else:
        return string

def make_qr(factura, qr_path):
...
def make_factura_pdf(files_names:dict,factura:dict) -> list:
    ...
    can.drawString(120 * mm, (y_page - 82) * mm, 'Método de pago: Mercado Pago')
    ...
    can.drawString(8 * mm, (y_page - 82) * mm, 'Cliente: '+limit_string(cliente_name,55))
    direccion_cliente = limit_string(factura['data_cliente']['domicilio']['calle'],45) + ' ' + factura['data_cliente']['domicilio']['altura']
    can.drawString(8 * mm, (y_page - 88) * mm, 'Direccion: '+ limit_string(direccion_cliente,55))
    can.drawString(8 * mm, (y_page - 94) * mm, factura['data_cliente']['domicilio']['localidad'])
    can.drawString(8 * mm, (y_page - 100) * mm, 'Email: '+factura['data_cliente']['email'])
    can.drawString(8 * mm, (y_page - 106) * mm, 'Condición: Consumidor Final')
