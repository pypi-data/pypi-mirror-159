...
class RenogyChargeController(minimalmodbus.Instrument):
    ...
    @retry(wait_fixed=200, stop_max_attempt_number=5)
    def retriable_read_float(
        self, registeraddress, functioncode=3, number_of_registers=2, byteorder=0
    ):
        return self.read_float(
            registeraddress, functioncode, number_of_registers, byteorder
        )
    ...
    def get_solar_voltage(self):
        ...
        return self.retriable_read_float(0x0107, 3, 2) * 10

    def get_solar_current(self):
        """PV array input in amps"""
        return self.retriable_read_float(0x0108, 3, 2) * 100

    def get_solar_power(self):
        """PV array input in watts"""
        return self.retriable_read_float(0x0109, 3, 2)

    def get_load_voltage(self):
        """Load output in volts"""
        return self.retriable_read_float(0x0104, 3, 2)  * 10

    def get_load_current(self):
        """Load output in amps"""
        return self.retriable_read_float(0x0105, 3, 2) * 100

    def get_load_power(self):
        """Load output in watts"""
        return self.retriable_read_float(0x0106, 3, 2)

    def get_battery_voltage(self):
        """Battery voltage"""
        return self.retriable_read_float(0x0101, 3, 2) * 10

    def get_battery_state_of_charge(self):
        """Battery state of charge"""
        return self.retriable_read_float(0x0100, 3, 2)
