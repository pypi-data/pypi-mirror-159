...
@prepare_command()
def clone(cnvrg, logger, name, override, cache_link, commit):
    """
    ...
    if os.path.exists(dataset.slug + '/' + CONFIG_FOLDER_NAME) and not override:
        ...
        return
    dataset.clone(progress_bar_enabled=True, override=override, commit=commit, use_cached=cache_link)
...
def verify(cnvrg, logger, names, time_out):
    ...
    while True:
        try:
            ...
            continue
    # return exit status 1 by passinf error=True(in use for Agent)
