...
class vector:
    ...
    def update_lengths(self):      
        self._lengthparts2D=np.zeros(self.nbvertices)
        self._lengthparts3D=np.zeros(self.nbvertices)
...
class Zones(wx.Frame):
    ...
    def OnClickfindactivate_vector(self,event):
        if self.parentGUI is not None:
            self.parentGUI.action = 'select active vector'
            self.parentGUI.active_zones=self

    def OnClickfindactivate_vector2(self,event):
        if self.parentGUI is not None:
            self.parentGUI.action = 'select active vector2'
    ...
    def OnClickInterpolate(self,event):
        
        mysupports = [curv for curv in self.myzones[1].myvectors]
        
        self.myinterp=Interpolator(self.myzones[0].myvectors[0],self.myzones[0].myvectors[1],mysupports,1)
        self.add_interpolator(self.myinterp.interpolants)
    ...
    def add_interpolator(self,pts):
        
        nb=0
        npt=1
        for curpt in pts:
            myzone = zone(name='Triangles'+str(npt),parent=self,is2D=False)
            ...
            for k in range(len(curpt)-1):
                ...
                for i in range(len(curl)-1):
                    curvec = vector(name='tr'+str(k)+'_'+str(i),parentzone=myzone)                
                    curvec.add_vertex([curl[i],curl[i+1],nextl[i]])
                    curvec.close_force()
                    myzone.add_vector(curvec)
                    
                    curvec = vector(name='tr'+str(k+1)+'_'+str(i),parentzone=myzone)                
