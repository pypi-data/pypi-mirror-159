...
class Locpot_yair:
    ...
    def __init__(self, input_locpot=None, axis_index=None, is_2D=False, is_bulk_material = False, Has_interface = True, Locpot_file_name = None, Locpot_file_directory= None, Bulk_Locpot_file_name = None, Bulk_Locpot_file_directory = None,Locpot_Full_path = None, Locpot_bulk_materials_full_path = None, to_flip = False, has_flipped = False):
        ...
        input_locpot : pymatgen.io.vasp.outputs.Locpot, optional, defualt: None
                Can be initialized with an existing pymatgen locpot object.
        axis_index : int,{0: x, 1: y, 2: z}, optional, defualt: 2
        ...
        Locpot_file_name : str, optional, defualt: None
                ...
                are located.
        Locpot_file_directory : str, optional, defualt: None
                Supply the string for the name of the locpot file, and also a string for the full path of the directory where the locpot files
                are located.
        Bulk_Locpot_file_name : list/tuple (str,str), optional, defualt: None
        ...
        Bulk_Locpot_file_directory : list/tuple (str,str), optional, defualt: None
                For two different bulk locpot file names and full paths - supply it in a form of a list\tuple. The first place is for the first bulk material
                and the second place is for the second bulk material.
        Locpot_Full_pat : str, optional, defualt: None
                Supply the string for the the full path of the Locpot file.
        Locpot_bulk_materials_full_path : list/tuple (str,str), optional, defualt: None
    ...
    def get_atoms_position(self):
        ...
        -------
        `numpy.ndarray`, (N,5)
                A matrix of the the coordinates of the atoms. There are 5 columns, the first three are the spacial coordinates of the atoms,
    ...
    def averaging_along_axis(self,locpot_vec = None, to_plot=False,to_plot_atoms=False,to_plot_widths=False,to_plot_heights=False,to_plot_cursor_choice=False):
        ...
        locpot_vec : np.array, optional. default : None
                2 Columns arrays for the spacial coordinates and the local potential values.
        ...
        -------
        `numpy.ndarray`, (N,2)
                It returns a locpot matrix.The first column is the vector that contains the spacial coordinates of system.
    ...
    def find_interface(self, x=None, y=None):
        ...
        Parameters
        ----------
        x : np.array, array_like, optional, defualt: None
                It stands for the spacial coordinates array.
                If not supplied, is calcualted from the instance attribute self.locpot.

        y : np.array, array_like, optional, defualt: None
        ...
        float, float
                ...
                If the system does not contain an interface, it just returns the last spacial grid coordinate position
    ...
    def approximately(self, compare_to, *args, tol=5):
        ...
        *args : int/float/array_like
        ...
        tol : int/float, optional, defualt: 5
    ...
    def find_unit_cell_lattice_parameter(self, is_axis_rel=True):
        ...
        is_axis_rel : bool, optional, defualt: True
    ...
    def set_axis_index(self, axis_ind=None):
        ...
        axis_ind : int, {0: x, 1: y, 2: z}, optional, defualt: None
    ...
    def set_is_2D(self, is2d=False):
        ...
        is2d : bool, optional, defualt: False
    ...
    def find_position_to_insert(self, zz=None, vv=None):
        ...
        system locpot.
        Can be supplied in advance zz - spacial coordinates vector, and vv - local potential vector,
        to be referenced as the main vector to be searched to position to insert.


        Parameters
        ----------
        zz : np.array, optional, defualt: None
        ...
        vv : np.array, optional, defualt: None
        ...
        float, int
                ...
                If we don't have an interface, it returns the last index,position of the spacial grid vector.
        ...
        # at the end of the system. It returns the index of the last position in the spacial grid vector
    ...
    def set_locpot_vec(self):
        ...
        # This loop guaruntee that the length of the system will be at least at the length of 20*spacial_spacing (dx)
    ...
    def elongate_locpot(self, multi_left = 1, multi_right = 1,manual = False,
        ...
        Parameters
        ----------
        multi_left : int, optional, defualt: 1
                It determines how many times to multiply the repeated locpot that will be inserted.
        multi_right : int, optional, defualt: 1
                It determines how many times to multiply the repeated locpot that will be inserted.
        manual : bool, optional, defualt: False
        ...
        pos_to_insert : float, optional, defualt: None
                If `manual =` ``True``, then it must be supplied. It determines where to insert the multiplied repeated locpot.
                It is a position in the spacial grid.
        multi_manual : int, optional, defualt: None
        ...
        z_to_insert_manual : np.array, optional, defualt: None
                If `manual =` ``True``, then it must be supplied. The manully inserted spacial coordinates vector.
        v_to_insert_manual : np.array, optional, defualt: None
                If  `manual =` ``True``, then it must be supplied. The manully inserted local potential vector.
        locpot_vec : `numpy.ndarray`, (N,2), optional, defualt: None
                ...
                the first column is for the spacial coordinates and the second column is for the local potential.

        Returns
        -------
        z_Locp, v_Locp : np.array, np.array
                The new elongated spacial grid vector, the new elongated local potential vector.
        '''
        ...
        if not manual and self.Has_interface:
            ...
            if not self.Has_interface:
                print('Your system does not have an interface, you should handle the elongation manully')
    ...
    def find_locpot_shift(self,locpot_vec = None, bulk_locpot_vec_first = None, bulk_locpot_vec_second = None):
        ...
        ----------
        locpot_vec : `numpy.ndarray`, (N,2), optional, defualt: None
                This is a two-column vector, the first column is for the spacial coordinates and the second column is for the local potential.
        bulk_locpot_vec_first : `numpy.ndarray`, (N,2), optional, defualt: None
                This is a two-column vector, the first column is for the spacial coordinates and the second column is for the local potential. This locpot matrix
                should describe the locpot matrix of the first bulk material.
        bulk_locpot_vec_second : `numpy.ndarray`, (N,2), optional, defualt: None
                This is a two-column vector, the first column is for the spacial coordinates and the second column is for the local potential. This locpot matrix
...
def find_bulk_like_potential(z,v,interface_position, num_of_peaks = 3,limits = None, left = False, right = False):
    ...
    ----------
    z : np.array
        Should be as the same size as wave_function vector. Spacial coordinates vector.
    v : np.array
    ...
    num_of_peaks : int, optional, defualt: 3
        If you have an interface, you can choose how many peaks to look for in the system locpot.
    limits : list\array_like, (float, float), optional, defualt: None
        ...
        Should be given as the same units of the spacial coordainates vector, z.
    left : bool, optional, defualt: False
        A flag that determines whether to look at the left side hand of the system.
    right : bool, optional, defualt: False
        A flag that determines whether to look at the right side hand of the system.

    Returns
    -------
    np.array, np.array, float
        The first argument it returns is the spacial grid range where the bulk-like locpot was evaluated.
    ...
    if not limits is None:
        ...
        if z[0] <= limits[0] <=z[-1] and z[0]<= limits[-1] <=z[-1]:
            ...
            raise ValueError('You should supply limits that are within the range of the spacial coordinates vector you supllied')
    ...
    assert mid_peak_z - z_peaks[mid_peak_ind - np.int64(np.floor(num_of_peaks/2))] > 0
    z_start = z_peaks[mid_peak_ind - np.int64(np.floor(num_of_peaks/2))]
    assert  z_peaks[mid_peak_ind + np.int64(np.floor(num_of_peaks/2))] <= z_peaks[-1]
...
def insert_potential_into_position(z_vec_original, v_vec_original, index_to_insert, z_vec_to_insert, v_vec_to_insert,
    ...
    multi : int, optional, defualt: 0
        When the defualt value is `0`, it means that there is no multiplication.
...
def fix_potential_edges(z, v):
    ...
    ----------
    z : np.array
        Represents the spacial coords vector.
    v : np.array
    ...
    simple_peak = False
    # Checkign for one the simplest kind of peak (sinusidual peak)
...
def multiply_z_v_vecs(z_vec_original, v_vec_original, multi):
    ...
    ----------
    z_vec_original : np.array
        The spacial coordinates vector whom we wish to multiply.
    v_vec_original :  np.array
    ...
    -------
    np.array, np.array
        The first array is the new multiplied spacial coordinates vector. The second array is the new multiplied local potential vector.
    '''
...
#     '''
#
#     Fit sinusodial potential vector to the input spacial sequence coordinates.
#     Parameters
#     ----------
#     zz : 1D np.array
...
def is_peak_maximum(peaks, peak):
    ...
    peaks : `numpt.ndarray`, (N,2)
        The first column stands for the spacial poisition of the peaks.
