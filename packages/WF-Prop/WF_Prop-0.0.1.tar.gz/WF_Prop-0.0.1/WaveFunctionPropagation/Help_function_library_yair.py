...
def find_peaks(x, y, to_plot= False, dist = None):
    ...
    x : np.array, array_like
        The Spacial coordinates vector.
    y : np.array, array_like
       The functional values of the spacial coordiantes vector. The definiion for `f(x) = y`. Usually will be the local potential vector.
    to_plot : bool, optional, defualt: False
        Flag that determines whether to enable plotting or not.
    dist : float, optional, defualt: None
...
def find_peaks_maxima(x, y, ignore_local_maxima = False, to_plot = False, prominence = None):
    ...
    ----------
    x : np.array, array_like
        Spacial coordinates vector.
    y : np.array, array_like
        Potential values v(z) vector.
    ignore_local_maxima : bool, optional, defualt: False
...
def get_prominence(z,v,num_of_min = None,is_max = False):
    ...
    num_of_min : int, optional, the default is None
...
def find_peaks_minima(x, y, ignore_local_minima = False,to_plot = False, prominence=None):
    ...
    y : np.array, array_like
       The potential values (v(z)) vector
    ignore_local_minima : bool, optional, defualt: False
...
def create_k_grid(N, L, dimention='1D'):
    ...
    dimention : str, {'1D', '2D'}, optional, defualt: '1D'
...
def normalize_wave_function_numerically(z,wave_function, units = 'Meter'):
    ...
    ----------
    z : np.array, array_like
        The spacial coordinates vector where the wave function spreads.
    wave_function : np.array, array_like
        The complex values of the wave function in at the relevant z positions.
    units : str, {'Meter', 'Angstrum'], optioanl, defualt: 'Meter'
...
def normalize_wave_function(wave_equation, bound_up=np.inf, bound_down=0, units = 'Meter'):
    ...
    bound_up : int/float, optional, defualt: smp.oo
        The intgration boundaries. (Defualt = inifinity)
    bound_down : int/float, optional, defualt: -smp.oo
        The intgration boundaries. (Defualt = -inifinity)
    units : str, {'Meter', 'Angstrum'], optioanl, defualt: 'Meter'
         The choices are in the brackets. It defines the units of the normilization factor.
    Returns
...
def is_normalized(z, psi_values,units = 'Meter'):
    ...
    ----------
    z : np.array, array_like
        The spacial coordinates the wave function is specified at.
    psi_values : np.array, array_like
        The wavefunction values along the spacial coordinates. Must suit the units for angstrums.
    units : str, {'Meter', 'Angstrum'], optioanl, defualt: 'Meter'
         The choices are in the brackets. It defines the units of the normilization factor.
    Returns
    -------
...
def get_psi0(k0, sigma, z0, z, dimention='1D', sqrd_psi=False,units= 'Meter'):
    ...
    dimention : str, {'1D', '2D'}, optional, defualt: '1D'
        The choices are in the brackets. It defines the K-grid for 1D or 2D mesh.
    sqrd_psi : bool, optional, defualt: False
    ...
    -------
    np.array, np.array
        The first vector represents the spacial coordinates grid
...
def get_first_derivative_fft(x, y, L=None):
    ...
    x : np.array
        The spacial coordinates vector.
    y : np.array
        The appropriate function-values corresponds the spacial coordinates vector.
    L : float, optional, defualt: None
        It should be supplied with the length of the system. It stands for the creation of the K-grid vector.
    Returns
    -------
    np.array, np.array
        It returns the spacial coordinates vector and the first derivative vecotr.
    '''
    x = to_1D_vec(x)
    y = to_1D_vec(y)
    if L is None:
...
def get_second_derivative_fft(x, y, L=None):
    ...
    -------
    np.array, np.array
        It returns the spacial coordinates vector and the second derivative vecotr.
    '''
    x = to_1D_vec(x)
    y = to_1D_vec(y)
    if L is None:
...
def get_first_derivative(x, y):
    ...
    -------
    np.array, np.array
        It returns the spacial coordinates vector and the first derivative vector.
    '''
    x = to_1D_vec(x)
    y = to_1D_vec(y)
...
def get_second_derivative(x, y):
    ...
    -------
    np.array, np.array
        It returns the spacial coordinates vector and the second derivative vector.
    '''
    x = to_1D_vec(x)
    y = to_1D_vec(y)
...
def initial_momentum_yair(sigma, E0, L, z0, A, T0):
    ...
    ----------
    sigma : float/int
        Standard deviation in spacial grid for the gaussin wave function.
    E0 : float/int
...
def find_adjacent_minima_maxima(z,v,start_peak_pos,reverse = False,prominence=None):
    ...
    reverse : bool, optional, defualt: None
...
def run_one_time_step(wave_function, time_step, v, z , k):
    ...
    z : np.array.
        Should be as the same size as `wave_function` vector.
        This is the spacial coordinates vector.
    k : np.array.
        Should be as the same size as `wave_function` vector.
    ...
    if not is_normalized(z,wave_function,units = 'Meter'):
        ...
        print(np.real(np.trapz(wave_function * np.conj(wave_function), z)))
...
def cumulative_probabilty(wave_function, z, z_cutoff, dz):
    ...
    wave_function : np.array
        The Wave function values vector to be propagated.
    z : np.array
        The z spacial coordinates vector
    z_cutoff : float
        The maximum value of to calculate the accumulated probability.
    dz : float
        The spacial gird differnce between points.
    Returns
    -------
    float
...
def adjust_grid_density(density,z,v):
    ...
    -------
    np.array, np.array
        It returns the new spacial coordinate vecor as z, and the new local potneital vector as v.
...
def reach_plateau(x,y,critiria_plat = 0.0001, tol_2 = 0.005):
    ...
    ----------
    x : np.array
        The independent variable, usually will be the spacial grid vector or time space vector.
    y : np.array
        The dependent variable. The matching value of a function to the respective x value. Can be the comulative probabilty.
    critiria_plat : float. optional, defualt: 0.0001
    ...
    tol_2 :  float. optional, defualt: 0.005
...
def plateau_val(x,y,critiria = 0.0001):
    ...
    ----------
    x : np.array
        The spacial coordinates array.
    y : np.array
        The relevent functional values that corresponds the spacial coordinates vector.
    critiria : float, optional, defualt: 0.0001
