...
class ProjectDB(DynamoModel):
    ...
    def get_project_statuses(self) -> List[ProjectStatus]:
        return [ProjectStatus(status) for status in self.project_status.split(",")]
...
class ProjectCredential(DynamoModel):
    """
        Used for DB row entry
    """
    project_uuid: str
    ...
    @staticmethod
    def dict_to_model(data: dict) -> 'ProjectCredential':
        return ProjectCredential(**data, project_uuid=data['primary'], service_name=data['secondary'])

    def create(self):
        AuthDynamos.project_credentials.create_item(self.get_pkey_values(), self.to_dynamo_dict())

    @staticmethod
    def get(project_uuid: str) -> 'List[ProjectCredential]':
        credentials: List[ProjectCredential] = []
        for item in AuthDynamos.project_credentials.get_items([
            project_uuid
        ]):
            credentials.append(ProjectCredential.dict_to_model(item))

        return credentials

    @staticmethod
    def get_for(project_uuid: str, service_name: str) -> 'ProjectCredential':
        return ProjectCredential.dict_to_model(
...
def __cloud_participant(cloud_participant_request: CloudParticipantRequest) -> Union[bool, CloudParticipantResponse]:
    ...
    with sentry_sdk.start_span(
        ...
        try:
            if not cloud_participant_request.ignore_project_enabled and not project.is_project_enabled():
...
def __remove_sensitive_data(cloud_participant_response: CloudParticipantResponse,
                            safe_participant: bool = False) -> dict:
