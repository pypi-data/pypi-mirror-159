...
class ToggleJupyterTensorboardApp(
    ...
    @property
    def name(self):
        return 'jupyter_tensorboard_p {}'.format(
            'enable' if self._toggle_value else 'disable')

    @property
    def description(self):
        return """
{} the jupyter_tensorboard_p extension in config.
Usage
...
class JupyterTensorboardApp(extensions.BaseExtensionApp):
    ...
    description = (
        'Enable or disable '
        'the jupyter_tensorboard_p extension')
    subcommands = dict(
        enable=(
            EnableJupyterTensorboardApp,
            'Enable the jupyter_tensorboard_p extension.'),
        disable=(
            DisableJupyterTensorboardApp,
            'Disable the jupyter_tensorboard_p extension.'),
    )
    examples = '\n'.join([
...
if __name__ == '__main__':  # pragma: no cover
    print('run jupyterlab tensorboard application')
    main()
