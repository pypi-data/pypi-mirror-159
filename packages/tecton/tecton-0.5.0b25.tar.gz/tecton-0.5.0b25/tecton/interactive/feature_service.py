...
class FeatureService(Fco):
    ...
    def get_historical_features(
        ...
        if conf.get_bool("ALPHA_QUERYTREE_ENABLED"):
            ...
            newtree = spine_node
            for fv in self.feature_views:
                fvtree = fv.get_historical_features(spine)._querytree
                newtree = JoinNode.ref(newtree, fvtree, how="inner", join_cols=fv._fd_wrapper().join_keys)
            return DataFrame._create(newtree)
