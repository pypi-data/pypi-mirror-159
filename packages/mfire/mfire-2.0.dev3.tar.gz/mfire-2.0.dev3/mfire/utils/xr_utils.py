...
def change_grid(da: xr.DataArray, grid_out: str) -> xr.DataArray:
    ...
    with xr.open_dataset(Settings().altitudes_filename, cache=False) as dgrid:
        grid_da = rounding(
            dgrid[grid_out].rename(
                {
                    f"latitude_{grid_out}": "latitude",
                    f"longitude_{grid_out}": "longitude",
                }
            )
        )
