...
class Local(BaseTarget):
    ...
    def do_send_as(self, local, remote, sudo):
        local =  Path(local)
        assert local.is_file()
        remote = Path(remote)
        ret, out, err = super().run(f'mkdir -p {remote.parent}', sudo=sudo)
        ok = ret == 0
        ret, out, err = super().run(f'cp {local} {remote}', sudo=sudo)
        ok = ok and ret == 0
    ...
    def do_fetch_as(self, remote, local, sudo, quiet=False):
