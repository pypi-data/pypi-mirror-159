...
SPLIT_IDS_FILE_PATTERN = '{split_idx}_ids.npy'
SPLIT_KERNEL_FILE_PATTERN = '{split_idx}_kernels.npy'
RNG_SEED = 123
...
# def to_channel_first_array(stimulus: pd.DataFrame) -> np.ndarray:
#     """
#     Converts a stimulus DataFrame to a Numpy array with shape (4, T).
# 
#     Where T is for example 24000. 
# 
#     Pandas likes to have index as the first column, whereas it is more
#     standard for ML datasets to have the "channel" dimension first. 
#     """
#     return stimulus.to_numpy().T
...
def create_kernel_training_data(
    ...
    for rec_id, df in by_rec:
        df = df.droplevel('Recording')
        assert type(rec_id) == int
        write_kernel_data(up_stimulus, df, rec_id, save_dir, kernel_len,
                post_kernel_pad, sampling_freq, samples_per_file)
...
def write_kernel_data(
    ...
    cluster_ids, kernels = spike_windows(up_stimulus, response, kernel_len,
            post_kernel_pad, stimulus_sampling_freq)
    _save_cluster_ids(cluster_ids, rec_id, data_root_dir)
    assert len(cluster_ids) == len(kernels)
    # Shuffle the cluster ids and kernels together.
    rng = np.random.default_rng(RNG_SEED)
    shuffle_idxs = np.arange(len(cluster_ids))
    rng.shuffle(shuffle_idxs)
    cluster_ids = cluster_ids[shuffle_idxs]
    kernels = kernels[shuffle_idxs]
    ...
    # Save the kernels and ids across multiple files.
    def save_splits():
        ids_split = np.array_split(rec_clusters, out_kernels_per_file)
        kernels_split = np.array_split(kernels, out_kernels_per_file)
        assert len(ids_split) == len(kernels_split)
        for i, (ids, ks) in enumerate(zip(ids_split, kernels_split)):
            _write_kernel_data_part(ids, ks, rec_dir, i)
    save_splits()


def _write_kernel_data_part(ids, kernels, save_dir, split_id):
    """
    Saves a subset of the of the kernel data.

    This is used internally to spread the kernel data over multiple files.
    """
    ids_filename = SPLIT_IDS_FILE_PATTERN.format(split_idx=split_id)
    kernels_filename = SPLIT_KERNEL_FILE_PATTERN.format(split_idx=split_id)
    ids_save_path = pathlib.Path(save_dir) / ids_filename
    kernel_save_path = pathlib.Path(save_dir) / kernels_filename
    with ids_save_path.open('wb') as f:
        np.save(f, ids)
    with kernel_save_path.open('wb') as f:
        np.save(f, kernels)
    return ids_save_path, kernel_save_path


def spike_windows(up_stimulus: pd.DataFrame, response: pd.DataFrame, 
        kernel_len: int = 800, post_kernel_pad: int = 200, sampling_freq=1000
        ) -> Tuple[np.ndarray, np.ndarray]:
    """
    Extract all spike windows.
    ...
    Returns: two np.ndarrays as a tuple. The first element contains ids of
        the clusters, and the second element contains the spike windows.
    """
    response = response.reset_index(level='Cell index').drop('Kernel', axis=1, 
            errors='ignore')

    def create_windows(spikes) -> List[np.ndarray]:
        """Create the kernels for a list of spikes."""
        wins = []
    ...
    # Gather the kernels and align them with a matching array of cluster ids.
    cluster_ids = []
    windows = []
