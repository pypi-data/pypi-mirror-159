...
def write_kernel_data(
    ...
    windows, cluster_ids, rec_ids = spike_windows(up_stimulus, response, 
            kernel_len, post_kernel_pad, stimulus_sampling_freq)
    assert np.all(rec_ids == rec_id), 'Expected windows for just 1 recording.'
...
def spike_windows(up_stimulus: pd.DataFrame, response: pd.DataFrame, 
    ...
    """
    rec_name_to_id = _list_to_index_map(recording_names(response))
    response = response.reset_index(level=('Cell index', 
        'Recording')).drop('Kernel', axis=1, errors='ignore')
    ...
    for row_as_tuple in response.itertuples(index=False):
        cluster_id, rec_name, spikes = row_as_tuple
        rec_id = rec_name_to_id[rec_name]
        wins = create_windows(spikes)
        cluster_ids.extend([cluster_id,]*len(wins))
        rec_ids.extend([rec_id,]*len(wins))
        windows.extend(wins)
    windows = np.stack(windows)
    cluster_ids = np.array(cluster_ids)
    rec_ids = np.array(rec_ids)
    return windows, cluster_ids, rec_ids
