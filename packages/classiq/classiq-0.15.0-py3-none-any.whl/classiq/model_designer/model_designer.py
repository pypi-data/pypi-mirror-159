...
from classiq.interface.generator.function_call import IOName, WireName
...
class ModelDesigner(function_handler.FunctionHandler, metaclass=AsyncifyABC):
    ...
    def create_inputs(
        ...
        wires = super().create_inputs(input_names=input_names)
        self._model.inputs = self._wire_obj_to_wire_name_dict(self._generated_inputs)
        return wires

    def set_outputs(self, outputs: Dict[IOName, wire.Wire]) -> None:
        super().set_outputs(outputs=outputs)
        self._model.outputs = self._wire_obj_to_wire_name_dict(self._generated_outputs)

    @staticmethod
    def _wire_obj_to_wire_name_dict(
        wire_obj_dict: Dict[IOName, wire.Wire]
    ) -> Dict[IOName, WireName]:
        return {
            io_name: wire_obj.wire_name for io_name, wire_obj in wire_obj_dict.items()
        }
