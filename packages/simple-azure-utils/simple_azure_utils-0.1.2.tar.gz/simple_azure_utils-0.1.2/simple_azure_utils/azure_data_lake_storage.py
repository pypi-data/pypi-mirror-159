...
class AzureStorageAccount:
    ...
    def upload_large_file(self, file_path, upload_path, metadata=None, overwrite=False, storage_account_name=None, settings_path=None):
        data_lake_client = self._get_storage_account_client(storage_account_name, settings_path)
        container_name = self._get_container_name(upload_path)
        upload_path = self.get_blob_path(upload_path)
        blob_client = data_lake_client.get_blob_client(container=container_name, blob=upload_path)
        if blob_client.exists and overwrite is True:
            blob_client.delete_blob()
            blob_client.create_append_blob()
        elif blob_client.exists and overwrite is False:
            error_message = 'Blob exists but overwrite is False'
            logging.error(error_message)
            raise Exception(error_message)
        upload_chunk_size = 4*1024*1024  # upload up to 4 MB at a time
        with open(file_path, 'rb') as f:
            upload_results = None
            while True:
                file_content_chunk = f.read(upload_chunk_size)
                if not file_content_chunk:
                    if metadata is not None:
                        set_metadata_results = blob_client.set_blob_metadata(metadata=metadata)
                        return {'upload_results': upload_results, 'set_metadata_results': set_metadata_results}
                    return {'upload_results': upload_results}
                upload_results = blob_client.append_block(file_content_chunk)
