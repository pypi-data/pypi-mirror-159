...
class BaseClient(object):
    ...
    def fast_swipe(self, x1, y1, x2, y2, velocity=0.5):
        """
        velocity: the smaller the faster
        """
        data = dict(fromX=x1, fromY=y1, toX=x2, toY=y2, velocity=velocity)
        return self._session_http.post('/wda/drag', data=data)

    def test(self, type: str, attribute: str, value: str, accurate="1", index="0"):
        """
        type: "9"-button "48"-staticText "49"-textField "50"-secureTextField "21"-NavigationBar
            "52"-TextView "1"-Other "43"-Image "75"-Cell "40"-Switch "32"-CollectionView
            "42"-Link "19"-Keyboard "20"-Key "45"-SearchField "7"-Alert
        attribute: "0"-identifier "1"-label
        """
        data = dict(type=type, attribute=attribute, value=value, accurate=accurate, index=index)
        return self._session_http.post('/wda/test', data=data)

    def longPress(self, element: dict, duration = 5):
        """
        type: "9"-button "48"-staticText "49"-textField "50"-secureTextField "21"-NavigationBar
            "52"-TextView "1"-Other "43"-Image "75"-Cell "40"-Switch "32"-CollectionView
            "42"-Link "19"-Keyboard "20"-Key "45"-SearchField "7"-Alert
        attribute: "0"-identifier "1"-label
        """
        data = dict(type=element["type"], attribute=element["attribute"], value=element["value"], accurate=element["accurate"], index=element["index"], duration=duration)
        return self._session_http.post('/wda/longPress', data=data)

    def dragToOtherElement(self, fromElement: dict, toElement: dict):
        """
        type: "9"-button "48"-staticText "49"-textField "50"-secureTextField "21"-NavigationBar
            "52"-TextView "1"-Other "43"-Image "75"-Cell "40"-Switch "32"-CollectionView
            "42"-Link "19"-Keyboard "20"-Key "45"-SearchField "7"-Alert
        attribute: "0"-identifier "1"-label
        """
        data = dict(fromElementType=fromElement["type"], fromElementAttribute=fromElement["attribute"], fromElementValue=fromElement["value"],
                    fromElementAccurate=fromElement["accurate"], fromElementIndex=fromElement["index"], toElementType=toElement["type"],
                    toElementAttribute=toElement["attribute"], toElementValue=toElement["value"], toElementAccurate=toElement["accurate"],
