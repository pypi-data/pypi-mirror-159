...
def _should_run_product_by_liveAnimal(product: dict):
    ...
    logRequirements(product, model=MODEL, term=term_id, key=MODEL_KEY, by='liveAnimal',
                    is_liveAnimal=is_liveAnimal)

    should_run = all([is_liveAnimal])
    logShouldRun(product, MODEL, term_id, should_run, key=MODEL_KEY, by='liveAnimal')
    return should_run
...
def _should_run_product_by_country(product: dict):
    ...
    logRequirements(product, model=MODEL, term=term_id, key=MODEL_KEY, by='country',
                    has_yield=has_yield,
                    not_already_set=not_already_set,
                    grouping=grouping)

    should_run = all([not_already_set, has_yield, grouping])
    logShouldRun(product, MODEL, term_id, should_run, key=MODEL_KEY, by='country')
    return should_run
...
def _should_run_product_by_share_0(product: dict):
    ...
    logRequirements(product, model=MODEL, term=term_id, key=MODEL_KEY, by='economicValueShare',
                    share_is_0=share_is_0)

    should_run = all([share_is_0])
    logShouldRun(product, MODEL, term_id, should_run, key=MODEL_KEY, by='economicValueShare')
    return should_run
...
def run(cycle: dict):
    ...
    return non_empty_list([
        (
            (
                (
                    _run_by_liveAnimal(p, country_id, year) if _should_run_product_by_liveAnimal(p)
                    else _run_by_country(p, country_id, year) if _should_run_product_by_country(p)
                    else None
                ) if should_run_by_country else None
            ) or (
                _product(p, 0) if _should_run_product_by_share_0(p) else None
            )
