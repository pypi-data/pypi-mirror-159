...
from runml_checks import CheckResult
from runml_checks.core import DatasetKind
from runml_checks.core.errors import runml_checksProcessError, runml_checksValueError, NotEnoughSamplesError
from runml_checks.utils.outliers import iqr_outliers_range
...
from runml_checks.vision.utils.image_functions import prepare_thumbnail
    ...
    def __init__(self,
        ...
        if properties is not None:
            ...
            if any(p['output_type'] == 'class_id' for p in self.user_properties):
                raise runml_checksValueError('Properties cannot have class_id as output_type for outliers checks')
        else:
...
def _ensure_property_shape(property_values, data, prop_name):
    ...
    if len(property_values) != len(data):
        raise runml_checksProcessError(f'Properties are expected to return value per image but instead got'
    ...
    if isinstance(property_values[0], t.Sequence):
        if any((not isinstance(x, t.Sequence) for x in property_values)):
            raise runml_checksProcessError(f'Property result is expected to be either all lists or all scalars but'
                                         f' got mix for property {prop_name}')
        if any((not _is_list_of_numbers(x) for x in property_values)):
            raise runml_checksProcessError(f'For outliers, properties are expected to be only numeric types but'
