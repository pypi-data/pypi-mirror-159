...
from runml_checks.vision import Batch, Context, TrainTestCheck, VisionData
from runml_checks.vision.utils.image_functions import draw_bboxes, prepare_thumbnail
...
class NewLabels(TrainTestCheck):
    ...
    def __init__(
        ...
        if not isinstance(max_images_to_display_per_label, int):
            raise runml_checksValueError('max_num_images_to_display_per_label must be an integer')
        if not isinstance(max_new_labels_to_display, int):
            raise runml_checksValueError('max_num_new_labels_to_display must be an integer')
