...
from runml_checks import CheckFailure, CheckResult, SuiteResult
from runml_checks.core import DatasetKind
from runml_checks.core.errors import (DatasetValidationError, runml_checksNotImplementedError, runml_checksNotSupportedError,
                                    runml_checksValueError, ModelValidationError, ValidationError)
...
class Context:
    ...
    def __init__(
        ...
        if model is not None:
            ...
            if not isinstance(model, nn.Module):
                get_logger().warning('runml_checks can\'t validate that model is in evaluation state.'
    ...
    def assert_predictions_valid(self, kind: DatasetKind = None):
        ...
        if error:
            raise runml_checksValueError(error)
