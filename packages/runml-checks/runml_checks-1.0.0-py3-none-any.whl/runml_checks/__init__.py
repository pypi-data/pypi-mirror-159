...
from runml_checks.analytics.anonymous_telemetry import send_anonymous_import_event
from runml_checks.core import (BaseCheck, BaseSuite, CheckFailure, CheckResult, Condition, ConditionCategory,
                             ConditionResult, ModelOnlyBaseCheck, SingleDatasetBaseCheck, SuiteResult,
                             TrainTestBaseCheck)
# TODO: remove in further versions
from runml_checks.tabular import (Context, Dataset, ModelComparisonCheck, ModelComparisonSuite, ModelOnlyCheck,
                                SingleDatasetCheck, Suite, TrainTestCheck)
from runml_checks.utils.ipython import is_notebook
from runml_checks.utils.logger import get_verbosity, set_verbosity
    ...
    def __getattribute__(self, name):
        ...
        if name in routines:
            warnings.warn(
                ...
                'the `runml_checks` package directly is deprecated, please '
                'import from `runml_checks.tabular` instead',
                DeprecationWarning
            )
