...
from runml_checks import CheckFailure, CheckResult
...
from runml_checks.tabular._shared_docs import docstrings
from runml_checks.tabular.dataset import Dataset
from runml_checks.tabular.utils.task_type import TaskType
from runml_checks.tabular.utils.validation import (ensure_predictions_proba, ensure_predictions_shape,
                                                 model_type_validation, validate_model)
from runml_checks.utils.decorators import deprecate_kwarg
from runml_checks.utils.features import calculate_feature_importance_or_none
from runml_checks.utils.logger import get_logger
from runml_checks.utils.metrics import get_default_scorers, init_validate_scorers, task_type_check
from runml_checks.utils.typing import BasicModel
        ...
        for feature_df in self.feature_df_list:
            ...
            if set(data.index).issubset(set(feature_df.index)):
                ...
                if feature_df.loc[data.index].fillna('').equals(data.fillna('')):
                    ...
                    raise runml_checksValueError('Data that has not been seen before passed for inference with static '
                                               'predictions. Pass a real model to resolve this')
        raise runml_checksValueError('Data with indices that has not been seen before passed for inference with static '
                                   'predictions. Pass a real model to resolve this')
...
class Context:
    ...
    def __init__(
        ...
        if train is None and test is None and model is None:
            raise runml_checksValueError('At least one dataset (or model) must be passed to the method!')
        ...
        if feature_importance is not None:
            if not isinstance(feature_importance, pd.Series):
                raise runml_checksValueError('feature_importance must be a pandas Series')
        self._train = train
        self._test = test
        self._model = model
    ...
    def train(self) -> Dataset:
        ...
        if self._train is None:
            raise runml_checksNotSupportedError('Check is irrelevant for Datasets without train dataset')
        return self._train
    ...
    def test(self) -> Dataset:
        ...
        if self._test is None:
            raise runml_checksNotSupportedError('Check is irrelevant for Datasets without test dataset')
        return self._test
    ...
    def model(self) -> BasicModel:
        ...
        if self._model is None:
            raise runml_checksNotSupportedError('Check is irrelevant for Datasets without model')
    ...
    def get_data_by_kind(self, kind: DatasetKind):
        ...
        if kind == DatasetKind.TRAIN:
            ...
            raise runml_checksValueError(f'Unexpected dataset kind {kind}')
    ...
    def finalize_check_result(self, check_result, check, kind: DatasetKind = None):
        ...
        if not isinstance(check_result, CheckResult):
            raise runml_checksValueError(f'Check {check.name()} expected to return CheckResult but got: '
