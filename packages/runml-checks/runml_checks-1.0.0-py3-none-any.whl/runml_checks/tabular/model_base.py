...
from runml_checks.core.check_result import CheckFailure, CheckResult
from runml_checks.core.errors import runml_checksNotSupportedError, runml_checksValueError
from runml_checks.core.suite import BaseSuite, SuiteResult
from runml_checks.tabular.context import Context
from runml_checks.tabular.dataset import Dataset
from runml_checks.utils.ipython import create_progress_bar
...
class ModelComparisonSuite(BaseSuite):
    ...
    def supported_checks(cls) -> Tuple:
        ...
        from runml_checks.tabular.base_checks import ModelComparisonCheck  # pylint: disable=import-outside-toplevel
...
class ModelComparisonContext:
    ...
    def __init__(
        ...
        if isinstance(train_datasets, Dataset) and isinstance(test_datasets, List):
            raise runml_checksNotSupportedError('Single train dataset with multiple test datasets is not supported.')

        if not isinstance(models, (List, Mapping)):
            raise runml_checksValueError('`models` must be a list or dictionary for compare models checks.')
        if len(models) < 2:
            raise runml_checksValueError('`models` must receive 2 or more models')
        ...
        if len(train_datasets) != len(models):
            raise runml_checksValueError('number of train_datasets must equal to number of models')
        if len(test_datasets) != len(models):
            raise runml_checksValueError('number of test_datasets must equal to number of models')
        ...
        for i in range(len(models)):
            ...
            if self.task_type is None:
                ...
                raise runml_checksNotSupportedError('Got models of different task types')
