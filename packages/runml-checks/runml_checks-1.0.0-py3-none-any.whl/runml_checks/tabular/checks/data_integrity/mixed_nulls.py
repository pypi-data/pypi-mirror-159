...
from runml_checks.tabular.utils.messages import get_condition_passed_message
from runml_checks.utils.dataframes import select_from_dataframe
from runml_checks.utils.features import N_TOP_MESSAGE
from runml_checks.utils.strings import format_percent, string_baseform
    ...
    def _validate_null_string_list(self, nsl) -> set:
        ...
        if nsl:
            if not isinstance(nsl, Iterable):
                raise runml_checksValueError('null_string_list must be an iterable')
            if len(nsl) == 0:
                raise runml_checksValueError("null_string_list can't be empty list")
            if any((not isinstance(string, str) for string in nsl)):
                raise runml_checksValueError("null_string_list must contain only items of type 'str'")
            result = set(nsl)
        else:
            # Default values
