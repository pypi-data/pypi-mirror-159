...
from runml_checks.core import CheckResult, ConditionCategory, ConditionResult
from runml_checks.core.errors import (runml_checksProcessError, runml_checksTimeoutError, runml_checksValueError,
                                    NotEnoughSamplesError)
from runml_checks.tabular import Context, SingleDatasetCheck
from runml_checks.utils import gower_distance
...
class OutlierSampleDetection(SingleDatasetCheck):
    ...
    def __init__(
        ...
        if not isinstance(extent_parameter, int) or extent_parameter <= 0:
            raise runml_checksValueError('extend_parameter must be a positive integer')
        if nearest_neighbors_percent <= 0 or nearest_neighbors_percent > 1:
            raise runml_checksValueError('nearest_neighbors_percent must be a float between 0 and 1')
    ...
    def run_logic(self, context: Context, dataset_kind) -> CheckResult:
        ...
        if predicted_time_to_run_in_seconds > self.timeout > 0:
            raise runml_checksTimeoutError(
        ...
        try:
            ...
            raise runml_checksProcessError('Out of memory error occurred while calculating the distance matrix. Try '
    ...
    def add_condition_outlier_ratio_less_or_equal(self, max_outliers_ratio: float = 0.005,
        ...
        if max_outliers_ratio > 1 or max_outliers_ratio < 0:
            raise runml_checksValueError('max_outliers_ratio must be between 0 and 1')
