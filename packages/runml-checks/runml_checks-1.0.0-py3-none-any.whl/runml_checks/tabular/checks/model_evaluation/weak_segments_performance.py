...
from runml_checks import ConditionCategory, ConditionResult, Dataset
from runml_checks.core import CheckResult
from runml_checks.core.check_result import DisplayMap
from runml_checks.core.errors import runml_checksNotSupportedError, runml_checksProcessError
from runml_checks.tabular import Context, SingleDatasetCheck
from runml_checks.tabular.context import _DummyModel
from runml_checks.tabular.utils.task_type import TaskType
from runml_checks.utils.dataframes import default_fill_na_per_column_type
from runml_checks.utils.performance.partition import (convert_tree_leaves_into_filters,
                                                    partition_numeric_feature_around_segment)
from runml_checks.utils.single_sample_metrics import calculate_per_sample_loss
...
class WeakSegmentsPerformance(SingleDatasetCheck):
    ...
    def run_logic(self, context: Context, dataset_kind) -> CheckResult:
        ...
        if len(dataset.features) < 2:
            raise runml_checksNotSupportedError('Check requires data to have at least two features in order to run.')
        ...
        if len(weak_segments) == 0:
            raise runml_checksProcessError('WeakSegmentsPerformance was unable to train an error model to find weak '
