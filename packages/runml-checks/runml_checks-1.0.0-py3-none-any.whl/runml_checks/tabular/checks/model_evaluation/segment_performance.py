...
from runml_checks.core.errors import DatasetValidationError, runml_checksValueError
from runml_checks.tabular import Context, SingleDatasetCheck
from runml_checks.utils.performance.partition import partition_column
...
class SegmentPerformance(SingleDatasetCheck):
    ...
    def __init__(
        ...
        **kwargs
        ...
        if feature_1 and feature_1 == feature_2:
            raise runml_checksValueError('"feature_1" must be different than "feature_2"')

        self.feature_1 = feature_1
        self.feature_2 = feature_2

        if not isinstance(max_segments, int) or max_segments < 0:
            raise runml_checksValueError('"num_segments" must be positive integer')
    ...
    def run_logic(self, context: Context, dataset_kind) -> CheckResult:
        ...
        if self.feature_1 is None and self.feature_2 is None:
            ...
            raise runml_checksValueError('Must define both "feature_1" and "feature_2" or none of them')
        else:
            # If both are defined, must be in dataset columns
            columns = dataset.data.columns
            if self.feature_1 not in columns or self.feature_2 not in columns:
                raise runml_checksValueError('"feature_1" and "feature_2" must be in dataset columns')

        if self.feature_1 not in (dataset.numerical_features + dataset.cat_features):
            raise runml_checksValueError('"feature_1" must be numerical or categorical, but it neither.')

        if self.feature_2 not in (dataset.numerical_features + dataset.cat_features):
            raise runml_checksValueError('"feature_2" must be numerical or categorical, but it neither.')
