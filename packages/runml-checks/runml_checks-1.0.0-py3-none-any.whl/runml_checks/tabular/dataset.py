...
from runml_checks.core.errors import DatasetValidationError, runml_checksNotSupportedError, runml_checksValueError
from runml_checks.tabular.utils.task_type import TaskType
from runml_checks.utils.dataframes import select_from_dataframe
from runml_checks.utils.features import infer_categorical_features, infer_numerical_features, is_categorical
from runml_checks.utils.logger import get_logger
from runml_checks.utils.strings import get_docs_link
...
class Dataset:
    ...
    def __init__(
        ...
        if len(df) == 0:
            raise runml_checksValueError('Can\'t create a Dataset object with an empty dataframe')
        ...
        if label is None:
            ...
            if isinstance(label, pd.Series):
                ...
                if label.name is not None:
                    ...
                    if label_name in self._data.columns:
                        raise runml_checksValueError(f'Data has column with name "{label_name}", use pandas rename to'
                                                   f' change label name or remove the column from the dataframe')
            elif isinstance(label, pd.DataFrame):
                # Validate shape
                if label.shape[1] > 1:
                    raise runml_checksValueError('Label must have a single column')
                # Set label name
                label_name = label.columns[0]
                label = label[label_name]
                if label_name in self._data.columns:
                    raise runml_checksValueError(f'Data has column with name "{label_name}", change label column '
                                               f'or remove the column from the data dataframe')
            elif isinstance(label, np.ndarray):
                # Validate label shape
                if len(label.shape) > 2:
                    raise runml_checksValueError('Label must be either column vector or row vector')
            ...
            if label.shape[0] != self._data.shape[0]:
                raise runml_checksValueError('Number of samples of label and data must be equal')

            # If no label found to set, then set default name
            if label_name is None:
                label_name = 'target'
                if label_name in self._data.columns:
                    raise runml_checksValueError('Can\'t set default label name "target" since it already exists in '
            ...
            if label_name not in self._data.columns:
                raise runml_checksValueError(f'label column {label_name} not found in dataset columns')
        else:
            raise runml_checksValueError(f'Unsupported type for label: {type(label).__name__}')
            ...
            if index_name is not None:
                if isinstance(index_name, str):
                    if index_name not in self._data.index.names:
                        raise runml_checksValueError(f'Index {index_name} not found in dataframe index level names.')
                elif isinstance(index_name, int):
                    if index_name > (len(self._data.index.names) - 1):
                        raise runml_checksValueError(f'Dataframe index has less levels than {index_name + 1}.')
                else:
                    raise runml_checksValueError(f'When set_index_from_dataframe_index is True index_name can be None,'
            ...
            if datetime_name is not None:
                if isinstance(datetime_name, str):
                    ...
                    if datetime_name > (len(self._data.index.names) - 1):
                        raise runml_checksValueError(f'Dataframe index has less levels than {datetime_name + 1}.')
                else:
                    raise runml_checksValueError(f'When set_index_from_dataframe_index is True index_name can be None,'
                                               f' int or str, but found {type(index_name)}')
        ...
        if features is not None:
            ...
            if len(difference) > 0:
                raise runml_checksValueError('Features must be names of columns in dataframe. '
        ...
        if self._label_name in self.features:
            raise runml_checksValueError(f'label column {self._label_name} can not be a feature column')

        if self._datetime_name in self.features:
            raise runml_checksValueError(f'datetime column {self._datetime_name} can not be a feature column')

        if self._index_name in self.features:
            raise runml_checksValueError(f'index column {self._index_name} can not be a feature column')

        if cat_features is not None:
            if set(cat_features).intersection(set(self._features)) != set(cat_features):
                raise runml_checksValueError('Categorical features must be a subset of features. '
    ...
    def from_numpy(
        ...
        **kwargs : Dict
        ...
        if len(columns_array.shape) != 2:
            raise runml_checksValueError(columns_error_message)
    ...
    def index_col(self) -> t.Optional[pd.Series]:
        ...
        if self._set_index_from_dataframe_index is True:
            ...
            if self._index_name is None:
                ...
                raise runml_checksValueError(f'Don\'t know to handle index_name of type {type(self._index_name)}')
    ...
    def assert_label(self):
        ...
        Raises
        ------
        runml_checksNotSupportedError
        """
        if not self.label_name:
            raise runml_checksNotSupportedError(
    ...
    def cast_to_dataset(cls, obj: t.Any) -> 'Dataset':
        ...
        if isinstance(obj, pd.DataFrame):
            get_logger().warning(
                'Received a "pandas.DataFrame" instance. It is recommended to pass a "runml_checks.tabular.Dataset" '
