...
__all__ = ['partition_column', 'runml_checksFilter', 'runml_checksBaseFilter', 'convert_tree_leaves_into_filters',
           'intersect_two_filters', 'partition_numeric_feature_around_segment']


class runml_checksFilter:
...
class runml_checksBaseFilter(runml_checksFilter):
    """Extend runml_checksFilter class for feature range based filters.
    ...
    def copy(self):
        ...
        return runml_checksBaseFilter(self.filters.copy(), self.filter_functions.copy(), self.label)


def intersect_two_filters(filter1: runml_checksFilter, filter2: runml_checksFilter) -> runml_checksFilter:
    """Merge two runml_checksFilters into one, an intersection of both filters."""
    return runml_checksFilter(filter1.filter_functions + filter2.filter_functions)
...
def partition_column(
    ...
    if column_name in dataset.numerical_features:
        ...
        if len(percentile_values) == 1:
            ...
            return [runml_checksFilter([f], label)]
        ...
        for start, end in zip(percentile_values[:-1], percentile_values[1:]):
            ...
            filters.append(runml_checksFilter([f], label))
        return filters
        ...
        for i in range(n_large_cats):
            ...
            filters.append(runml_checksFilter([f], str(cat_hist_dict.index[i])))

        if len(cat_hist_dict) > n_large_cats:
            f = lambda df, values=cat_hist_dict.index[:n_large_cats]: ~df[column_name].isin(values)
            filters.append(runml_checksFilter([f], 'Others'))

        return filters


def convert_tree_leaves_into_filters(tree, feature_names: List[str]) -> List[runml_checksBaseFilter]:
    """Extract the leaves from a sklearn tree and covert them into runml_checksBaseFilter.

    The function goes over the tree from root to leaf and concatenates (by intersecting) the relevant filters along the
    way. The function returns a list in which each element is a runml_checksFilter representing the path between the root
    to a different leaf.
    ...
    def recurse(node_idx: int, filter_of_node: runml_checksBaseFilter):
    ...
    filters_to_leaves = recurse(0, runml_checksBaseFilter())
