...
def widget_to_html(
    ...
    """
    my_resources = files('runml_checks.core')
...
def get_docs_link():
    ...
    """
    if runml_checks.__version__ and runml_checks.__version__ != 'dev':
        version_obj: Version = Version(runml_checks.__version__)
        # The version in the docs url is without the hotfix part
        version = f'{version_obj.major}.{version_obj.minor}'
    else:
        version = 'stable'
    return f'https://docs.runml_checks.com/{version}/'
...
def generate_check_docs_link(check):
    ...
    # NOTE:
    # it is better to import runml_checks.tabular.checks, runml_checks.vision.checks
    # to be sure that those packages actually exists and we are using right names,
    # but we do not know with what set of extra dependencies runml_checks was
    # installed, therefore we do not want to cause ImportError.
    # Refer to the setup.py for more understanding

    if not (
        module_path.startswith('runml_checks.tabular.checks')
        or module_path.startswith('runml_checks.vision.checks')
    ):
    ...
    #
    # - runml_checks.tabular.checks.integrity.StringMismatchComparison
    # - https://docs.runml_checks.com/{version}/checks_gallery/tabular/integrity/plot_string_mismatch_comparison.html # noqa: E501 # pylint: disable=line-too-long

    # Remove 'runml_checks' from the start and 'checks' from the middle
    module_path = module_path[len('runml_checks.'):]
