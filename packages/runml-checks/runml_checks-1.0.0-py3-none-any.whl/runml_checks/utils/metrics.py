...
from runml_checks import tabular  # pylint: disable=unused-import; it is used for type annotations
...
from runml_checks.utils.simple_models import PerfectModel
from runml_checks.utils.strings import is_string_column
from runml_checks.utils.typing import BasicModel, ClassificationModel
...
class runml_checkscorer:
    ...
    def __init__(self, scorer: t.Union[str, t.Callable], name: str):
        ...
        if isinstance(scorer, str):
            ...
            raise errors.runml_checksValueError(message)
    ...
    def validate_fitting(self, model, dataset: 'tabular.Dataset', should_return_array: bool):
        ...
        if should_return_array:
            ...
            if not isinstance(result, np.ndarray):
                raise errors.runml_checksValueError(f'Expected scorer {self.name} to return np.ndarray '
            ...
            if kind not in expected_types:
                raise errors.runml_checksValueError(f'Expected scorer {self.name} to return np.ndarray of number kind '
                                                  f'but got: {kind}')
            # Validate returns value for each class
            if len(result) != len(single_label_data):
                raise errors.runml_checksValueError(f'Found {len(single_label_data)} classes, but scorer {self.name} '
            ...
            if not isinstance(result, Number):
                raise errors.runml_checksValueError(f'Expected scorer {self.name} to return number '
                                                  f'but got: {type(result).__name__}')
    ...
    if isinstance(model, BaseEstimator):
        if not hasattr(model, 'predict_proba'):
            if is_string_column(label_col):
                raise errors.runml_checksValueError(
...
def init_validate_scorers(scorers: t.Mapping[str, t.Union[str, t.Callable]],
                          ...
                          dataset: 'tabular.Dataset',
                          class_avg: bool = True,
                          model_type=None) -> t.List[runml_checkscorer]:
    """Initialize scorers and return all of them as runml_checks scorers.
    ...
    """
    return_array = model_type in [TaskType.MULTICLASS, TaskType.BINARY] and class_avg is False
    scorers: t.List[runml_checkscorer] = [runml_checkscorer(scorer, name) for name, scorer in scorers.items()]
    for s in scorers:
