...
from runml_checks.utils.strings import get_ellipsis
...
def create_failures_dataframe(
    ...
    for it in failures:
        if isinstance(it, check_types.CheckResult):
            ...
            message = (
                it.exception.html
                if isinstance(it.exception, errors.runml_checksBaseError)
                else str(it.exception)
            )
            error_types = (
                errors.DatasetValidationError,
                errors.ModelValidationError,
                errors.runml_checksProcessError,
            )
...
def requirejs_script(connected: bool = True):
    ...
    if connected is True:
        ...
        js = pkgutil.get_data('runml_checks', path).decode('utf-8')
