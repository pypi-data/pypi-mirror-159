...
@click.option('-t', '--toml', type=click.Path(exists=True, dir_okay=False, path_type=pathlib.Path),
...
def print_list_info(name, url, client_id, client_secret, toml, fields, hidden):
    ...
    if url is None or client_id is None or client_secret is None:
        raise click.UsageError("Essential options missing. Either provide 'url', 'client_id', and 'client_secret', or a respective 'toml'-file.")
