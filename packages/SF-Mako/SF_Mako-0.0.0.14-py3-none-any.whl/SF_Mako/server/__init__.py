...
class Response(object):
    ...
    def __init__(self, response=None, status_code=200, file_object=None, redirect=None, ignore_and_drop=False, gzip=False):
        if isinstance(response, File_Object):
            file_object = response
            response = None
            self.response_type = File_Object
        else:
...
class Resolver(object):
    ...
    def check_match(self, request:HTTP_Request) -> Union[list, str, bool]:
        ...
        if type(self.urls) == type(""):
            if self.ignore_case:
                if re.match(self.urls.lower(), request.path.lower()):
                    return self.urls
            else:
            ...
            for path in self.urls:
                if self.ignore_case:
                    if re.match(path.lower(), request.path.lower()):
                        return path
                else:
    ...
    def __repr__(self) -> str:
        return f"< SF_Mako Resolver - Match URLS [{self.urls}], Bind Function: {self.function} >"
...
class __cache_control__:
    def __init__(self):
        self.cache_get = True
        self.cache_post = False
        self.cache_query = False

    def check(self, request:HTTP_Request) -> bool:
        is_cached:bool = False
        if request.method == "GET":
            is_cached = self.cache_get
        if request.method == "POST":
            is_cached = self.cache_post
        if len(request.args.keys) == 0:
            is_cached = True
        else:
            is_cached = self.cache_query
...
class HTTP_Server(object):
    ...
    def __init__(self, HOST="AUTO", PORTS=None, max_connections=5, favicon=None, use_analytics=False, ssl_context=None, buffer_size=16384):
        ...
        self.auto_head_responses = True
        self.cache_rules = __cache_control__()
    ...
    def add_url(self, urls:Union[str, list]=None, methods=["GET"], function=None, ignore_case:bool=False):
        resolver = Resolver(urls, methods, function, ignore_case)
        def add_to_resolvers(func):
            resolver.function = func
            self.add_resolver_direct(resolver)
            return func
    ...
    def add_error_resolver_direct(self, resolver:Error_Resolver):
        ...
        self.error_resolvers[resolver.code] = resolver
    ...
    def __find_resolver__(self, request:HTTP_Request):
    ...
    def __client_responses__(self, user_socket:socket_ops.socket.socket, addr=None):
        ...
        while True:
            ...
            if self.__run_on_directory__ == False:
                # Normal Server
                isHeadRequest = False
                if request.method == "HEAD" and self.auto_head_responses:
                    request.method = "GET"
                    isHeadRequest = True
                ...
                if resolver is None:                            # If it doesnt exist, 
                    ...
                    if isinstance(resp, Response):
                        ...
                        print(f"\n{colorama.Fore.RED}WARNING:{colorama.Fore.RESET} You have enrolled a resolver that did not return anything. Function:", str(resolver.function.__name__), "\n")
    ...
    def __run_nonssl__(self, HOST=None, PORT=80):
        ...
        self.__system_message__("Build Complete")
        print("Server Hosting URL: http://" + host + ":" + str(PORT))
    ...
    def __run_ssl__(self, HOST=None, PORT=443):
        ...
        self.__system_message__("Secure Build Complete")
        print("SSL Server Hosting URL: https://" + host + ":" + str(PORT))
