...
class PALBase(hana_ml.ml_base.MLBase, ModelSavingServices):
    ...
    def __init__(self, conn_context=None):
        ...
        self.progress_id = "{}".format(uuid.uuid1()).replace('-', '_').upper()
    ...
    def set_scale_out(self,
        ...
        route_optimization_level : {'mininal', 'all'}, optional
            ...
            Guides the optimizer to compile with ``route_optimization_level`` 'mininal' or to default to ``route_optimization_level``.
            If the 'mininal' compiled plan is cached, then it compiles once more using the default optimization level during the first execution.
