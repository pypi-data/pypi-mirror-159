...
class DataBeaver(Object):
    ...
    def build(self, model=None):
        ...
        for model_file_name in model_files:
            ...
            file_name = model_file_name[model_file_name.rfind('/'):]
            ...
            if file_name.count('.') == 3:
                ...
                if index_number > 1:
                    ...
                    self._logger.info(f"{previous_sql_name} is a depedency for {model_file_name}")
        ...
        for name, data in file_info.items():
            self._logger.info(f"{name}")
            self._logger.info(f"{data['dependencies']} - {data['status']}")
