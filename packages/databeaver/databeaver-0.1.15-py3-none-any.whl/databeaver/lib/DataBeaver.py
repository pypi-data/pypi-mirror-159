...
class DataBeaver(Object):
    ...
    def _parse_sql(self, model_info, file_info, schema):
        ...
        for file_name, file in file_info.items():
            ...
            self._logger.info(f"Processing SQL for {file_name} ({model_name})")
    ...
    def build(self, model=None):
        ...
        for model_file_name in model_files:
            start_pos = model_file_name.rfind('/')
            model_name = model_file_name[start_pos + 1:]
            model_name = model_name[:model_name.find('.')]
            file_name = model_file_name[start_pos:]
            ...
            if file_name.count('.') == 3:
                ...
                if index_number > 1:
                    ...
                    # self._logger.info(f"{previous_sql_name} is a dependency for {model_file_name}")
        ...
        for name, data in model_info.items():
            self._logger.info(f"{name} - {data}")

        for name, data in file_info.items():
            self._logger.info(f"{name} - {data['dependencies']} - {data['status']}")
