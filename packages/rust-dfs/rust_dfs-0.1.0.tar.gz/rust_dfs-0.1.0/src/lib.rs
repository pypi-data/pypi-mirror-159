...
use primitive::{InputSet, InputType};
use rayon::prelude::*;

use feature::{generate_fake_features as r_generate_fake_features, Feature};
use pyo3::prelude::*;

use crate::{dfs::generate_features_for_primitives, primitive::Primitive};
...
fn generate_features(features: Vec<Feature>, primitives: Vec<Primitive>) -> PyResult<Vec<Feature>> {
    ...
    let new_features: Vec<Feature> = t1_primitives
        .par_iter()
        // .iter()
        .map(|&p| generate_features_for_primitives(p, features_ref))
        .flatten()
        .collect();
