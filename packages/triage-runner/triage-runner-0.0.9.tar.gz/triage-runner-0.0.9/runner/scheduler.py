...
class GPUWatcher:
    ...
    async def _run_task(self, task: Task) -> Task:
        ...
        running_task = await asyncio.create_subprocess_shell(task.cmd, stdout=redirect, stderr=redirect, loop=loop)
        task = set_exit_code(task, running_task.returncode)
    ...
    def get_finished(self) -> Tuple[Task, ...]:
        ...
        done, pending = asyncio.run(asyncio.wait(self._running_tasks, timeout=0, loop=loop))
