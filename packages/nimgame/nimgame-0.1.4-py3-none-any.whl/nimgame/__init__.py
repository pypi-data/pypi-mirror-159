...
modules in "source/playing"
    Functions to play the game. E.g. demo.py can be run to do interactions for
    executing automatic runs or interactive games. The same is executed when
    
    -   the package itself is called (see the "__main__.py" module), e.g.
        :command:`python -m nimgame`
    -   the :command:`nimgame.exe` (in Scripts) is executed. NB, the python
        Scripts dir is usually in the execution path, i.e. the nimgame.exe should
        be available from anywhere in the CLI prompt.
modules in "tests"
