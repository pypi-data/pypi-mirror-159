...
class Nim(calculations.Mixin):
    ...
    This class publishes the following methods. See also the calculation methods from the mixin
    in :mod:`source.calculations`.
    ...
    **Instance Attributes:**
    ...
    def __post_init__(self):
        """Validate all input parameters for the instantiation, being paranoid
        
        This is rather an excercise for playing with a validator package.
        ...
        #validate the heap range ends; less than 1 heap makes no any sense; 26 heaps are from A to Z, no more to allow
        v8.validate(
    ...
    def setup_heaps(self, 
        ...
        Args:
            heapcounts: The list elements state the number of coins in each
                ...
                created with random number of coins.
        """
    ...
    def set_start(self, 
        ...
        "overwriting" it, as the default remains the 'a' for subsequent calls
        and also the outer variable, where the argument comes from, remains
        unaffected.
        ...
        The attribute `nextturn` is an :obj:`iterator.__next_`. Calling it
        infinitely yields the swapped player names. This attribute is also
        used in :meth:`do_move`.
        
        Args:
            myturn: The logical value from the Player's point of view.
            
                - 'a' (by default): the Computer figures out which party is to start
                  the game, based on the heap counts and the required error rate
                - truthy or 'p': Player
                - falsy or 'c': Computer
                - 'r': random
        """
        ...
        #reset the "nextturn" cyclic iterator
    ...
    def get_heapstatus(self) -> str:
        ...
        if self.activeplayer == 'start':
            ...
            status = ' '.join([f' {chr(65+h)}' for h in range(len(self.heaps))]) + '\n'
