...
class AbstractMesh:
    def __init__(self, type, node_num, face_num, cell_num):
        ...
        self.cell_volume = np.zeros([self.cell_num], dtype=np.float64)
        self.cell_face = np.zeros([self.cell_num, self.face_per_cell], dtype=np.int32)
        self.cell_coord = np.zeros([self.cell_num, self.dim], dtype=np.float64)
        self.faces = np.zeros([self.face_num, 2], dtype=np.int32)
        self.face_cell = np.full([self.face_num, 2], fill_value=-1, dtype=np.int32)
        self.face_center = np.zeros([self.face_num, self.dim], dtype=np.float64)
        self.face_length = np.zeros([self.face_num], dtype=np.float64)
        self.face_markers = np.zeros([self.face_num], dtype=np.int32)
        self.face_norm = np.zeros([self.face_num, self.dim], dtype=np.float64)
        self.nodes = np.zeros([self.node_num, self.dim], dtype=np.float64)
    ...
    def _cal_face_markers(self):
        print("[Error] Undefine cal face markers function.")
        exit(7)
