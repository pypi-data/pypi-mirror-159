...
def find_delta(optiontype: str, K, T, r, sigma):
    if optiontype == "C":
...
def find_premium(optiontype: str, K, T, r, sigma):
    if optiontype == "C":
...
def find_strike(optiontype: str, S, T, r, sigma, d):
    if optiontype == "C":
        return find_strike_call(S, T, r, sigma, d)
    elif optiontype == "P":
        return find_strike_put(S, T, r, sigma, d)
    else:
        raise ValueError("Option type must be either C or P")
...
class Curve:
    ...
    def getIvFromStrike(self, strike):
        ...
        print(self.strikes, self.markIvs)
        if lowestStrike == 0:
            return self.markIvs[idx]
