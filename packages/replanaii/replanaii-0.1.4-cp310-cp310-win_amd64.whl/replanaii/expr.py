...
class Expr:
    ...
    def __add__(self, other):
        if type(other) == Expr:
            ...
            idx = self.arena.add_f64(self.idx, float(other))
            return Expr(idx, self.arena)
    ...
    def __sub__(self, other):
        if type(other) == Expr:
            ...
            idx = self.arena.sub_f64(self.idx, float(other))
            return Expr(idx, self.arena)
    ...
    def __mul__(self, other):
        if type(other) == Expr:
            ...
            if float(other) == 1.0:
                ...
                idx = self.arena.mul_f64(self.idx, float(other))
                return Expr(idx, self.arena)
    ...
    def __truediv__(self, other):
        if type(other) == Expr:
            ...
            idx = self.arena.div_f64(self.idx, float(other))
            return Expr(idx, self.arena)

    def __radd__(self, other):
        if type(other) is not Expr:
            idx = self.arena.rev_add_f64(float(other), self.idx)
            return Expr(idx, self.arena)

    def __rsub__(self, other):
        if type(other) is not Expr:
            idx = self.arena.rev_sub_f64(float(other), self.idx)
            return Expr(idx, self.arena)

    def __rmul__(self, other):
        if type(other) is not Expr:
            idx = self.arena.rev_mul_f64(float(other), self.idx)
            return Expr(idx, self.arena)

    def __rtruediv__(self, other):
        if type(other) is not Expr:
            idx = self.arena.rev_div_f64(float(other), self.idx)
            return Expr(idx, self.arena)
