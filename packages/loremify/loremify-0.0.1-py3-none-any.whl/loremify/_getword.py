...
class WordFactory:
  ...
  def _loadWords(self):
    ...
    commonPath = os.path.join(os.path.dirname(__file__), 'common_words.txt')
    wordPath = os.path.join(os.path.dirname(__file__), 'words.txt')
    if self.words is None and self.commonWords is None:
      with open(commonPath, 'r') as f:
        words = f.readlines()
        self.commonWords = [self.noSpace(word) for word in words if word]
      with open(wordPath, 'r') as f:
        words = f.readlines()
        self.words = [self.noSpace(word) for word in words if word]

  def __call__(self, n: int = None, f: int = None) -> list[str]:
