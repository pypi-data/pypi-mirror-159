...
from cp_common.req import Requests as ua_req
    ...
    def run(self, url):
        ...
        try:
            ...
            for path, fingers in path_finger.items():
                ...
                if path == '/':
                    self.title = path_hot_finger.title
                    self.headers = path_hot_finger.headers
                    self.status_code = path_hot_finger.status_code
...
def async_run(urls: list) -> list:
    with ThreadPoolExecutor(max_workers=5) as executor:
        ...
        for future in as_completed(future_to_url):
            ...
            logger.info(future.result().title)
