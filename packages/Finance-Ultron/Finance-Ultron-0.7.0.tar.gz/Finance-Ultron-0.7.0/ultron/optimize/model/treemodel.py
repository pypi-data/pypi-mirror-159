...
class VotingRegressor(create_model_base('sklearn')):
    ...
    def __init__(self, 
                ...
                **kwargs):
        super().__init__(features=features, fit_target=fit_target)
        self.impl = VotingRegressorImpl(estimators=estimators,
                                        voting=voting,
                                        **kwargs)
    @property
    def importances(self):
...
class StackingClassifier(create_model_base('mlxtend')):
    def __init__(self, classifiers=[], meta_classifier=None, features=None,
                fit_target=None,
                **kwargs):
        super().__init__(features=features, fit_target=fit_target)
    ...
    def fit(self, x: pd.DataFrame, y: np.ndarray):
        if self.eval_sample:
            x_train, x_eval, y_train, y_eval = train_test_split(
                x[self.features].values, y, test_size=self.eval_sample,
                random_state=42)
