...
def main():
    ...
    def runthesim(processmodel,tune):
        ...
        if float(modeldt.get())+float(modeltc.get())*5 < minsize:
            rangesize = minsize
        elif float(modeldt.get())+float(modeltc.get())*5 >maxsize:
            rangesize = maxsize
        else:
    ...
    def tune(selector="A"):
        try:
            ...
            if selector=="A":            
                tuneA_button['state']='disabled'
                tuneB_button['state']='normal'
                tuner.calc(model)
            elif selector=='B':            
                tuneA_button['state']='normal'
                tuneB_button['state']='disabled'
                tuner.calcFullFat(model)       

            if selector == 'I' or ((selector == 'A' or selector == 'B') and str(dep_button['state']) =='normal'):
    ...
    tuneA_button = ttk.Button(root,text='Conservative',command=lambda :[tune("A")])
    tuneA_button.grid(row=6,column=0,padx=5,pady=4,sticky="NESW")
    tuneA_button['state']='disabled'

    tuneB_button = ttk.Button(root,text='Aggressive',command=lambda :[tune("B")])
    tuneB_button.grid(row=7,column=0,padx=5,pady=4,sticky="NESW")
