...
class Command(BaseCommand):
    ...
    def generate_sequential_id(self, store):
        tz = pytz.timezone(settings.TIME_ZONE)
        payment_recvd = store.payment_received.astimezone(tz)

        count = (
            CheckoutStore.objects.filter(status__in=[30, 31])
            .exclude(id=store.id)
            .filter(payment_received__lt=payment_recvd)
            .filter(payment_received__year=payment_recvd.year)
            .filter(payment_received__month=payment_recvd.month)
        )
        if settings.SALEBOX["CHECKOUT"]["SEQUENTIAL_TRANSACTION_ID"]["RATE"] == "daily":
            count = count.filter(payment_received__day=payment_recvd.day)

        # fetch format
        prefix = settings.SALEBOX["CHECKOUT"]["SEQUENTIAL_TRANSACTION_ID"]["PREFIX"]
        digits = settings.SALEBOX["CHECKOUT"]["SEQUENTIAL_TRANSACTION_ID"]["DIGITS"]

        # replace characters in prefix
        prefix = prefix.replace("{yyyy}", str(payment_recvd.year))
        prefix = prefix.replace("{yy}", str(payment_recvd.year)[-2:])
        prefix = prefix.replace("{mm}", ("0" + str(payment_recvd.month))[-2:])
        prefix = prefix.replace("{dd}", ("0" + str(payment_recvd.day))[-2:])

        count = count.count() + 1
        return f"{prefix}{count:0{digits}d}"
        ...
        data["transaction"] = json.dumps(
            {
                ...
                "messages": store.data.get("messages", {}),
    ...
    def transaction_meta(self, store):
        ...
        return {
            ...
            "sequential_id": self.generate_sequential_id(store),
    ...
    def transaction_total(self, store):
        ...
        # do we apply vat on shipping?
        if settings.SALEBOX["MISC"]["VAT_INCLUDE_SHIPPING"]:
            # gross_total += store.data["shipping_method"]["price"]
            gross_vat_applicable += store.data["shipping_method"]["price"]
