...
class Command(BaseCommand):
    ...
    def generate_sequential_id(self):
        ...
        count = (
            ...
            .filter(payment_received__lt=now)
            .filter(payment_received__year=now.year)
            .filter(payment_received__month=now.month)
        )
        if settings.SALEBOX["CHECKOUT"]["SEQUENTIAL_TRANSACTION_ID"]["RATE"] == "daily":
            count = count.filter(payment_received__day=now.day)
        ...
        prefix = prefix.replace("{yyyy}", str(now.year))
        prefix = prefix.replace("{yy}", str(now.year)[-2:])
        prefix = prefix.replace("{mm}", ("0" + str(now.month))[-2:])
        prefix = prefix.replace("{dd}", ("0" + str(now.day))[-2:])
    ...
    def transaction_meta(self, store):
        ...
        return {
            ...
            "sequential_id": self.generate_sequential_id(),
