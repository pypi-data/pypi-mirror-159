...
class Topic:
    ...
    type: Type[AbstractDataType] = attr.ib(on_setattr=attr.setters.frozen, converter=dt.get)
    ...
    def parse_delta(cls, delta: bytes) -> Iterator[Union[bytes, slice]]:
        ...
        match = None
        while offset < length:
            chunk = cls.encoder.loads(delta[offset:])
            if isinstance(chunk, bytes):
                ...
                match = None
            offset += len(cls.encoder.dumps(chunk))
