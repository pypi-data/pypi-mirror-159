...
class SessionMetricCollector(MetricCollector):
    ...
    removes_metrics_with_no_matches: typing.Optional[bool] = False
    """
    Returns: `True` if metrics with no matches should be removed.
    """
    ...
    group_by_properties: typing.List[pydantic.StrictStr] = pydantic.Field(
        default=[], serializer="session-property-keys"
    )
    """
    Returns: list of properties to group by
    """
...
class SessionMetricCollectorBuilder(BuilderBase[SessionMetricCollector]):
    ...
    def create(self, name: str, session_filter: str) -> SessionMetricCollector:
        """
        Create a new `SessionMetricCollector` using the values
        currently known to this builder.

        Args:
            name: the name of the `SessionMetricCollector`
        ...
        Returns: a new `SessionMetricCollector` with all of the
