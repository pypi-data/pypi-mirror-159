...
DEFAULT_SSH_CONFIG_FILE = os.path.join(__location__, "ssh_config")
...
@click.pass_context
def create_ssh_config(ctx):
    system_os = platform.system()
    execute_shell_command = "sh -c"
    if system_os == "Windows":
        execute_shell_command = (
            "C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe"
        )

    with open(DEFAULT_SSH_CONFIG_FILE, "w") as file:
        file.write(
            (
                "Host i-* mi-*\n"
                f'    ProxyCommand {execute_shell_command} "aws ssm start-session --target {ctx.obj["ssh_instance_id"]} --document AWS-StartSSHSession --parameters portNumber=22 --region={ctx.obj["aws_region"]} --profile {ctx.obj["aws_profile"]}"'
            )
        )
