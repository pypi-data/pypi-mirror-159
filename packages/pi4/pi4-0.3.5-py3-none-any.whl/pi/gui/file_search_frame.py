...
UPLOAD_URL = 'http://zhangys.org.cn/home/upload'
...
class FileSearchFrame(ttk.Frame):
    ...
    def upload_image(self):
        ...
        if curItem:          
            ...
            wapp = ttk.Toplevel(title='Uploading...', size=(300,50))
            wframe = WaitingFrame(wapp)
            wframe.task_to_run = lambda: self.send_data_to_server(wframe, imgpath, {'IP':IP_addres, 'MAC':mac, 'operator':'zys','timestamp': datetime.now()})
            wframe.start_task()

    def send_data_to_server(self, frame, image_path, metadata):
        ...
        try: # to handle network down exception, etc.
            r = requests.post(UPLOAD_URL, 
            files=multipart_form_data, data=metadata)
            title = 'HTTP Status Code ' + str(r.status_code)
            message = r.text
            # frame.message.set( + '\n' + r.text)
        ...
        if sys.platform == 'win32': # in linux, toast in sub thread is buggy
            toast = ToastNotification(
                title=title,
                message=message, 
                duration=2000,
            ...
            toast.show_toast()
            # Messagebox.show_info(message, title) # cannot do this in a sub thread
