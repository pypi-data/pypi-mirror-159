...
if __package__:
    ...
    from .emo import EmotionClassifier
else:
...
class MainApp(ttk.Frame):
    ...
    def create_buttonbox(self, container = None):
        ...
        btn = ttk.Button(
            master=container,
            text = 'CAPTURE',# Emoji.get('CAMERA WITH FLASH'),
            padding=PADDING,
        ...
        btn = ttk.Button(
            master=container,
            text='BROWSE',#Emoji.get('open file folder'),
            # image='folder',
            padding=PADDING,
        ...
        btn = ttk.Button(
            master=container,
            text='THEME',#Emoji.get('LOWER LEFT PAINTBRUSH'),
            padding=PADDING,
        ...
        btn = ttk.Button(
            master=container,
            text='EXIT',#Emoji.get('NEGATIVE SQUARED CROSS MARK'),
            padding=PADDING,
    ...
    def get_image(self):
        ...
        if self.task.get() == 'emotion':
            if 'emotion' not in self.__dict__ or self.emotion is None:
                self.emotion = EmotionClassifier()
            input_img = self.emotion.detect_emotion(input_img)
