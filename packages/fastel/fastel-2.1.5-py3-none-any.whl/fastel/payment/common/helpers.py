...
from fastel.utils import TW, timestamp_postfix
    ...
    def parse_order(self, order: Order) -> IssueModel:
        ...
        for extra_item in order.extra_items:
            ...
            if order.invoice_type == InvoiceTypes.B2B:
                _amount += "|" + str(extra_item.get("sale_amount", 0))
                _price += "|" + str(extra_item.get("unit_sales", 0))
            else:
                _amount += "|" + str(extra_item.get("amount", 0))
                _price += "|" + str(extra_item.get("price", 0))
        for discount_item in order.extra_items:
            item_name += "|" + discount_item.get("name", "")
            item_count += "|1"
            item_unit += "|個"
            if order.invoice_type == InvoiceTypes.B2B:
                _amount += "|-" + str(discount_item.get("sale_amount", 0))
                _price += "|-" + str(discount_item.get("unit_sales", 0))
            else:
                _amount += "|-" + str(discount_item.get("amount", 0))
                _price += "|-" + str(discount_item.get("price", 0))
        if order.invoice_type == InvoiceTypes.B2B:
            item_amount = "|".join(
