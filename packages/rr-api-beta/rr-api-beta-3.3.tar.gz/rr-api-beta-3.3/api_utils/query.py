...
class Players:
    ...
    @staticmethod
    def get_players_by_ids(players):
        return f"select distinct rp.wyscout_id as id, rp.rankset_name as name, rp.rankset_id from rankset.players as rp" \
...
class Ranking:
    ...
    def players_ranking(data_type: int, object_id: int = None):
        LIMIT = ' and rpr.current_season_rank between 10 and 116 '
        match data_type:
            case 1:
                return "select distinct rp.wyscout_id, rp.rankset_id as player_id, rp.rankset_name as player_name," \
                       " cast(avg(rpr.current_season_rank) as decimal (10, 0)) as rank, " \
                       "cast(avg(rpr.retro_rank) as decimal (10, 2)) as retro_avg_rank, cast(avg(rpr.minutes_played) as decimal (10, 2)) as by_avg_minutes " \
                       "from rankset.players as rp, rankset.player_rank as rpr where rpr.player_id = rp.rankset_id " \
                       f"{LIMIT} group by rp.rankset_id, rp.rankset_name order by rank desc;"
            case 2:
                return "select distinct rp.rankset_id as player_id, rp.rankset_name as player_name," \
                       "cast(avg(rpr.current_season_rank) as decimal (10, 0)) as rank, " \
                       "cast(avg(rpr.retro_rank) as decimal (10, 2)) as retro_avg_rank, " \
                       "cast(avg(rpr.minutes_played) as decimal (10, 2)) as by_avg_minutes " \
                       "from rankset.players as rp, rankset.teams as rt, rankset.player_rank as rpr " \
                       f"where rt.rankset_id = rp.main_id and rpr.player_id = rp.rankset_id and rc.wyscout_id = {object_id} " \
                       f"{LIMIT} group by rp.rankset_id, rp.rankset_name order by rank desc;"
            ...
            case 3:
                return "select distinct rp.rankset_id as player_id, rp.rankset_name as player_name," \
                       ...
                       "cast(avg(rpr.retro_rank) as decimal (10, 2)) as retro_avg_rank, " \
                       "cast(avg(rpr.minutes_played) as decimal (10, 2)) as by_avg_minutes " \
                       "from rankset.players as rp, rankset.teams as rt, " \
                       "rankset.countries as rc, rankset.player_rank as rpr where rt.rankset_id = rp.main_id " \
                       f"and rc.rankset_id = rt.main_id and rpr.player_id = rp.rankset_id and rc.wyscout_id = {object_id} " \
                       f"{LIMIT} group by rp.rankset_id, rp.rankset_name order by rank desc;"

    @staticmethod
    def player_rank(player_id: int):
        return f"select distinct rp.rankset_id, rp.rankset_name, cast(avg(rpr.current_season_rank) as decimal (10, 2)) as avg_rank, " \
               "cast(avg(rpr.retro_rank) as decimal (10, 2)) as retro_avg_rank, cast(avg(rpr.minutes_played) as decimal (10, 2)) as by_avg_minutes " \
    ...
    def player_rank_info(player_id: int):
        return "select distinct rp.rankset_id, rp.rankset_name, rpr.retro_rank, rpr.current_season_rank, rpr.minutes_played, rpr.tour_id, rpr.season " \
               f"from rankset.players as rp, rankset.player_rank as rpr where rpr.player_id = {player_id} " \
