...
class DinningTableSetup(BaseNModel,DictModel):
    ...
    def generate_weblink_details_list(self, outlet_key, weblink_url_format):
        table_list_in_list = self.table_list.split(',')
        table_weblink_details_list = []
        for table in table_list_in_list:
            web_link = weblink_url_format.format(outlet_key=outlet_key, table=table)
...
class DinningOption(BaseNModel,DictModel):
    ...
    dict_properties = ['name','prefix', 'is_dinning_input', 'is_archive', 'is_default', 'dinning_table_is_required', 'assign_queue']
    
    @staticmethod
    def create(name, prefix, merchant_acct, is_dinning_input=False, is_default=False, dinning_table_is_required=False, assign_queue=False):
    ...
    @staticmethod
    def update(dinning_option, name, prefix, is_dinning_input=False, is_default=False, dinning_table_is_required=False, assign_queue=False):
