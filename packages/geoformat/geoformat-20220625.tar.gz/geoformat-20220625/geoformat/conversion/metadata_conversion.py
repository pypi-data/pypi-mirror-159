...
def merge_geolayer_metadata(geolayer_a, geolayer_b, geolayer_name, rename_geolayer_field='auto'):

    metadata = copy.deepcopy(geolayer_a['metadata'])
    metadata["name"] = geolayer_name
    ...
    for key, value in geolayer_b['metadata'].items():
        if key in {'fields', 'geometry_ref'} and key not in metadata:
            metadata[key] = copy.deepcopy(value)
        else:
            if key == 'fields':
                for geolayer_b_field_name, geolayer_b_field_metadata in geolayer_b['metadata'][key].items():
                    if metadata['fields'].get(geolayer_b_field_name):
                        if geolayer_b_name not in rename_field_correspondance:
                            rename_field_correspondance[geolayer_b_name] = {}
                        if rename_geolayer_field == 'auto':
                            exists = True
                            it = 1
                            new_field_name = geolayer_b_field_name
                            while exists is True:
                                new_field_name = f'{new_field_name}{it}'
                                if new_field_name in metadata['fields']:
                                    it += 1
                                else:
                                    exists = False
                        else:
                            new_field_name = rename_geolayer_field[geolayer_b['metadata']['name']].get(geolayer_b_field_name)
                            if new_field_name is None:
                                raise Exception(f'{geolayer_b_field_name} still exists in geolayer')
                        rename_field_correspondance[geolayer_b_name][geolayer_b_field_name] = new_field_name
                    else:
                        new_field_name = geolayer_b_field_name

                    metadata['fields'][new_field_name] = dict(geolayer_b_field_metadata)
                    # reorder index
                    metadata['fields'][new_field_name]['index'] = len(metadata['fields']) - 1
            elif key == 'geometry_ref':
                metadata['geometry_ref']['type'].update(geolayer_b['geometry_ref']['type'])
                metadata['geometry_ref']['crs'].update(geolayer_b['geometry_ref'].get('crs'))
