...
from geoformat.conf.error_messages import missing_field, python_inferior_to_3_7_forbidden
...
def rename_field(geolayer, old_field_name, new_field_name):
    ...
    """
    if geolayer_fields_metadata := geolayer.get('metadata', {}).get('fields', {}):
        if old_field_name in geolayer_fields_metadata:
            # rename in geolayer metadata
            geolayer_fields_metadata[new_field_name] = dict(geolayer_fields_metadata[old_field_name])
            ...
            raise Exception(missing_field.format(missing_field=old_field_name))
