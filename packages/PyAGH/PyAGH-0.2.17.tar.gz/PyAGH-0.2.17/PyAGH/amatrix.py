...
def makeA(data_ord):
    ...
    loc=loc[loc["y"] != N]
    Tinv = sparse.coo_matrix((loc["data"], (loc["y"], loc["x"])), shape=(N, N), dtype=np.float64).tocsc()
    di =  sparse.coo_matrix((np.sqrt(1/np.array(dii_c)), (np.array(range(N)), np.array(range(N)))), shape=(N,N), dtype=np.float64).tocsc()
    X = Tinv.dot(di) ##upper

    #Ip=np.identity(N)

    #tu = spsolve_triangular(X,Ip,False)
    tu = sparse.linalg.inv(X)
    A = (tu.T).dot(tu)
