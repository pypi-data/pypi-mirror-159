...
def makeA(data_ord):
    ...
    X = sparse.csr_matrix(Tinv.dot(di),dtype=np.float64)
    print("X")
    Ip = sparse.identity(N,dtype='float64', format='dia').toarray() ## 最快的方法 7.7秒3W int8
    print("ip")
    tu = spsolve_triangular(X,Ip,False,overwrite_A=True, overwrite_b=True)
    tu = sparse.csr_matrix(tu,dtype=np.float64)
    print("tu")
    del Ip, X
    gc.collect()
    ...
    A = tu.T.dot(tu)
    return [A.astype(np.float32), data_ord.iloc[:,0]]