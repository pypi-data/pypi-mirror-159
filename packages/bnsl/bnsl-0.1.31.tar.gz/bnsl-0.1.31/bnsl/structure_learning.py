...
def hc(data, method='standard', score_function='default', debug=False, alpha=0.3):
    '''
                    ...
                    for T in all_subsets(ncnav):
                        ...
                        if validity_test(cpdag, x, y, T, 'insert'):
                            ...
                            if parents_new not in cache[y]:
                                ...
                                cache[y][parents_new] = local_score(data, cols, score_function, alpha=alpha)
                            score_new = cache[y][parents_new]
                            if parents_cur not in cache[y]:
                                cols = [varnames.index(v) for v in (y,) + parents_cur]
                                cache[y][parents_cur] = local_score(data, cols, score_function, alpha=alpha)
