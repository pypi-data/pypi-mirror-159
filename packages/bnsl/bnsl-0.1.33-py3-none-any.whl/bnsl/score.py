...
def ll(data, cols, weight=None):
    arities = np.amax(data, axis=0) + 1
    data_local = np.delete(data, data[:, cols].min(axis=1) < 0, axis=0)
    return ll_counter(data_local, arities, cols, weight) / data_local.shape[0]


@njit(fastmath=True)
def ll_counter(data, arities, cols, weight=None):
    if weight is None:
        weight = np.ones(data.shape[0])
    strides = np.empty(len(cols), dtype=np.uint32)
    idx = len(cols) - 1
    ...
    for i in range(stride):
        if N_ijk[i] != 0:
            ll += N_ijk[i] * np.log(N_ijk[i] / N_ij[i])
    return ll
...
def nal(data, cols, weight=None):
    ...
    if data_local.shape[0] == 0:
        ...
        if weight is None:
            # weight = np.ones(data_local.shape[0]) * data.shape[0] / data_local.shape[0]
            # alpha = 0.3 + data_local.shape[0] / data.shape[0] / 10
            alpha = 0.4
        else:
            # alpha = 0.2 + data_local.shape[0] / sum(weight) / 10
            alpha = 0.3 + data_local.shape[0] / sum(weight) / 10
        return nal_counter(data_local, arities, cols, weight, alpha)


def nal_va(data, cols, weight=None):
    arities = np.amax(data, axis=0) + 1
    data_local = np.delete(data, data[:, cols].min(axis=1) < 0, axis=0)
    if data_local.shape[0] == 0:
        return -np.inf
