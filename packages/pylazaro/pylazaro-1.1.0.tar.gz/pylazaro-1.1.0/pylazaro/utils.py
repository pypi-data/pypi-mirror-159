...
def fuse_spans(output_tokens: List[Token]) -> List[Borrowing]:
    ...
    for i, token in enumerate(output_tokens):
        if token.has_outside_label():
            if half_boiled_label:
                bor = Borrowing(half_boiled_token, half_boiled_label, start_pos, i, output_tokens)
                new_output.append(bor)
                half_boiled_label = None
                half_boiled_token = []
            start_pos = i
            #new_output.append((token, label))
        if token.has_begin_label():
            if half_boiled_label:
                bor = Borrowing(half_boiled_token, half_boiled_label, start_pos, i, output_tokens)
                new_output.append(bor)
            ...
            start_pos = i
        if token.has_inside_label():
            half_boiled_label = token.lang_label
            if not half_boiled_token:
                start_pos = i
            half_boiled_token.append(token)
    if half_boiled_label:
        bor = Borrowing(half_boiled_token, half_boiled_label, start_pos, len(output_tokens), output_tokens)
        new_output.append(bor)
    return new_output
