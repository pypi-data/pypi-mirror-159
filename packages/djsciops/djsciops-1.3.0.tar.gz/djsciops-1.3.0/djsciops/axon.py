    ...
    if (
        ...
        file_paths = {
            ...
            if (fp.is_file()
                ...
                and re.match(permit_regex, (relative_fp := pathlib.PurePosixPath(fp.relative_to(source))).as_posix())
                and not re.match(ignore_regex, relative_fp.as_posix()))
        }
        file_paths = (
            file_paths
            or set([pathlib.PurePosixPath(source.name)]
                   if re.match(permit_regex, source.name) and not re.match(ignore_regex, source.name) else [])
...
def _generate_download_mapping(
    ...
    if (
        ...
        object_paths = {
            ...
            if (re.match(permit_regex, (relative_op := pathlib.PurePosixPath(op.key).relative_to(object_root)).as_posix())
...
def list_files(
    ...
    objects = [
        dict(key=op.key, _size=op.size,
             _contents_hash=op.Object().metadata['contents_hash'] if include_contents_hash else None)
        for op in session.s3.Bucket(s3_bucket).objects.filter(Prefix=s3_prefix)
        if (re.match(permit_regex, op.key.replace(s3_prefix, ''))
...
def upload_files(
    ...
    if not file_paths or not object_paths:
        raise Exception("All upload-permitted files already exist in object store.")
...
def download_files(
    ...
    if not file_paths or not object_paths:
        raise Exception("All download-permitted files already exist locally.")
