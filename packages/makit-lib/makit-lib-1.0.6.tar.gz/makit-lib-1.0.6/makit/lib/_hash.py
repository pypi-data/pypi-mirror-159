...
from makit.lib.encoding import convert_bytes
...
class PBKDF2PasswordHandler(PasswordHandler):
    ...
    def pbkdf2(cls, password, salt, iterations, dklen=0, digest=None):
        ...
        if digest is None:
            digest = hashlib.sha256
        dklen = dklen or None
        password = convert_bytes(password)
        salt = convert_bytes(salt)
    ...
    def verify(self, password, encoded):
        ...
        return secrets.compare_digest(convert_bytes(encoded), convert_bytes(encoded2))
