    ...
    if len(x[i].shape) == 2:
        if len(corr_x[i]) == len(u_x[i]):
            ...
            for j in range(len(u_x[i][0])):
                ...
                MC_data[:,j,:] = generate_sample_cov(MCsteps,x[i][:,j].flatten(),
                    cov_x,dtype=dtype).reshape(x[i][:,j].shape+(MCsteps,))
        else:
            MC_data = np.zeros((u_x[i].shape)+(MCsteps,))
            for j in range(len(u_x[i][:,0])):
                cov_x = cm.convert_corr_to_cov(corr_x[i],u_x[i][j])
        ...
        MC_data = generate_sample_cov(MCsteps,x[i].flatten(),cov_x,dtype=dtype).reshape(
            x[i].shape+(MCsteps,))
...
def generate_sample_random(MCsteps,param,u_param,dtype=None):
    ...
    if not hasattr(param,"__len__"):
        ...
        return (np.random.normal(size=(len(param),MCsteps)).astype(
            dtype)*u_param[:,None]+param[:,None])
    elif len(param.shape) == 2:
        return (np.random.normal(size=param.shape+(MCsteps,))*u_param[:,:,
                                                                   None]+param[:,:,None])
    elif len(param.shape) == 3:
        return (np.random.normal(size=param.shape+(MCsteps,)).astype(
            dtype)*u_param[:,:,:,None]+param[:,:,:,None])
    elif len(param.shape) == 4:
        return (np.random.normal(size=param.shape+(MCsteps,)).astype(
            dtype)*u_param[:,:,:,:,None]+param[:,:,:,:,None])
    else:
        raise ValueError("punpy.mc_propagation: parameter shape not supported: %s %s"%(
        param.shape,param))
...
def generate_sample_systematic(MCsteps,param,u_param,dtype=None):
    ...
    if not hasattr(param,"__len__"):
        ...
        return (np.dot(u_param[:,None],
            np.random.normal(size=MCsteps).astype(dtype)[None,:],)+param[:,
                                                                             None])
    elif len(param.shape) == 2:
        return (np.dot(u_param[:,:,None],
...
def generate_sample_cov(MCsteps,param,cov_param,dtype=None):
    ...
    if dtype is None:
        ...
        return (np.dot(L,np.random.normal(size=(len(param),MCsteps)).astype(dtype))+param[:,None])
...
def correlate_sample_corr(sample,corr,dtype=None):
    ...
    if np.max(corr) > 1.000001 or len(corr) != len(sample):
        ...
        for j in np.ndindex(sample[0][...,0].shape):
            sample_j = np.array([sample[i][j] for i in range(len(sample))],
                dtype=dtype)
            ...
            means = np.array([np.mean(sample[i][j]) for i in range(len(sample))],
                dtype=dtype,)[:,None]
            stds = np.array([np.std(sample[i][j]) for i in range(len(sample))],
                dtype=dtype,)[:,None]
            ...
            # If any of the variables has no uncertainty, the normalisation will fail. Instead we leave the parameters without uncertainty unchanged.
                ...
                sample_j[id_nonzero] = (np.dot(L[id_nonzero][:,id_nonzero],
                    (sample_j[id_nonzero]-means[id_nonzero])/stds[id_nonzero],)*stds[
                                             id_nonzero]+means[id_nonzero])
