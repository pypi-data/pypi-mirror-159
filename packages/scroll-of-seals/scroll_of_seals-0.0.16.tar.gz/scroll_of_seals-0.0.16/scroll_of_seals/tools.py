...
def display_video(video_path, video_width = 600):
    """ 
    Function to show videos in colab. Can receive
    videos in '.mp4' and '.avi' formats.
    ...
    video_path : str
        The video location in drive or HDD
    video_width: int
        The width to diplay the video into your colab.

    Example
    ----------

    video_path = '/content/video1.mp4'
    display_video(path,init,end,save)
    ...
    assert isinstance(video_path,type('a')), 'The input must be the video path'

    if video_path[-4:].lower() == '.mp4':
    ...
    elif video_path[-4:].lower() == '.avi':
        
        clip=VideoFileClip(video_path)

        return clip.ipython_display(width=video_width)
    
def find_file(file_path, key, endswith = True):
    """ 
    Function to find files in a folder
    ...
    endswith : bool
        if True: the key must be the final term of the file.
        if False: the key must be the first terms of the file.

    Returns
    ----------
    listaPath : list
        List with the path of the files inside the directory.

    Example
    ----------
    
    1)
    file_path = '/content/folder_videos'
    find_file(file_path,'.mp4',endswith = True)

    2)
    file_path = '/content/folder_videos'
    find_file(file_path,'RAT01',endswith = False)
    ...
    for filename in os.listdir(file_path):
       if endswith:
            if filename.endswith(key): 
                listaPath.append(os.path.join(file_path, filename))
       else:
             if filename.startsswith(key): 
                listaPath.append(os.path.join(file_path, filename))   
...
def smooth(data,factor):
    """ 
    Function to filter and smooth the data to plot
    ...
    factor : int
        ...
        The greater the value, the greater the noise attenuation.
...
def parula_corlormap(): 
    ...
    cm_data = [[0.2422, 0.1504, 0.6603],[0.2444, 0.1534, 0.6728],[0.2464, 0.1569, 0.6847],
