...
def rename_parameter(old_name: str, new_name: str):
    """
    Creates a decorator to mark a parameter as deprecated due to it being renamed. Use this to
    ...
    def decorator(node):
        ...
        # Add deprecated parameter name as a keyword parameter.
        node_kwargs = node.kwargs.copy()
        node_kwargs[old_name] = forge.kwarg(
            old_name, type=Optional[arg_type], default=None
        )
        node.kwargs = node_kwargs

        # Update create_node_data to throw a warning if called with the deprecated parameter name.
        original_create_node_data = node.create_node_data

        def new_create_node_data(*args, **kwargs):
            old_value = kwargs[old_name]
