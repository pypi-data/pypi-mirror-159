...
class User(BaseModel):
    ...
    # TODO: Lazily load the groups for a User, and return actual Group objects that can be introspected
    @property
    def groups(self):
        return [group.split('@')[0] for group in self._data['groups']]

    @groups.setter
    def groups(self, value):
        group_list = value if isinstance(value, list) else [value]
        regex_subbed_list = [re.sub(r'[^a-zA-Z_0-9]', '_', group) for group in group_list]
        self._data['groups'] = ['{}@{}'.format(group, self._data['organization']) for group in regex_subbed_list]
...
class Group(BaseModel):
    ...
    Args:
        ...
        roles (:obj:`dict`): A mapping of role IDs to role labels.
    """
    _ATTRIBUTES_TO_IGNORE = ['deleteTime', 'destroyer', 'groupDeleted']
...
class Environments(CollectionModel):
    ...
    def _get_all_results_from_api(self, id=None, environment_id=None, **kwargs):
        ...
        """
        if id:
            warnings.warn("The 'id' parameter for filtering Environments has been replaced by 'environment_id'. Please"
                          " use 'environment_id', as the 'id' parameter will be removed in a future release.",
                          DeprecationWarning)
            environment_id = environment_id or id
...
class EnvironmentBuilder:
    ...
    def build(self, environment_name, **kwargs):
        ...
        """
        if kwargs.get('environment_type'):
            warnings.warn('The environment_type parameter is no longer required for the EnvironmentBuilder.build()'
...
class DeploymentBuilder:
    ...
    def build(self, deployment_name, engine_type, engine_version, environment, **kwargs):
        ...
        if kwargs.get('deployment_type'):
            warnings.warn('The deployment_type parameter is no longer required for the DeploymentBuilder.build() method'
...
class Deployments(CollectionModel):
    ...
    def _get_all_results_from_api(self, id=None, deployment_type=None, environment_id=None,
        ...
        """
        if id:
            warnings.warn("The 'id' parameter for filtering Deployments has been replaced by 'deployment_id'. Please"
                          " use 'deployment_id', as the 'id' parameter will be removed in a future release.",
                          DeprecationWarning)
            deployment_id = deployment_id or id
