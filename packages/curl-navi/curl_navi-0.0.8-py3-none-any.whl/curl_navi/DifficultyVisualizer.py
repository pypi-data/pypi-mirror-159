...
class DifficultyVisualizer:
        ...
        for color_index,baseline in enumerate(self.trajectory_yaml_list):
            ...
            for path in yaml_data:
                for xy in path:
                    ...
                    if c >= self.map_width or c < 0 : continue
                    if r >= self.map_height or r < 0 : continue
    ...
    def plot_metrics(self):
        load_obstacle_map = np.load(self.yaml_data['source'][:-4]+'_bit.npy')
        shape = load_obstacle_map.shape
        diff = self.DifficultyMetrics(load_obstacle_map, disp_radius=1)
    ...
    class DifficultyMetrics:
      ...
      def __init__(self, map, disp_radius):
        self.map = map
        self.rows = len(map)
        self.cols = len(map[0])
