...
STATES={'NO_LOG':False, 'NO_COMMENT':False, 'NO_SAVE':False, 'NO_PLOT':False}
...
@cachier()
def read_daily(path=None,open=0,close=0,high=0,low=0,tr=0,sharenum=0,volume=0,unadjust=0,start=20130326):
...
def show_corrs(factors: list[pd.DataFrame] = None,factor_names: list[str] = None,print_bool: bool = True,
               show_percent: bool = True) -> pd.DataFrame:
    '''展示很多因子的截面相关性'''
    corrs=[]
...
def read_minute_mat(file:str=None)->list[str,pd.DataFrame]:
    ...
    df=pd.DataFrame(df,columns=['date', 'open', 'high', 'low', 'close', 'amount', 'money'])
    df=df.groupby(['date']).apply(lambda x:x.assign(num=list(range(1,len(x)+1)))).reset_index(drop=True)
    return code,df

class sqlConfig(object):
    def __init__(self,db_name:str=None,db_user='root',db_host='127.0.0.1',db_port=3306,db_password='Kingwila98'):
...
def database_update_minute_files(startdate:str=None,enddate:str=None):
    '''
    ...
    for stock in tqdm.tqdm(stocks,desc='正在下载分钟数据'):
        ...
        df_sql=df.rename(columns={
            'tradetime':'date','openprice':'open','highprice':'high','lowprice':'low','closeprice':'close','volume':'amount','value':'money'})
        df_sql=df_sql[['date', 'open', 'high', 'low', 'close', 'amount', 'money']]
        df_sql=df_sql.groupby(['date']).apply(lambda x:x.assign(num=list(range(1,len(x)+1)))).reset_index(drop=True)
...
def database_update_daily_files(startdate:str=None,enddate:str=None):
    ...
    if enddate:
        ...
        if enddate.hour<17:
            ...
            ...
        enddate=datetime.datetime.strftime(enddate,'%Y%m%d')
        logger.info(f"本次将更新到{datetime.datetime.strftime(pd.Timestamp(enddate),format='%Y-%m-%d')}")
    #交易日历
...
'''读取最终因子值'''
def database_read_final_factors(name:str=None,order:int=None,output=True,new=False)->pd.DataFrame:
    ...
    if output:
        if new:
            if os.path.exists(ans.split('_')[0]):
                fac_name=ans.split('_')[0]+'/'+ans.split('_')[0]+'因子'+final_date+'因子值.csv'
            else:
            ...
            if os.path.exists(ans.split('_')[0]):
                fac_name=ans.split('_')[0]+'/'+ans.split('_')[0]+'因子截至'+final_date+'因子值.csv'
            else:
                os.makedirs(ans.split('_')[0])
                fac_name=ans.split('_')[0]+'/'+ans.split('_')[0]+'因子截至'+final_date+'因子值.csv'
            df.to_csv(fac_name)
...
class pure_mail(object):
    ...
    def sendemail(self,tolist,subject,body,lastemail_path):
        ...
        for path in lastemail_path:
            ...
            att1.add_header('Content-Disposition','attachment',filename=path.split('/')[-1])
            message.attach(att1)
        try:
