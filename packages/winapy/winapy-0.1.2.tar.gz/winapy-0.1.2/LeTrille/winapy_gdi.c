...
static PyObject* PyGetAspectRatioFilterEx(PyObject* _, PyObject* args) {
    HDC hdc;
    SIZE szOut;
    if (!PyArg_ParseTuple(args, "l", &hdc)) return NULL;
    if (RaiseExceptionCheckInvalid(GetAspectRatioFilterEx(hdc, &szOut))) return NULL;
    return Py_BuildValue("(ll)", szOut.cx, szOut.cy);
}
static PyObject* PyGetBitmapBits(PyObject* _, PyObject* args) {
    HBITMAP hbm;
    BOOL bString;
    BITMAP bmp;
    if (!PyArg_ParseTuple(args, "Lp", &hbm, &bString)) return NULL;
    if (RaiseExceptionCheckInvalid(GetObjectW(hbm, sizeof(bmp), &bmp))) return NULL;
    LONG sz = bmp.bmHeight * bmp.bmWidthBytes * bmp.bmPlanes;
    CHAR* bits = (CHAR*)malloc(sz);
    if (bits == NULL) {
        return PyErr_NoMemory();
    }
    DWORD dwBytesWritten = GetBitmapBits(hbm, sz, bits);
    if (RaiseExceptionCheckInvalid(dwBytesWritten == sz)) {
        free(bits);
        return NULL;
    }
    PyObject* res;
    if (bString) {
        res = PyBytes_FromStringAndSize(bits, sz);
    } else {
        res = PyTuple_New(sz);
        for (DWORD i = 0; i < sz; i++) {
            PyTuple_SetItem( res, i, PyLong_FromLong(bits[i]) );
        }
    }
...
static PyObject* PySetBitmapBits(PyObject* _, PyObject* args) {
    HBITMAP hbm;
    PyObject* obData;
    BITMAP bmp;
    if (!PyArg_ParseTuple(args, "LO", &hbm, &obData)) return NULL;
    if (RaiseExceptionCheckInvalid(GetObjectW(hbm, sizeof(bmp), &bmp))) return NULL;
    LONG sz = bmp.bmHeight * bmp.bmWidthBytes * bmp.bmPlanes;
    CHAR* data;
    BOOL isBytes = PyBytes_Check(obData);
    if (!isBytes) {
        if ((!PyTuple_Check(obData)) || (PyTuple_GET_SIZE(obData) < 1)) {
            PyErr_SetString(PyExc_TypeError, "argument 2: must be bytes or tuple object");
            return NULL;
        }
        LONGLONG len = PyTuple_GET_SIZE(obData);
        data = (CHAR*)malloc(len * sizeof(CHAR));
        if (data == NULL) {
            return PyErr_NoMemory();
        }
        for (INT i = 0; i < len; i++) {
            data[i] = (CHAR)PyLong_AsLong(PyTuple_GetItem(obData, i));
        }

    } else {
        data = PyBytes_AsString(obData);
    }

    LONG res = SetBitmapBits(hbm, sz, data);
...
static PyMethodDef module_methods[] = {
    { "SetBitmapBits", PySetBitmapBits, METH_VARARGS },
    { "GetBitmapBits", PyGetBitmapBits, METH_VARARGS },
    { "GetAspectRatioFilterEx", PyGetAspectRatioFilterEx, METH_VARARGS },
