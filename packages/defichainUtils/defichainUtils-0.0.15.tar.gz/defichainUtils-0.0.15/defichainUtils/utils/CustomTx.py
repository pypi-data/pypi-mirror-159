#import inspect
from utils.utils import int2hex, convert_hex, encodeBech32AddressToHex
...
class DfTx():
    ...
    def hex(self):
        ...
        return OPCodes.OP_RETURN + lengthInfo + int2hex(int(len(h)/2),'little') + h
    ...
    def __handleParam(self,p,isCustomTxType=False):
        ...
        if p.__class__.__name__ == 'Vault':
            return convert_hex(p.value,'big','little')
        elif p.__class__.__name__ == 'Address':
            return encodeBech32AddressToHex(p.value)
        ...
        #     return int2hex(len(p.value),'little') + h
            ...
            for amount in p.value:
                h = h + int2hex(amount[1],'little',4) + int2hex(int(amount[0]*DECIMALS),'little',8)
            return int2hex(len(p.value),'little') + h
            ...
            for address in d:
                ...
                h = h + '01' + encodeBech32AddressToHex(address) + int2hex(len(d[address].keys()),'little')
                for tokenId in d[address]:
                    h = h + int2hex(tokenId,'little',4) + int2hex(int(d[address][tokenId]*DECIMALS),'little',8)
            return h
        else:
            return ''
