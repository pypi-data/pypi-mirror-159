from utils import ParamType as pType
...
class TakeLoan(CustomTx.DfTx):
    def __init__(self,vaultId:str,address:str,amounts:list,rpcConnector):
        ...
        self.params = [CustomTx.CustomTxType.TakeLoan,pType.Vault(vaultId),pType.Address(address),pType.Amounts(resAmounts)]
...
class AddPoolLiquidity(CustomTx.DfTx):
    def __init__(self,amountPerAddress:list[tuple[str,str]],shareAddress:str,rpcConnector):
        ...
        self.params = [CustomTx.CustomTxType.AddPoolLiquidity,pType.AddressesWithAmount(resAddressAmount),pType.Address(shareAddress)]
