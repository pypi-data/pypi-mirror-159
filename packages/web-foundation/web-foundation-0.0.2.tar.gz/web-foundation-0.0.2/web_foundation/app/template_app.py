...
from web_foundation.app.config import DbConfig
from web_foundation.kernel import IDistributor
from web_foundation.workers import TaskIMessage
...
class AnyService(Service):
    ...
    async def person_details(self, inc: InputContext, prot: ProtectIdentity):
        ...
        await inc.channel.produce(TaskIMessage("bg_worker", self.test, args=("asfasf",)))
        return {}
...
async def main():
    ...
    config = app.load_config("config.json", AppConfg)
    ...
    web_server = WebServer(f"{app.name}_sanic_{1}", app.config.server, sock)
