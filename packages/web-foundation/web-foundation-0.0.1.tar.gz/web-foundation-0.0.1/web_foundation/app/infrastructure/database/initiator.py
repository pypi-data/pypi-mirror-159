...
class DatabaseInitiator(Initiator):
    ...
    def __init__(self, app: "App", modules: List[ModuleType],
    ...
    async def _migrations(self, schema_exists: bool):
    ...
    async def configure_db(self):
        ...
        if not self.db_conf.with_migrations or not self.db_conf.migrations_path:
            raise ValueError("Field needmigrations_aeirch in db config set to false, can't migrate")
        else:
            await self._migrations(schema_exists)
