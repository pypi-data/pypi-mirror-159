...
from mldesigner._generate._generators._module_generator import SUPPORTED_COMPONENT_TYPES
    ...
    def match_remote_pattern(cls, match_method, asset, result):
        ...
        if workspace_name in result:
            ...
            if workspace_tuple != cls.match_remote(existing_pattern):
                raise UserErrorException(f"Pattern {asset} and {existing_pattern} has conflict module name.")
            else:
        ...
        with tqdm(
            ...
            for file in component_files:
                ...
                try:
                    ...
                    component_type = yaml_dict["type"]
                    if component_type in SUPPORTED_COMPONENT_TYPES:
                        component = load_component(path=file)
                        components.append(component)
                    else:
                        raise UserErrorException(f"Not supported component type {component_type}")
...
@monitor_with_activity(generate_pkg_logger, "mldesigner.generate_package", ActivityType.PUBLICAPI)
def _generate_package(
    ...
    **kwargs,
    ...
    if isinstance(assets, list):
        assets = AssetsUtil.load_from_list(assets)
    elif isinstance(assets, dict):
        assets = AssetsUtil.load_from_dict(assets)
    elif isinstance(assets, str):
        assets = AssetsUtil.load_from_file(assets)
    else:
        raise UserErrorException(f"Only list, dict and str are supported for assets, got {type(assets)}.")

    warning_summary, error_summary = [], []
    working_dir = Path(".")
    generate_pkg_logger.info(f"========== Generating package in {working_dir.absolute().as_posix()} ==========")
    with TimerContext() as timer:
        assets, pattern_to_components, warnings = AssetsUtil.parse_validate_asset_dict(assets)
        warning_summary += warnings
