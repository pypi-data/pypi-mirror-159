...
class ZbxCommon(object):
    ...
    def get_dependencies(self, auto_install=False):
        ...
        matchs = re.findall('from\s+([\w]+)\s+import\s+\w+|import\s+([\w]+)', content)
        need = []
        for m in matchs:
            for i in m:
                if i != '':
    ...
    def dict2sql(self, table_name, table_data):
        row = {}
        ...
        fields = list(map(lambda x: '`%s`' % (x), fields))
        fields_str = ','.join(list(fields))
        data_str = ",\n".join(rows)
        sql = "INSERT INTO {table_name} ({fields_str}) \nVALUES\n{data_str};".format(table_name=table_name,
    ...
    def dump_mysql_ddl(self, conn, dir='', tables=[]):
        tables_info = []
        infos = []
        if len(tables) == 0:
        ...
        for table in tables:
            ...
            infos.append(info[0]['Create Table'] + ";")
        if dir != "":
            with open("{dir}/ddl.txt".format(dir=dir), 'w+') as fp:
                fp.write("\n\n\n".join(infos))
        return tables_info

    def dump_mysql_data(self, conn, dir='', limit=10, tables=[]):
        ...
        for table in tables:
            ...
            sql_str = self.dict2sql(table, table_data)
    ...
    def get_kafka_producer(self, dsn):
        '''
        :param dsn: kafka://127.0.0.1:9002/topic?bootstrap_servers=127.0.0.1:9002,127.0.0.1:9003
        :return:
        '''
        import dsnparse
        ...
        if bootstrap_servers is None:
            bootstrap_servers = info.hostloc
    ...
    def get_kafka_consumer(self, dsn):
        ...
        if bootstrap_servers is None:
            bootstrap_servers = info.hostloc
        consumer = self.kafka.KafkaConsumer(info.dbname,bootstrap_servers, **config)
        return consumer
...
dsn="kafka://127.0.0.1:9002/topic?group_id=2&bootstrap_servers=127.0.0.1:9002,127.0.0.1:9003"

cli.get_kafka_consumer(dsn)

dsn="kafka://127.0.0.1:9002/topic?bootstrap_servers=127.0.0.1:9002,127.0.0.1:9003"

cli.get_kafka_producer(dsn)
