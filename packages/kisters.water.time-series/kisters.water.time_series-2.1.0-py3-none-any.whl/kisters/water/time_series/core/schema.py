...
class EnsembleMember(Model):
    ...
    def check_t0(cls, value: Any) -> Any:
        if isinstance(value, np.datetime64):
            return value.astype("datetime64[ms]").astype(object).replace(tzinfo=timezone.utc)  # type: ignore
        return value
...
class TimeSeriesMetadata(Model):
    ...
    timezone: str = "UTC"
    _client: Any = None  # Placeholder so underscore_attrs properly works
    _tzinfo: Optional[BaseTzInfo] = None  # Automatically generated from timezone

    def __init__(self, **data: Any):
        super().__init__(**data)
        self._tzinfo = pytz.timezone(self.timezone)
