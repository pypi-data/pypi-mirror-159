...
class TimeSeriesStoreBasicTest:
    ...
    ENSEMBLE_MEMBERS = [
        EnsembleMember(
            t0=datetime(year=2020, month=1, day=1, tzinfo=utc),
            dispatch_info="info1",
            member="0",
        ),
        EnsembleMember(
        ...
        EnsembleMember(
            t0=datetime(year=2020, month=1, day=1, hour=12, tzinfo=utc),
            dispatch_info="info1",
            member="0",
        ),
        EnsembleMember(
    ...
    def test_07_write_data_frame(self):
        ...
        assert np.allclose(df.values, read_df.values)
    ...
    def test_11_bulk_read(self):
        ...
        start = datetime(year=2020, month=2, day=1, tzinfo=utc)
        end = datetime(year=2020, month=3, day=15, tzinfo=utc)
        for ensemble in self.ENSEMBLE_MEMBERS:
            t0s.append(ensemble.t0)
            dispatch_infos.append(ensemble.dispatch_info)
            members.append(ensemble.member)
