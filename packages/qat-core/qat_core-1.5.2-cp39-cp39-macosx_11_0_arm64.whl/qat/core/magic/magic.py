...
class QAT(Magics):
    ...
    def plot(self, circuit, depth, filename=None):
        ...
        if filename is not None and not filename.endswith(".pdf"):
            raise ValueError("Filename must end with *.pdf, got %s" % filename)

        try:
            # Create a temporary PDF file
            tmp_pdf_file = (
                tmpf.NamedTemporaryFile(prefix="qat2pdf_", suffix='_circ.pdf', dir=".")
            ...
            with tmp_pdf_file, tmpf.NamedTemporaryFile(prefix="qat2pdf_", suffix="_circ.png", dir=".") as tmp_png_image:
                ...
                # Create PNG file
                pdf2png(pdf_file=tmp_pdf_filename_radix + '.pdf', png_file=tmp_png_image.name)

                # Display image
                self.shell.run_cell("import IPython; IPython.display.Image('%s')" % tmp_png_image.name,
                                    store_history=False)
    ...
    def cell2circ(self, cell):
        ...
        with tmpf.NamedTemporaryFile(suffix='.aqasm') as tmp_file_aq, \
             tmpf.NamedTemporaryFile(suffix='.circ') as tmp_file_ci:
