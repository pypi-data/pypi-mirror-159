...
class ElectraAPI(object):
    ...
    async def _get_sid(self, force: bool = False) -> None:
        ...
        if self._last_sid_request_ts and current_ts < (
            ...
            logger.debug(
                "Session ID was requested less than 5 minutes ago! waiting in order to"
                ' prevent "intruder lockdown"...'
            )
    ...
    async def get_devices(self, with_last_telemtry: bool = False) -> list:
        fetch_state_tasks = []
        await self._get_sid()
        ...
        if resp[Attributes.STATUS] == STATUS_SUCCESS:
            for ac in resp[Attributes.DATA][Attributes.DEVICES]:
                if ac["deviceTypeName"] == "A/C":
                    ...
                    if with_last_telemtry:
                        fetch_state_tasks.append(
                            create_task(self.get_last_telemtry(ac_list[ac]))
                        )
