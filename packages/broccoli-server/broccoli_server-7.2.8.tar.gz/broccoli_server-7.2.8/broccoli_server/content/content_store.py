...
class ContentStore(object):
    ...
    def append(self, doc: Dict, idempotency_key: str):
        if idempotency_key not in doc:
            logger.error(
                "Idempotency key is not found in payload",
                extra={"idempotency_key": idempotency_key, "payload": doc},
            )
            return

        idempotency_value = doc[idempotency_key]
        existing_doc_count = self.collection.count_documents(
            {idempotency_key: idempotency_value}
        )
        if existing_doc_count != 0:
            logger.info(
                f"Document with {idempotency_key}={idempotency_value} is already present"
            )
            return
    ...
    def append_multiple(self, docs: List[Dict], idempotency_key: str):
        ...
        for doc in docs:
            if idempotency_key not in doc:
                logger.error(
                    "Idempotency key is not found in one of the payloads",
                    extra={"idempotency_key": idempotency_key, "payload": doc},
                )
            else:
        ...
        for doc in keyed_docs:
            ...
            if existing_doc_count != 0:
                logger.info(
                    f"Document with {idempotency_key}={idempotency_value} is already present in persistence"
                )
                continue
            ...
            if doc_exists_in_batch:
                logger.info(
                    f"Document with {idempotency_key}={idempotency_value} is already present in current batch"
                )
                continue
    ...
    def query(
        ...
        datetime_q: Optional[List[Dict]] = None,
        skip: Optional[int] = None,
    ) -> List[Dict]:
        # Append datetime query
        if datetime_q:
            for qd in datetime_q:
                q[qd["key"]] = {"$" + qd["op"]: milliseconds_to_datetime(qd["value"])}
    ...
    def update_one(self, filter_q: Dict, update_doc: Dict, allow_many: bool = False):
        ...
        if existing_doc_count == 0:
            logger.error("Document does not exist", extra={"query": filter_q})
            return

        more_than_one = existing_doc_count > 1
        if more_than_one and not allow_many:
            logger.error("More than one documents exist", extra={"query": filter_q})
            return
        ...
        logger.error(
            "More than one document is updated because of allow_many",
            extra={"filter_q": filter_q},
        )
    ...
    def update_one_binary_string(self, filter_q: Dict, key: str, binary_string: str):
        if not ContentStore._check_if_string_is_binary(binary_string):
            logger.error(
                "binary_string is not a 01 string", extra={"value": binary_string}
            )
            return
        # todo: should not update an existing field
        self.update_one(filter_q, {"$set": {key: binary_string}})

    def query_nearest_hamming_neighbors(
        self,
        q: Dict,
        binary_string_key: str,
        from_binary_string: str,
        max_distance: int,
    ) -> List[Dict]:
        ...
        for q_result in self.query(q, limit=None):
            if not ContentStore._check_if_q_result_has_valid_binary(
                q_result, binary_string_key, from_binary_string
            ):
                continue
            q_binary_string = q_result[binary_string_key]
            distance = ContentStore._compute_binary_hamming_distance(
                q_binary_string, from_binary_string
            )
            if distance <= max_distance:
                results.append(q_result)
        return results

    def query_n_nearest_hamming_neighbors(
        self, q: Dict, binary_string_key: str, from_binary_string: str, pick_n: int
    ) -> List[Dict]:
        # todo: use a metric tree
        if not ContentStore._check_if_string_is_binary(from_binary_string):
            logger.error(
                "from_binary_string is not a 01 string",
                extra={"value": from_binary_string},
            )
            return []
    ...
    @staticmethod
    def _check_if_q_result_has_valid_binary(
        q_result: Dict, binary_string_key: str, from_binary_string: str
    ) -> bool:
