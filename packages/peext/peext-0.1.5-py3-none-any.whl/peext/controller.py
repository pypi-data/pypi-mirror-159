...
class CHPControlMultiEnergy(G2PControlMultiEnergy):
    ...
    def power_cap(self):
        if self.el_power_led:
            ...
            return self.ref_gas_consume * self.conversion_factor_kgps_to_mw() * self.efficiency
...
class RegulatedP2HControlMultiEnergy(Controller):
    ...
    def get_power_load(self, multinet):
        try:
            ...
            power_load = multinet[NETS_ACCESS][self.name_net_power].load.loc[
                            self.elm_idx_power, 'p_mw'].values[:]
        return power_load

    def get_heat_production(self, multinet):
        try:
            return multinet[NETS_ACCESS][self.name_net_heat].heat_exchanger.at[self.__heat_exchanger_id, 'qext_w']
        except (ValueError, TypeError):
            return multinet[NETS_ACCESS][self.name_net_power].heat_exchanger.loc[self.__heat_exchanger_id, 'qext_w'].values[:]
    ...
    def calc_heat(self, power_in_w):
        return power_in_w * self.efficiency
    ...
    def write_to_net(self, multinet):
        try:
            multinet[NETS_ACCESS][self.name_net_power].load.at[
                self.elm_idx_power, 'p_mw'] = self.power_load
        except (ValueError, TypeError):
            multinet[NETS_ACCESS][self.name_net_power].load.loc[
                self.elm_idx_power, 'p_mw'] = self.power_load

        try:
            multinet[NETS_ACCESS][self.name_net_heat].heat_exchanger.at[self.__heat_exchanger_id, 'qext_w'] = -self.heat_energy
        except (ValueError, TypeError):
            multinet[NETS_ACCESS][self.name_net_heat].heat_exchanger.loc[self.__heat_exchanger_id, 'qext_w'].values[:] = -self.heat_energy
...
def create_p2h(mn, 
    ...
    p2h_heat_feed_in = ppipes.create_heat_exchanger(heat_net, from_junction=junction_heat_from, to_junction=junction_heat_to, 
                                                    diameter_m=heat_diameter_m, qext_w=-10000)
    return RegulatedP2HControlMultiEnergy(mn, p2h_gen, p2h_heat_feed_in, efficiency, name)
