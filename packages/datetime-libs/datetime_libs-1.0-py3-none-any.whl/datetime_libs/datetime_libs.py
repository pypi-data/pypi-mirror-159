...
def get_next_prev_datetime_and_timestamp(**kwargs):
    ...
    dt_ts = {'datetime': next_prev_dt_ts, 'timestamp': next_prev_dt_ts.timestamp()}
    return dt_ts
...
def date_string_to_datetime_and_timestamp(date_str: str, date_format: str, time_part: bool = True, tz=None):
    ...
    try:
        ...
        local_dt = datetime.strptime(date_str, date_format).replace(tzinfo=ZoneInfo(ctz))
...
def start_datetime_timestamp(**kwargs):
    try:
        ...
        local_dt = datetime.strptime(date_str, format_str).replace(tzinfo=ZoneInfo(ctz))
