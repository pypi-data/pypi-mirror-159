...
def call_named_album_gui_conda_path():
    ...
    cmd = subprocess.run(["conda", "run", "-n", 'album', "album", "uninstall", "--solution", '<solution>'])
...
def call_prefixed_album_gui_conda_path():
    ...
    cmd = subprocess.run(
        ["conda", "run", "-p", str(Path.home().joinpath('.album', 'envs', 'album')), "album", "uninstall", "--solution",
         '<solution>'])
    return cmd.returncode
...
def call_named_album_gui_conda_exe():
    ...
    if platform.system() == 'Windows':
        cmd = subprocess.run(
            [str(Path.home().joinpath('.album', 'Miniconda', 'condabin', 'conda.bat')), "run", "-n", 'album', "album", "uninstall",
             "--solution", '<solution>'])
        return cmd.returncode
    else:
        cmd = subprocess.run(
            [Path.home().joinpath('.album', 'Miniconda', 'condabin', 'conda'), "run", "-n", 'album', "album", "uninstall",
             "--solution", '<solution>'])
        return cmd.returncode
...
def call_prefixed_album_gui_conda_exe():
    ...
    if platform.system() == 'Windows':
        ...
        cmd = subprocess.run([Path.home().joinpath('.album', 'Miniconda', 'condabin', 'conda'), "run", "-p",
                              Path.home().joinpath('.album', 'envs', 'album'), "album", "uninstall", "--solution",
                              '<solution>'])
