...
# TODO: avoid steep gradients -> vertically mirror the bands
...
for idx_luma_band in range(n_luma_bands):
    band_ = np.flip(luma_band, axis=0) if idx_luma_band % 2 == 1 else luma_band
    brightness_factor = 1. - (idx_luma_band / (n_luma_bands - 1))
    idx_start = idx_luma_band * band_.shape[0]
    result[idx_start: idx_start + band_.shape[0]] = band_ * brightness_factor
