...
def main(table_list, outfile, namefile, noheader, column, exclude=False):
    df_list = []
...
def run():
    ...
    parser.add_argument('-n', dest="namefile", type=argparse.FileType('r'), default=(None if sys.stdin.isatty() else sys.stdin),
                        help="whose name is listed in FILE.")
    ...
    parser.add_argument('-e', dest="exclude", action="store_true", help="name is not listed in FILE.")
    args = parser.parse_args()
    main(args.input, args.output, args.namefile, args.noheader, args.column, args.exclude)
