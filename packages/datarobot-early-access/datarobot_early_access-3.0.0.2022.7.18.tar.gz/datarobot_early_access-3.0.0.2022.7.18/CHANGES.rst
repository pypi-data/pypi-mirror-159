...
- Altered method :meth:`Model.request_predictions <datarobot.models.Model.request_predictions>` to accept a new parameter ``dataset`` - this is in addition to the existing parameter ``dataset_id`` - this new parameter accepts a :class:`Dataset <datarobot.models.Dataset>` and is mutually exclusive with ``dataset_id``
