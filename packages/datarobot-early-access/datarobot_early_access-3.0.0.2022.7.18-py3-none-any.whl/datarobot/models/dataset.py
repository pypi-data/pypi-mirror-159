...
class Dataset(APIObject, BrowserMixin):
    ...
    def upload(
        ...
        ------
        InvalidUsageError
            If source parameter cannot be determined to be a URL, filepath, file or DataFrame.
        ...
        if isinstance(source, str):
            ...
            if parse_result.scheme and parse_result.netloc:
                return cls.create_from_url(url=source)
            elif Path(source).is_file():
                return cls.create_from_file(file_path=source)
            else:
            ...
            return cls.create_from_file(filelike=source)
        else:
    ...
    def create_from_file(
        ...
        dataset: TDataset = cls.from_location(new_dataset_location)
        if categories:
    ...
    @classmethod
    def get(cls, dataset_id: str) -> TDataset:
    ...
    def update(self) -> None:
        ...
        None
        """
        new_dataset: Dataset = self.get(self.id)
        update_attrs = (
            "name",
            "created_by",
            "created_at",
    ...
    def get_details(self) -> TDatasetDetails:
        """
...
class DatasetDetails(APIObject):
    ...
    @classmethod
    def get(cls, dataset_id: str) -> TDatasetDetails:
        """
