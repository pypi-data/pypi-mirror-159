...
class Layers(Enum):
    ...
    def set_node_items(
        self,
        nodes: list[dict],
        label_indicator: str = "item",
        children_indicator: str = "units",
        level: int = 1,
    ) -> None:
        """Add a `label_indicator` key (defaults to "item") to each node in the
        `nodes` list recursively.

        Recursion happens on every `children_indicator` key (defaults to
        ...
        if isinstance(nodes, list):
            for idx, node in enumerate(nodes, start=0):
                if label_indicator not in node:
                    item_type: ListType = self.get_item_type(level)
                    category: list = item_type.value
                    node[label_indicator] = category[idx]
                if node.get(children_indicator, None):
                    self.set_node_items(
