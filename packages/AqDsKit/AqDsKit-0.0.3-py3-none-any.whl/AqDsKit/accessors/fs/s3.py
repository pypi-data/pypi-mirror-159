...
class S3Configs:
    ...
    def create_client(self, session):
        return session.client(
            self.service_name, api_version = self.api_version, use_ssl = self.use_ssl,
            verify = self.verify, endpoint_url = self.endpoint_url,
            config = self.config
        )

    def create_resource(self, session):
...
class S3ConfigOs(S3Configs):

    def __init__(self, botocore_session = None, use_ssl: bool = None, config = None):
        super().__init__(
            ...
            service_name = os.getenv('service_name', 's3'),
...
class S3Accessor:
    def __init__(self, s3_config: S3Configs):
        try:
            self.session = s3_config.create_session()
            self.client = s3_config.create_client(self.session)
            self.resource = s3_config.create_resource(self.session)

        except Exception as e:
            logging.error(f"Boto3 failed to create client to access {s3_config.service_name} \n{e}")
        else:
    ...
    def delete_obj(self, remote_path: str, bucket_name: str = None) -> int:
        bucket_name = self.if_not_exist_return_default_bucket(bucket_name)
        self.resource.Object(Bucket = bucket_name, Key = remove_begin_slash(remote_path)).delete()
        return 1
