...
class ClfValidator(Validator):
    def __init__(self, clf, Xs, y_actual, y_labels=None):
        self.clf = clf
        self.Xs = Xs
        ...
        if len(self.y_labels) == 2:  # adjust for binary classifier -- increase to 2D dimension
    ...
    def precision_recall(self, average: str = "macro", plot: bool = True, figsize = (5, 5)) -> dict:
        ...
        for i, label in enumerate(self.y_labels):
            ...
            aps[label] = self.average_precision_score()
        ...
        if plot:
            ...
            sns.lineplot(x=pre_avg, y=rec_avg, label=legend, linewidth=4, color="navy", linestyle="--")
...
class ClfValidatorCV(ClfValidator):
    ...
    def __init__(self, clf, Xs, y_actual, y_labels=None, cv=3):
        self.clf = clf
        self.Xs = Xs
        self.y_actual = y_actual
...
class ClfModelSelector(ModelSelector):
    def __init__(self, clf_dict: dict, Xs, y_actual, y_labels = None):
        self.models = {k : ClfValidator(clf, Xs, y_actual, y_labels) for k, clf in clf_dict.items()}
...
class ClfModelSelectorCV(ClfModelSelector):
    def __init__(self, clf_dict: dict, Xs, y_actual, y_labels = None):
        self.models = {k : ClfValidatorCV(clf, Xs, y_actual, y_labels) for k, clf in clf_dict.items()}
