...
def get_head() -> Commit:
    ...
    print("git log stdout:", result.stdout)
    head, parents = result.stdout.decode().strip().split(",")
    return Commit(head, *parents.split(" "))
...
def changed_since(start_commit: str, file_pattern: str) -> bool:
    cmd = ["git", "diff", "--name-only", start_commit, "HEAD"]
    result = run(cmd, stdout=PIPE)
    filenames = result.stdout.decode().strip().split("\n")
    matches = fnmatch.filter(filenames, file_pattern)
...
def _is_ancestor(commit_a: str, commit_b: str) -> bool:
    ...
    result = run(cmd, stdout=PIPE)
    print("is ancestor:", commit_a, commit_b, result.returncode)
