...
if toolkit in ('pyside2', 'qt5'):
    ...
    class MouseMixin(MouseBase):
        ...
        def mousePressEvent(self, event):
            self.mouse_pressed = (event.button() == self.button)
        
        def mouseReleaseEvent(self, event):
            if self.mouse_pressed and (event.button() == self.button):
    ...
    class MouseMixin(MouseBase):
        ...
        def OnLeftUp(self, event):
            if self.mouse_pressed:
                self.forward_event(self.event_signal, event, *self.event_args)
            self.mouse_pressed = False
