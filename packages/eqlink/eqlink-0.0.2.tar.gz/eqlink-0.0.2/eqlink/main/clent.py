...
class LinkClient:
    ...
    def client_int(self, data_to_server):
        ...
        try:
            ...
            '''
            print('[eqlink] [连接服务端] [success] IP:' + self.server_conf['IP'])
        except socket.error as e:
            print("[eqlink] connected to server error: %s" + str(e))
            sys_exit()
        ...
        while True:
            ...
            try:
                ...
                print('[eqlink] [获取Provider服务列表]:', str(data, 'UTF-8'))
            except socket.error:
                print("[eqlink] Send failed!!")
                sys_exit()
